...
# --- HealthDash / MedSafe voice + in-house vector-memory patch -----------------
# This block is intentionally self-contained: no Weaviate server, no external API,
# and no required audio dependency at import time. Optional STT/TTS packages are
# loaded lazily when the voice buttons are used.

VOICE_PATCH_VERSION = "2026-05-08.single-file.voice-memory-barkcpp"
VOICE_MEMORY_MAX_DOCS = 640
VOICE_MEMORY_VECTOR_DIMS = 384
VOICE_MEMORY_CONTEXT_TOP_K = 8
VOICE_MEMORY_CONTEXT_MAX_CHARS = 5200


def _voice_patch_sha(value: Any) -> str:
    return hashlib.sha256(str(value or "").encode("utf-8", "replace")).hexdigest()


def _voice_patch_tokens(value: Any) -> List[str]:
    text = sanitize_text(value, max_chars=12000).lower()
    return re.findall(r"[a-z0-9][a-z0-9_'-]{1,48}", text)


def _voice_patch_tags(value: Any) -> List[str]:
    lowered = sanitize_text(value, max_chars=8000).lower()
    tag_rules = {
        "therapy": ("anxious", "sad", "overwhelmed", "panic", "feel", "stress", "therapy", "cope", "ground"),
        "recovery": ("craving", "relapse", "sober", "clean", "urge", "trigger", "streak", "recovery"),
        "medication": ("dose", "mg", "pill", "med", "medication", "schedule", "late", "missed"),
        "dental": ("tooth", "teeth", "gum", "dental", "brush", "floss", "rinse", "crown", "extraction"),
        "movement": ("walk", "exercise", "stretch", "movement"),
        "voice": ("voice", "listen", "microphone", "bark", "speak", "audio"),
    }
    tags = [tag for tag, needles in tag_rules.items() if any(n in lowered for n in needles)]
    return tags or ["general"]


def _voice_patch_sentiment_hint(value: Any) -> float:
    tokens = set(_voice_patch_tokens(value))
    positive = {"good", "better", "calm", "safe", "ready", "proud", "progress", "thanks", "helped", "clear"}
    negative = {"bad", "worse", "anxious", "panic", "scared", "unsafe", "craving", "relapse", "overwhelmed", "angry"}
    score = (len(tokens & positive) - len(tokens & negative)) / max(4.0, float(len(tokens) or 1))
    return max(-1.0, min(1.0, score * 4.0))


def _voice_patch_vector(value: Any, dims: int = VOICE_MEMORY_VECTOR_DIMS) -> List[float]:
    vec = [0.0] * int(dims)
    tokens = _voice_patch_tokens(value)
    if not tokens:
        return vec
    grams: List[str] = []
    grams.extend(tokens)
    ...
    for gram in grams:
        digest = hashlib.blake2b(gram.encode("utf-8", "replace"), digest_size=8).digest()
...
def _voice_patch_cosine(left: List[float], right: List[float]) -> float:
...
class InHouseWeaviateLikeVectorDB:
    """Tiny local replacement for the older Weaviate InteractionHistory idea.

    It stores compact documents inside the encrypted app vault data structure and
    uses deterministic hashed lexical vectors. That makes it single-file, offline,
    auditable, and safe to pass through the existing Gemma worker snapshot.
    """

    def __init__(self, data: Dict[str, Any]):
        self.data = data
        memory = self.data.get("assistant_vector_memory")
        if not isinstance(memory, dict):
            memory = {}
        docs = memory.get("docs")
        if not isinstance(docs, list):
            docs = []
        memory["docs"] = docs[-VOICE_MEMORY_MAX_DOCS:]
        memory["schema"] = "InHouseInteractionHistory/v2"
        memory["updated_ts"] = safe_float(memory.get("updated_ts")) or time.time()
        self.data["assistant_vector_memory"] = memory
        self.memory = memory

    @property
    def docs(self) -> List[Dict[str, Any]]:
        docs = self.memory.get("docs")
        return docs if isinstance(docs, list) else []

    def add_turn(self, user_text: str, assistant_text: str, *, mode: str = "General") -> bool:
        clean_user = sanitize_text(user_text, max_chars=2400)
        clean_assistant = sanitize_text(assistant_text, max_chars=6000)
        if not clean_user or not clean_assistant:
            return False
        joined = f"User: {clean_user}\nAssistant: {clean_assistant}"
        signature = _voice_patch_sha(f"{normalize_assistant_mode(mode)}\n{joined}")
        if any(str(doc.get("id")) == signature for doc in self.docs[-80:]):
            return False
        doc = {
            "id": signature,
            "kind": "InteractionHistory",
            "mode": normalize_assistant_mode(mode),
            "user": clean_user,
            "assistant": clean_assistant,
            "summary": self._summarize_turn(clean_user, clean_assistant),
            "tags": _voice_patch_tags(joined),
            "sentiment": _voice_patch_sentiment_hint(joined),
            "timestamp": time.time(),
            "vector": _voice_patch_vector(joined),
        }
        docs = self.docs + [doc]
        self.memory["docs"] = docs[-VOICE_MEMORY_MAX_DOCS:]
        self.memory["updated_ts"] = time.time()
        return True

    def search(self, query: str, *, mode: str = "General", top_k: int = VOICE_MEMORY_CONTEXT_TOP_K) -> List[Dict[str, Any]]:
        clean_query = sanitize_text(query, max_chars=2400)
        if not clean_query:
            return []
        q_vec = _voice_patch_vector(f"{normalize_assistant_mode(mode)} {clean_query}")
        q_tags = set(_voice_patch_tags(clean_query))
        scored: List[Tuple[float, Dict[str, Any]]] = []
        now = time.time()
        for doc in self.docs:
            vec = doc.get("vector") if isinstance(doc.get("vector"), list) else []
            score = _voice_patch_cosine(q_vec, vec)
            tags = set(doc.get("tags") or [])
            if q_tags & tags:
                score += 0.08
            if normalize_assistant_mode(doc.get("mode")) == normalize_assistant_mode(mode):
                score += 0.04
            age_days = max(0.0, (now - safe_float(doc.get("timestamp"))) / 86400.0)
            score += max(0.0, 0.05 * (1.0 - min(age_days, 14.0) / 14.0))
            if score > 0.06:
                scored.append((score, doc))
        scored.sort(key=lambda item: item[0], reverse=True)
        return [dict(doc, _score=round(score, 4)) for score, doc in scored[: max(1, int(top_k))]]

    def _summarize_turn(self, user_text: str, assistant_text: str) -> str:
        user_short = sanitize_text(user_text, max_chars=420)
        assistant_plain = markdown_to_plain_text(assistant_text, max_chars=900)
        sentences = re.split(r"(?<=[.!?])\s+", assistant_plain)
        useful = " ".join(sentence for sentence in sentences[:2]).strip() or assistant_plain[:420]
        return sanitize_text(f"User asked: {user_short}\nUseful answer memory: {useful}", max_chars=900)

    def context_block(self, query: str, *, mode: str = "General") -> str:
        hits = self.search(query, mode=mode)
        if not hits:
            return "[in_house_vector_memory]\nNo relevant older vector memories found.\n[/in_house_vector_memory]"
        lines = [
            "[in_house_vector_memory]",
            "Local Weaviate-like InteractionHistory recall. Use as soft memory only; current user request and vault facts win.",
        ]
        total = 0
        for index, hit in enumerate(hits, 1):
            summary = sanitize_text(hit.get("summary") or "", max_chars=780)
            tags = ", ".join([sanitize_text(tag, max_chars=24) for tag in list(hit.get("tags") or [])[:6]])
            stamp = format_timestamp(safe_float(hit.get("timestamp")))
            line = f"{index}. score={hit.get('_score')} mode={normalize_assistant_mode(hit.get('mode'))} tags={tags} when={stamp}\n{summary}"
            total += len(line)
            if total > VOICE_MEMORY_CONTEXT_MAX_CHARS:
                lines.append("[memory trimmed]")
                break
            lines.append(line)
        lines.append("[/in_house_vector_memory]")
        return "\n".join(lines)


def build_in_house_vector_memory_context(data: Dict[str, Any], prompt: str, mode: str) -> str:
    try:
        return InHouseWeaviateLikeVectorDB(data).context_block(prompt, mode=mode)
    except Exception as exc:
        return f"[in_house_vector_memory]\nMemory unavailable: {sanitize_text(exc, max_chars=180)}\n[/in_house_vector_memory]"


VOICE_PATCH_ORIGINAL_RUN_ASSISTANT_REQUEST = run_assistant_request
...
def run_assistant_request(
    ...
    active_mode = normalize_assistant_mode(mode)
    memory_block = build_in_house_vector_memory_context(data, prompt, active_mode)
    enhanced_prompt = "\n\n".join(
        [
            sanitize_text(prompt, max_chars=1600),
            memory_block,
            "Voice mode note: when this request came from speech, answer in shorter spoken-friendly chunks unless the user asked for detail.",
        ]
    )
    return VOICE_PATCH_ORIGINAL_RUN_ASSISTANT_REQUEST(
        key,
        data,
        enhanced_prompt,
        selected_med_id,
...
class BarkCppVoiceEngine:
    def __init__(self, app: "DesktopMedSafeApp") -> None:
        self.app = app
    ...
    def default_binary_path(self) -> Path:
        raw = os.environ.get("BARK_CPP_BIN", "").strip()
        if raw:
            return Path(raw).expanduser()
        paths = require_paths()
        exe = "main.exe" if sys.platform.startswith("win") else "main"
        return paths.models_dir / "bark.cpp" / "build" / "examples" / "main" / exe

    def default_model_path(self) -> Path:
        raw = os.environ.get("BARK_CPP_MODEL", "").strip()
        if raw:
            return Path(raw).expanduser()
        paths = require_paths()
        return paths.models_dir / "bark.cpp" / "models" / "bark-small" / "ggml_weights_q8_0.bin"

    def availability_report(self) -> str:
        binary = self.default_binary_path()
        model = self.default_model_path()
        parts = []
        parts.append("Bark binary: found" if binary.exists() else f"Bark binary missing: {binary}")
        parts.append("q8_0 model: found" if model.exists() else f"q8_0 model missing: {model}")
        return " | ".join(parts)
    ...
    def speak_async(self, text: str) -> None:
        clean = markdown_to_plain_text(text, max_chars=1600)
    ...
    def _speak_worker(self, text: str) -> None:
        ...
        if not binary.exists() or not model.exists():
            self.app.run_on_ui_thread(self.app._voice_set_status, self.availability_report())
            return
        self.stop_event.clear()
        wav = require_paths().temp_dir / f"bark_reply_{os.getpid()}_{uuid.uuid4().hex}.wav"
        command = [str(binary), "-m", str(model), "-p", text, "-o", str(wav), "-t", str(max(2, min(8, (os.cpu_count() or 4))))]
        self.app.run_on_ui_thread(self.app._voice_set_status, "Bark.cpp is generating voice...")
        try:
            import subprocess
            with self.lock:
                self.playing = True
            ...
            if self.stop_event.is_set():
                self.app.run_on_ui_thread(self.app._voice_set_status, "Voice interrupted.")
                return
            if proc.returncode != 0:
                message = sanitize_text((err or b"").decode("utf-8", "replace"), max_chars=260)
                self.app.run_on_ui_thread(self.app._voice_set_status, f"Bark.cpp failed: {message or proc.returncode}")
                return
            self._play_wav(wav)
        except Exception as exc:
            self.app.run_on_ui_thread(self.app._voice_set_status, f"Voice unavailable: {sanitize_text(exc, max_chars=220)}")
        finally:
            with self.lock:
                self.current_process = None
    ...
    def _play_wav(self, wav: Path) -> None:
        ...
        try:
            ...
            data, samplerate = _sf.read(str(wav), dtype="float32")
            self.app.run_on_ui_thread(self.app._voice_set_status, "Speaking with Bark.cpp. Start talking to interrupt.")
            _sd.play(data, int(samplerate), blocking=False)
            while not self.stop_event.is_set():
                try:
                    if not _sd.get_stream().active:
                        break
                except Exception:
                    break
                time.sleep(0.05)
            if self.stop_event.is_set():
                ...
                self.app.run_on_ui_thread(self.app._voice_set_status, "Voice reply finished.")
            return
        except Exception:
            pass
        try:
            # Last-resort playback: not sample-accurate and not reliably interruptible,
            # but keeps the single-file patch useful on machines without sounddevice.
            if sys.platform.startswith("win"):
                os.startfile(str(wav))  # type: ignore[attr-defined]
            elif sys.platform == "darwin":
                import subprocess
                subprocess.Popen(["open", str(wav)])
            else:
                import subprocess
                subprocess.Popen(["xdg-open", str(wav)])
            self.app.run_on_ui_thread(self.app._voice_set_status, "Opened Bark wav with the OS audio player.")
        except Exception as exc:
            self.app.run_on_ui_thread(self.app._voice_set_status, f"Could not play Bark wav: {sanitize_text(exc, max_chars=180)}")


class VoiceActivityInterruptor:
    def __init__(self, app: "DesktopMedSafeApp") -> None:
        self.app = app
        self.thread: Optional[threading.Thread] = None
        self.stop_event = threading.Event()
        self.energy_floor = 320.0
        self.voice_frames = 0
    ...
    def _worker(self) -> None:
        try:
            ...
            self.app.run_on_ui_thread(self.app._voice_set_status, f"Interrupt mic unavailable: {sanitize_text(exc, max_chars=180)}")
            return
        recognizer = sr.Recognizer()
        try:
            with sr.Microphone() as source:
                recognizer.adjust_for_ambient_noise(source, duration=0.6)
                self.energy_floor = max(220.0, float(getattr(recognizer, "energy_threshold", 320.0)))
                while not self.stop_event.is_set():
                    try:
                        audio = recognizer.listen(source, timeout=0.35, phrase_time_limit=0.55)
                    except Exception:
                        continue
                    raw = audio.get_raw_data(convert_rate=16000, convert_width=2)
                    rms = float(audioop.rms(raw, 2)) if raw else 0.0
                    now = time.time()
                    speaking = bool(getattr(self.app, "bark_voice", None) and self.app.bark_voice.playing)
                    if rms > max(self.energy_floor * 1.55, self.energy_floor + 220.0):
                        self.voice_frames += 1
                    else:
                        self.voice_frames = max(0, self.voice_frames - 1)
                    if speaking and self.voice_frames >= 2 and now >= self.cooldown_until:
                        self.cooldown_until = now + 1.1
                        self.voice_frames = 0
                        self.app.run_on_ui_thread(self.app.on_voice_interrupt_detected, rms)
        except Exception as exc:
            self.app.run_on_ui_thread(self.app._voice_set_status, f"Interrupt detector stopped: {sanitize_text(exc, max_chars=180)}")
...
class LocalSpeechRecognizer:
    def __init__(self, app: "DesktopMedSafeApp") -> None:
        ...
        self._recording_chunks: List[Any] = []

    def recognize_once(self, *, phrase_time_limit: float = 18.0) -> str:
        try:
            import speech_recognition as sr  # type: ignore
        except Exception as exc:
            raise RuntimeError(f"Install SpeechRecognition plus faster-whisper or pocketsphinx for voice input. Detail: {exc}") from exc
        recognizer = sr.Recognizer()
        with sr.Microphone() as source:
            recognizer.adjust_for_ambient_noise(source, duration=0.45)
            audio = recognizer.listen(source, timeout=8.0, phrase_time_limit=phrase_time_limit)
    ...
    def _push_to_talk_worker(self) -> None:
        try:
            ...
            self.app.run_on_ui_thread(self.app._voice_set_status, f"STT unavailable: {sanitize_text(exc, max_chars=180)}")
            return
        recognizer = sr.Recognizer()
        try:
            with sr.Microphone() as source:
                recognizer.adjust_for_ambient_noise(source, duration=0.35)
                self.app.run_on_ui_thread(self.app._voice_set_status, "Push-to-talk recording. Release to send.")
                frames: List[bytes] = []
                ...
                while not self._recording_stop.is_set():
                    ...
                    if len(frames) > int(sample_rate / max(1, source.CHUNK) * 45):
                        break
                if not frames:
                    self.app.run_on_ui_thread(self.app._voice_set_status, "No voice captured.")
                    return
                audio = sr.AudioData(b"".join(frames), sample_rate, sample_width)
                self.app.run_on_ui_thread(self.app._voice_set_status, "Transcribing local voice...")
                text = self._recognize_audio(recognizer, audio)
                self.app.run_on_ui_thread(self.app._voice_submit_text, text)
        except Exception as exc:
            self.app.run_on_ui_thread(self.app._voice_set_status, f"Voice capture failed: {sanitize_text(exc, max_chars=220)}")

    def _recognize_audio(self, recognizer: Any, audio: Any) -> str:
        # Prefer offline/local recognizers. Cloud STT is opt-in only.
        try:
            if hasattr(recognizer, "recognize_whisper"):
                text = recognizer.recognize_whisper(audio, model=os.environ.get("VOICE_WHISPER_MODEL", "base.en"))
                return sanitize_text(text, max_chars=1600)
        except Exception:
            pass
        try:
            ...
            pass
        if os.environ.get("ALLOW_CLOUD_STT", "0") == "1":
            try:
                text = recognizer.recognize_google(audio)
                return sanitize_text(text, max_chars=1600)
            except Exception:
                pass
        raise RuntimeError("No offline STT result. Install faster-whisper/whisper or pocketsphinx, or set ALLOW_CLOUD_STT=1 for Google STT.")


VOICE_PATCH_ORIGINAL_DESKTOP_INIT = DesktopMedSafeApp.__init__


def _voice_patch_desktop_init(self: "DesktopMedSafeApp") -> None:
    VOICE_PATCH_ORIGINAL_DESKTOP_INIT(self)
    self.voice_mode = "push_to_talk"
    self.voice_listener_enabled = False
    self.voice_interrupt_enabled = True
    self.voice_tts_enabled = True
    self.voice_last_spoken_signature = ""
    self.voice_last_stored_signature = ""
    self.voice_last_user_prompt = ""
    self.voice_listener_thread = None
    self.voice_listener_stop = threading.Event()
    self.bark_voice = BarkCppVoiceEngine(self)
    self.voice_interruptor = VoiceActivityInterruptor(self)
    self.local_stt = LocalSpeechRecognizer(self)


DesktopMedSafeApp.__init__ = _voice_patch_desktop_init


VOICE_PATCH_ORIGINAL_BUILD_ASSISTANT_TAB = DesktopMedSafeApp._build_assistant_tab


def _voice_patch_build_assistant_tab(self: "DesktopMedSafeApp", tab: Any) -> None:
    VOICE_PATCH_ORIGINAL_BUILD_ASSISTANT_TAB(self, tab)
    if ctk is None:
        return
    chat_card = self.ids.get("assistant_chat_card")
    if chat_card is None:
        return
    try:
        chat_card.grid_rowconfigure(4, weight=0)
        voice_frame = ctk.CTkFrame(chat_card, fg_color=DESKTOP_SURFACE_ALT, corner_radius=16)
        voice_frame.grid(row=4, column=0, sticky="ew", padx=18, pady=(0, 16))
        voice_frame.grid_columnconfigure((0, 1, 2, 3), weight=1)
        self.ids["voice_frame"] = voice_frame
        ctk.CTkLabel(
            voice_frame,
            text="Bark.cpp Voice + In-house Vector Memory",
            anchor="w",
            text_color=DESKTOP_TEXT,
            font=ctk.CTkFont(size=14, weight="bold"),
        ).grid(row=0, column=0, columnspan=4, sticky="ew", padx=12, pady=(12, 4))
        voice_status = self._label(
            voice_frame,
            text="Voice ready. Set BARK_CPP_BIN and BARK_CPP_MODEL or place bark.cpp under the MedSafe models folder.",
            muted=True,
            wrap=820,
        )
        voice_status.widget.grid(row=1, column=0, columnspan=4, sticky="ew", padx=12, pady=(0, 10))
        self._register("voice_status_line", voice_status)
        ptt = self._button(voice_frame, text="Hold to Talk", command=lambda: None, tone="secondary")
        ptt.grid(row=2, column=0, sticky="ew", padx=(12, 6), pady=(0, 12))
        try:
            ptt.bind("<ButtonPress-1>", lambda _event: self.on_voice_ptt_down())
            ptt.bind("<ButtonRelease-1>", lambda _event: self.on_voice_ptt_up())
        except Exception:
            pass
        self.ids["voice_ptt_button"] = ptt
        listener = self._button(voice_frame, text="Listener Off", command=self.on_voice_toggle_listener, tone="neutral")
        listener.grid(row=2, column=1, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_listener_button"] = listener
        interrupt = self._button(voice_frame, text="Interrupt On", command=self.on_voice_toggle_interrupt, tone="neutral")
        interrupt.grid(row=2, column=2, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_interrupt_button"] = interrupt
        speak_toggle = self._button(voice_frame, text="Bark Speak On", command=self.on_voice_toggle_tts, tone="neutral")
        speak_toggle.grid(row=2, column=3, sticky="ew", padx=(6, 12), pady=(0, 12))
        self.ids["voice_tts_button"] = speak_toggle
        self._voice_set_status(self.bark_voice.availability_report())
        self.voice_interruptor.start()
    except Exception as exc:
        self.set_status(f"Voice UI patch failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp._build_assistant_tab = _voice_patch_build_assistant_tab


def _voice_set_status(self: "DesktopMedSafeApp", message: str) -> None:
    clean = sanitize_text(message, max_chars=420)
    label = self.ids.get("voice_status_line")
    if label is not None:
        try:
            label.text = clean
        except Exception:
...
DesktopMedSafeApp._voice_set_status = _voice_set_status


def _voice_submit_text(self: "DesktopMedSafeApp", text: str) -> None:
    clean = sanitize_text(text, max_chars=1600)
    if not clean:
        self._voice_set_status("No speech text captured.")
        return
    lowered = clean.lower().strip()
    if lowered in {"stop", "stop talking", "cancel", "interrupt"}:
        self.stop_bark_voice()
        return
    if "therapy mode" in lowered:
        self.on_assistant_mode_change("Therapy")
        clean = re.sub(r"\btherapy mode\b", "", clean, flags=re.I).strip() or "Can you help me ground and think this through?"
    elif "recovery mode" in lowered or "recovery coach" in lowered:
        self.on_assistant_mode_change("Recovery Coach")
        clean = re.sub(r"\b(recovery mode|recovery coach)\b", "", clean, flags=re.I).strip() or "Help me protect my recovery today."
    try:
        self.ids.assistant_input.text = clean
        self.on_assistant_send()
        self._voice_set_status(f"Voice sent: {clean[:120]}")
    except Exception as exc:
        self._voice_set_status(f"Could not submit voice text: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp._voice_submit_text = _voice_submit_text


def on_voice_ptt_down(self: "DesktopMedSafeApp") -> None:
    self.voice_mode = "push_to_talk"
    if self.voice_interrupt_enabled:
        self.stop_bark_voice()
    self.local_stt.start_push_to_talk()
    button = self.ids.get("voice_ptt_button")
    try:
        if button is not None:
            button.configure(text="Listening...")
    except Exception:
        pass


def on_voice_ptt_up(self: "DesktopMedSafeApp") -> None:
    self.local_stt.stop_push_to_talk()
    button = self.ids.get("voice_ptt_button")
    try:
        if button is not None:
            button.configure(text="Hold to Talk")
    except Exception:
        pass


DesktopMedSafeApp.on_voice_ptt_down = on_voice_ptt_down
DesktopMedSafeApp.on_voice_ptt_up = on_voice_ptt_up


def on_voice_toggle_listener(self: "DesktopMedSafeApp") -> None:
    self.voice_listener_enabled = not bool(self.voice_listener_enabled)
    button = self.ids.get("voice_listener_button")
    try:
        if button is not None:
            button.configure(text="Listener On" if self.voice_listener_enabled else "Listener Off")
    except Exception:
        pass
    if self.voice_listener_enabled:
        self.voice_listener_stop.clear()
        self.voice_mode = "listener"
        self.voice_listener_thread = threading.Thread(target=self._voice_listener_loop, daemon=True)
        self.voice_listener_thread.start()
        self._voice_set_status("Listener mode on. Say a short request; say 'stop' to interrupt Bark.")
    else:
        self.voice_listener_stop.set()
        self._voice_set_status("Listener mode off.")


DesktopMedSafeApp.on_voice_toggle_listener = on_voice_toggle_listener


def _voice_listener_loop(self: "DesktopMedSafeApp") -> None:
    while not self.voice_listener_stop.is_set():
        try:
            text = self.local_stt.recognize_once(phrase_time_limit=12.0)
            if text:
                self.run_on_ui_thread(self._voice_submit_text, text)
        except Exception as exc:
            self.run_on_ui_thread(self._voice_set_status, f"Listener waiting: {sanitize_text(exc, max_chars=220)}")
            time.sleep(1.0)


DesktopMedSafeApp._voice_listener_loop = _voice_listener_loop


def on_voice_toggle_interrupt(self: "DesktopMedSafeApp") -> None:
    self.voice_interrupt_enabled = not bool(self.voice_interrupt_enabled)
    button = self.ids.get("voice_interrupt_button")
    try:
        if button is not None:
            button.configure(text="Interrupt On" if self.voice_interrupt_enabled else "Interrupt Off")
    except Exception:
        pass
    if self.voice_interrupt_enabled:
        self.voice_interruptor.start()
        self._voice_set_status("Interrupt detection on. Talking over Bark stops playback.")
    else:
        self.voice_interruptor.stop()
        self._voice_set_status("Interrupt detection off.")


DesktopMedSafeApp.on_voice_toggle_interrupt = on_voice_toggle_interrupt


def on_voice_toggle_tts(self: "DesktopMedSafeApp") -> None:
    self.voice_tts_enabled = not bool(self.voice_tts_enabled)
    if not self.voice_tts_enabled:
        self.stop_bark_voice()
    button = self.ids.get("voice_tts_button")
    try:
        if button is not None:
            button.configure(text="Bark Speak On" if self.voice_tts_enabled else "Bark Speak Off")
    except Exception:
        pass
    self._voice_set_status("Bark TTS enabled." if self.voice_tts_enabled else "Bark TTS disabled.")


DesktopMedSafeApp.on_voice_toggle_tts = on_voice_toggle_tts


def stop_bark_voice(self: "DesktopMedSafeApp") -> None:
    try:
        self.bark_voice.stop()
    except Exception:
        pass
    self._voice_set_status("Voice stopped.")


DesktopMedSafeApp.stop_bark_voice = stop_bark_voice


def on_voice_interrupt_detected(self: "DesktopMedSafeApp", rms: float = 0.0) -> None:
    if not bool(self.voice_interrupt_enabled):
        return
    self.stop_bark_voice()
    self._voice_set_status(f"Interrupt detected from microphone energy {int(rms)}. Listening wins over speaking.")


DesktopMedSafeApp.on_voice_interrupt_detected = on_voice_interrupt_detected


def _voice_memory_store_latest_turn(self: "DesktopMedSafeApp") -> None:
    try:
        history = list(self.data_cache.get("assistant_history") or [])
        last_user = ""
        last_assistant = ""
        last_mode = normalize_assistant_mode(getattr(self, "assistant_mode", "General"))
        for item in reversed(history):
            role = sanitize_text(item.get("role") or "", max_chars=24).lower()
            content = sanitize_text(item.get("content") or "", max_chars=10000)
            if role == "assistant" and not last_assistant:
                last_assistant = content
                last_mode = normalize_assistant_mode(item.get("mode") or last_mode)
            elif role == "user" and last_assistant:
                last_user = content
                break
        signature = _voice_patch_sha(f"{last_mode}\n{last_user}\n{last_assistant}")
        if not last_user or not last_assistant or signature == getattr(self, "voice_last_stored_signature", ""):
            return
        db = InHouseWeaviateLikeVectorDB(self.data_cache)
        if db.add_turn(last_user, last_assistant, mode=last_mode):
            self.voice_last_stored_signature = signature
            self.persist_data_async()
            self.refresh_assistant_context_panel()
    except Exception as exc:
        self._voice_set_status(f"Vector memory store skipped: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp._voice_memory_store_latest_turn = _voice_memory_store_latest_turn


def _voice_speak_reply_if_needed(self: "DesktopMedSafeApp", reply: str) -> None:
    if not bool(getattr(self, "voice_tts_enabled", True)):
        return
    clean = markdown_to_plain_text(reply, max_chars=1800)
    if not clean:
        return
    signature = _voice_patch_sha(clean)
    if signature == getattr(self, "voice_last_spoken_signature", ""):
        return
    self.voice_last_spoken_signature = signature
    self.bark_voice.speak_async(clean)


DesktopMedSafeApp._voice_speak_reply_if_needed = _voice_speak_reply_if_needed


VOICE_PATCH_ORIGINAL_ON_ASSISTANT_SEND = DesktopMedSafeApp.on_assistant_send


def _voice_patch_on_assistant_send(self: "DesktopMedSafeApp") -> None:
    try:
        self.voice_last_user_prompt = sanitize_text(self.ids.assistant_input.text, max_chars=1600)
    except Exception:
        self.voice_last_user_prompt = ""
    return VOICE_PATCH_ORIGINAL_ON_ASSISTANT_SEND(self)


DesktopMedSafeApp.on_assistant_send = _voice_patch_on_assistant_send


VOICE_PATCH_ORIGINAL_FINISH_LIVE_STREAM = DesktopMedSafeApp._finish_assistant_live_stream


def _voice_patch_finish_live_stream(self: "DesktopMedSafeApp", request_id: int, final_text: str, mode: str) -> None:
    VOICE_PATCH_ORIGINAL_FINISH_LIVE_STREAM(self, request_id, final_text, mode)
    if request_id == getattr(self, "assistant_request_id", 0):
        self._voice_memory_store_latest_turn()
        self._voice_speak_reply_if_needed(final_text)


DesktopMedSafeApp._finish_assistant_live_stream = _voice_patch_finish_live_stream


VOICE_PATCH_ORIGINAL_ASSISTANT_STREAM_TICK = DesktopMedSafeApp._assistant_stream_tick


def _voice_patch_assistant_stream_tick(self: "DesktopMedSafeApp", request_id: int) -> None:
    VOICE_PATCH_ORIGINAL_ASSISTANT_STREAM_TICK(self, request_id)
    if request_id != getattr(self, "assistant_request_id", 0):
        return
    if bool(getattr(self, "assistant_reply_streaming", False)):
        return
    try:
        history = list(self.data_cache.get("assistant_history") or [])
        if history and sanitize_text(history[-1].get("role") or "", max_chars=24).lower() == "assistant":
            reply = sanitize_text(history[-1].get("content") or "", max_chars=10000)
            self._voice_memory_store_latest_turn()
            self._voice_speak_reply_if_needed(reply)
    except Exception:
        pass


DesktopMedSafeApp._assistant_stream_tick = _voice_patch_assistant_stream_tick


VOICE_PATCH_ORIGINAL_REFRESH_CONTEXT_PANEL = DesktopMedSafeApp.refresh_assistant_context_panel


def _voice_patch_refresh_assistant_context_panel(self: "DesktopMedSafeApp") -> None:
    VOICE_PATCH_ORIGINAL_REFRESH_CONTEXT_PANEL(self)
    try:
        box = self.ids.get("assistant_context_window_report")
        if box is None:
            return
        current = sanitize_text(getattr(box, "text", ""), max_chars=6000)
        prompt = sanitize_text(self.ids.assistant_input.text if "assistant_input" in self.ids else "", max_chars=1200)
        db = InHouseWeaviateLikeVectorDB(self.data_cache)
        count = len(db.docs)
        block = db.context_block(prompt or "current support context", mode=getattr(self, "assistant_mode", "General"))
        extra = f"\n\nVector memory docs: {count}\n{sanitize_text(block, max_chars=1600)}"
        box.text = sanitize_text(current + extra, max_chars=5000)
    except Exception:
        pass


DesktopMedSafeApp.refresh_assistant_context_panel = _voice_patch_refresh_assistant_context_panel


VOICE_PATCH_ORIGINAL_ON_CLOSE = DesktopMedSafeApp.on_close


def _voice_patch_on_close(self: "DesktopMedSafeApp") -> None:
    try:
        if getattr(self, "voice_listener_stop", None) is not None:
            self.voice_listener_stop.set()
        if getattr(self, "voice_interruptor", None) is not None:
            self.voice_interruptor.stop()
        if getattr(self, "bark_voice", None) is not None:
            self.bark_voice.stop()
    except Exception:
        pass
    VOICE_PATCH_ORIGINAL_ON_CLOSE(self)


DesktopMedSafeApp.on_close = _voice_patch_on_close
# --- end voice + in-house vector-memory patch ----------------------------------


# --- advanced voice/memory/therapy patch layer ---------------------------------
# This second layer upgrades the first single-file patch without changing the
# external app layout. It adds a local memory graph, diversity-aware retrieval,
# crisis-aware prompt routing, wake/sleep listener controls, queued Bark chunks,
# and post-interruption auto-capture.

ADVANCED_VOICE_MEMORY_PATCH_VERSION = "2026-05-08.advanced.memory-graph.voice-session"
ADVANCED_MEMORY_MAX_FACTS = 420
ADVANCED_MEMORY_MAX_EVENTS = 220
ADVANCED_MEMORY_RECALL_TOP_K = 9
ADVANCED_MEMORY_CONTEXT_CHARS = 6400
ADVANCED_WAKE_WORDS = ("medsafe", "med safe", "hey medsafe", "hey med safe", "assistant")
ADVANCED_STOP_COMMANDS = ("stop", "stop talking", "cancel", "pause", "quiet", "be quiet", "interrupt")


def _advanced_normalize_for_match(value: Any) -> str:
    return re.sub(r"\s+", " ", sanitize_text(value, max_chars=4000).lower()).strip()


def _advanced_sentence_chunks(value: Any, *, max_chars: int = 360) -> List[str]:
    text = markdown_to_plain_text(value, max_chars=3600)
    ...
    for sentence in sentences:
        sentence = sanitize_text(sentence, max_chars=max_chars * 2).strip()
        if not sentence:
            continue
...
def _advanced_extract_entities(value: Any) -> Dict[str, List[str]]:
    text = sanitize_text(value, max_chars=8000)
    lowered = text.lower()
    meds = sorted(set(re.findall(r"\b([a-z][a-z0-9_-]{2,32})\s+(?:\d+(?:\.\d+)?\s*)?(?:mg|mcg|g|ml)\b", lowered)))
    doses = sorted(set(re.findall(r"\b\d+(?:\.\d+)?\s*(?:mg|mcg|g|ml|hours?|hrs?|minutes?|mins?)\b", lowered)))
    moods = sorted(set(word for word in ("anxious", "panic", "sad", "angry", "numb", "overwhelmed", "calm", "hopeful", "craving", "triggered", "ashamed", "lonely") if word in lowered))
    people = sorted(set(re.findall(r"\b(?:my|with|from)\s+(therapist|doctor|dentist|sponsor|partner|mom|dad|friend|roommate)\b", lowered)))
    goals = sorted(set(re.findall(r"\b(?:goal|want|trying|need)\s+(?:to\s+)?([^.!?]{3,90})", lowered)))[:6]
    return {"meds": meds[:12], "doses": doses[:12], "moods": moods[:12], "people": people[:12], "goals": [sanitize_text(g, max_chars=100) for g in goals]}


def _advanced_importance_score(user_text: str, assistant_text: str, risk: Optional[Dict[str, Any]] = None) -> float:
    joined = _advanced_normalize_for_match(f"{user_text}\n{assistant_text}")
    score = 0.25
    if len(joined) > 240:
        score += 0.08
    if any(token in joined for token in ("i always", "i never", "remember", "important", "prefer", "don't", "do not", "trigger", "craving", "panic", "dose", "allergy", "medication")):
        score += 0.25
    if risk and risk.get("level") in {"critical", "high"}:
        score += 0.35
    if _advanced_extract_entities(joined).get("moods"):
        score += 0.10
    return max(0.0, min(1.0, score))


def advanced_therapist_risk_route(prompt: Any, mode: str = "General") -> Dict[str, Any]:
    text = _advanced_normalize_for_match(prompt)
    rules: List[Tuple[str, str, Tuple[str, ...]]] = [
        ("critical", "possible immediate self-harm or suicide risk", ("kill myself", "suicide", "end my life", "self harm", "hurt myself", "i want to die", "can't stay safe")),
        ("critical", "possible overdose, poisoning, or dangerous withdrawal risk", ("overdose", "od", "poison", "took too much", "can't breathe", "chest pain", "seizure", "withdrawal seizure", "delirium tremens")),
        ("high", "possible violence or immediate danger", ("hurt someone", "weapon", "unsafe at home", "being abused", "being stalked", "threatened me")),
        ("high", "high relapse or acute craving risk", ("about to relapse", "going to use", "craving is unbearable", "can't stop myself", "dealer", "buy drugs")),
        ("medium", "distress signal", ("panic", "spiral", "overwhelmed", "can't cope", "dissociate", "flashback", "triggered")),
    ]
    matched: List[str] = []
    level_rank = {"low": 0, "medium": 1, "high": 2, "critical": 3}
    level = "low"
    for candidate_level, reason, needles in rules:
    ...
    return {
        ...
        "reasons": matched[:5],
        "mode": normalize_assistant_mode(mode),
        "needs_human_support": level in {"critical", "high"},
        "prompt_digest": _voice_patch_sha(text)[:16],
    }


def _advanced_fact_candidates(user_text: str, assistant_text: str, mode: str) -> List[Dict[str, Any]]:
    clean_user = sanitize_text(user_text, max_chars=2400)
    clean_assistant = markdown_to_plain_text(assistant_text, max_chars=1600)
    joined = f"{clean_user}\n{clean_assistant}"
    entities = _advanced_extract_entities(joined)
    risk = advanced_therapist_risk_route(clean_user, mode)
    candidates: List[Dict[str, Any]] = []
    lowered = clean_user.lower()
    patterns = [
        ("preference", r"\b(?:i prefer|i like|i don't like|i do not like|please|don't|do not)\s+([^.!?]{4,180})"),
        ("stable_user_fact", r"\b(?:i am|i'm|i have|i live|i work|my)\s+([^.!?]{4,180})"),
        ("trigger", r"\b(?:trigger|triggers|craving|panic|spiral)\b[^.!?]{0,180}"),
        ("goal", r"\b(?:i want to|i need to|my goal is|trying to)\s+([^.!?]{4,180})"),
    ]
    for kind, pattern in patterns:
        for match in re.finditer(pattern, lowered, flags=re.I):
            value = sanitize_text(match.group(0), max_chars=220)
            if len(value.split()) >= 3:
                candidates.append({"kind": kind, "text": value, "source": "user", "confidence": 0.68})
    for med in entities.get("meds", []):
        candidates.append({"kind": "medication_reference", "text": f"Mentioned medication or dose context: {med}", "source": "entity", "confidence": 0.58})
    for mood in entities.get("moods", []):
        candidates.append({"kind": "affective_signal", "text": f"Recent emotional/recovery signal: {mood}", "source": "entity", "confidence": 0.54})
    if risk.get("level") in {"high", "critical"}:
        candidates.append({"kind": "safety_signal", "text": f"Safety routing previously flagged {risk.get('level')}: {', '.join(risk.get('reasons') or [])}", "source": "risk_route", "confidence": 0.84})
    # Keep facts compact and deduplicated.
    seen = set()
    compact: List[Dict[str, Any]] = []
    for item in candidates:
        text = sanitize_text(item.get("text") or "", max_chars=240)
        key = _voice_patch_sha(f"{item.get('kind')}:{text.lower()}")
        if not text or key in seen:
            continue
        seen.add(key)
        compact.append(dict(item, id=key, text=text, mode=normalize_assistant_mode(mode), tags=_voice_patch_tags(text), vector=_voice_patch_vector(text), updated_ts=time.time()))
    return compact[:18]


class AdvancedTherapeuticMemoryGraph:
    """Local memory graph layered on top of the in-house vector store.

    It keeps three types of memory in the encrypted vault data: episodic events,
    extracted stable facts/preferences, and a tiny session profile. Retrieval uses
    vector score + tag overlap + importance + recency with MMR-style diversity.
    """

    def __init__(self, data: Dict[str, Any]):
        self.data = data
        raw = data.get("assistant_advanced_memory")
        if not isinstance(raw, dict):
            raw = {}
        raw.setdefault("schema", "AdvancedTherapeuticMemoryGraph/v3")
        raw.setdefault("facts", [])
        raw.setdefault("events", [])
        raw.setdefault("profile", {})
        raw.setdefault("updated_ts", time.time())
        ...
        if not isinstance(raw.get("profile"), dict):
            raw["profile"] = {}
        raw["facts"] = raw["facts"][-ADVANCED_MEMORY_MAX_FACTS:]
        raw["events"] = raw["events"][-ADVANCED_MEMORY_MAX_EVENTS:]
        data["assistant_advanced_memory"] = raw
        self.memory = raw

    @property
    def facts(self) -> List[Dict[str, Any]]:
        return self.memory.get("facts") if isinstance(self.memory.get("facts"), list) else []

    @property
    def events(self) -> List[Dict[str, Any]]:
        return self.memory.get("events") if isinstance(self.memory.get("events"), list) else []

    def store_turn(self, user_text: str, assistant_text: str, *, mode: str = "General") -> bool:
        clean_user = sanitize_text(user_text, max_chars=2400)
        clean_assistant = sanitize_text(assistant_text, max_chars=6000)
        if not clean_user or not clean_assistant:
            return False
        active_mode = normalize_assistant_mode(mode)
        risk = advanced_therapist_risk_route(clean_user, active_mode)
        entities = _advanced_extract_entities(f"{clean_user}\n{clean_assistant}")
        event_id = _voice_patch_sha(f"advanced-event\n{active_mode}\n{clean_user}\n{clean_assistant}")
        if not any(str(item.get("id")) == event_id for item in self.events[-60:]):
            event = {
                "id": event_id,
                "kind": "advanced_episode",
                "mode": active_mode,
                "user": clean_user,
                "assistant_summary": sanitize_text(markdown_to_plain_text(clean_assistant, max_chars=1100), max_chars=1100),
                "entities": entities,
                "risk": risk,
                "tags": sorted(set(_voice_patch_tags(clean_user) + _voice_patch_tags(clean_assistant))),
                "importance": _advanced_importance_score(clean_user, clean_assistant, risk),
                "sentiment": _voice_patch_sentiment_hint(clean_user),
                "timestamp": time.time(),
                "vector": _voice_patch_vector(f"{active_mode}\n{clean_user}\n{clean_assistant}"),
            }
            self.memory["events"] = (self.events + [event])[-ADVANCED_MEMORY_MAX_EVENTS:]
        self._merge_facts(_advanced_fact_candidates(clean_user, clean_assistant, active_mode))
        self._update_profile(clean_user, clean_assistant, active_mode, risk, entities)
        self.memory["updated_ts"] = time.time()
        return True

    def _merge_facts(self, candidates: List[Dict[str, Any]]) -> None:
        facts_by_id = {str(item.get("id")): dict(item) for item in self.facts if item.get("id")}
        for candidate in candidates:
            cid = str(candidate.get("id") or "")
            if not cid:
                continue
            prior = facts_by_id.get(cid, {})
            candidate["created_ts"] = safe_float(prior.get("created_ts")) or time.time()
            candidate["updated_ts"] = time.time()
            candidate["hits"] = int(prior.get("hits") or 0) + 1
            candidate["confidence"] = max(safe_float(prior.get("confidence")), safe_float(candidate.get("confidence")))
            facts_by_id[cid] = candidate
        facts = list(facts_by_id.values())
        facts.sort(key=lambda item: (safe_float(item.get("confidence")), int(item.get("hits") or 0), safe_float(item.get("updated_ts"))), reverse=True)
        self.memory["facts"] = facts[:ADVANCED_MEMORY_MAX_FACTS]

    def _update_profile(self, user_text: str, assistant_text: str, mode: str, risk: Dict[str, Any], entities: Dict[str, List[str]]) -> None:
        profile = self.memory.get("profile") if isinstance(self.memory.get("profile"), dict) else {}
        profile["last_mode"] = normalize_assistant_mode(mode)
        profile["last_updated_ts"] = time.time()
        profile["turn_count"] = int(profile.get("turn_count") or 0) + 1
        profile["last_risk_level"] = risk.get("level", "low")
        profile["last_entities"] = entities
        prior_valence = safe_float(profile.get("affective_valence"))
        profile["affective_valence"] = round((prior_valence * 0.78) + (_voice_patch_sentiment_hint(user_text) * 0.22), 4)
        counts = profile.get("tag_counts") if isinstance(profile.get("tag_counts"), dict) else {}
        for tag in _voice_patch_tags(f"{user_text}\n{assistant_text}"):
            counts[tag] = int(counts.get(tag) or 0) + 1
        profile["tag_counts"] = dict(sorted(counts.items(), key=lambda item: item[1], reverse=True)[:18])
        self.memory["profile"] = profile

    def _score_item(self, item: Dict[str, Any], q_vec: List[float], q_tags: set, mode: str) -> float:
        vec = item.get("vector") if isinstance(item.get("vector"), list) else []
        score = _voice_patch_cosine(q_vec, vec)
        tags = set(item.get("tags") or [])
        score += 0.10 * len(q_tags & tags)
        if normalize_assistant_mode(item.get("mode")) == normalize_assistant_mode(mode):
            score += 0.05
        score += 0.16 * safe_float(item.get("importance"))
        score += 0.06 * safe_float(item.get("confidence"))
        age_days = max(0.0, (time.time() - safe_float(item.get("updated_ts") or item.get("timestamp"))) / 86400.0)
        score += max(0.0, 0.06 * (1.0 - min(age_days, 21.0) / 21.0))
        return score

    def recall(self, query: str, *, mode: str = "General", top_k: int = ADVANCED_MEMORY_RECALL_TOP_K) -> List[Dict[str, Any]]:
        clean_query = sanitize_text(query, max_chars=2400)
        if not clean_query:
            return []
        q_vec = _voice_patch_vector(f"{normalize_assistant_mode(mode)}\n{clean_query}")
        q_tags = set(_voice_patch_tags(clean_query))
        pool: List[Tuple[float, Dict[str, Any]]] = []
        for item in self.facts + self.events:
            score = self._score_item(item, q_vec, q_tags, mode)
            if score > 0.08:
                pool.append((score, item))
        pool.sort(key=lambda pair: pair[0], reverse=True)
        selected: List[Tuple[float, Dict[str, Any]]] = []
        selected_vectors: List[List[float]] = []
        for base_score, item in pool[:40]:
            vec = item.get("vector") if isinstance(item.get("vector"), list) else []
            redundancy = max((_voice_patch_cosine(vec, chosen) for chosen in selected_vectors), default=0.0)
            mmr = (0.78 * base_score) - (0.22 * redundancy)
            selected.append((mmr, dict(item, _score=round(base_score, 4), _mmr=round(mmr, 4))))
            if vec:
                selected_vectors.append(vec)
            selected.sort(key=lambda pair: pair[0], reverse=True)
            selected = selected[:top_k]
            selected_vectors = [entry.get("vector") for _, entry in selected if isinstance(entry.get("vector"), list)]
        return [item for _score, item in selected[:top_k]]

    def context_block(self, query: str, *, mode: str = "General") -> str:
        active_mode = normalize_assistant_mode(mode)
        risk = advanced_therapist_risk_route(query, active_mode)
        profile = self.memory.get("profile") if isinstance(self.memory.get("profile"), dict) else {}
        lines = [
            "[advanced_therapeutic_memory_graph]",
            f"schema={self.memory.get('schema')} mode={active_mode} recall_policy=vector+tags+importance+recency+diversity",
            f"risk_route={risk.get('level')} reasons={'; '.join(risk.get('reasons') or ['none'])}",
            f"profile_turns={int(profile.get('turn_count') or 0)} affective_valence={safe_float(profile.get('affective_valence')):.2f} last_risk={profile.get('last_risk_level', 'unknown')}",
        ]
        if risk.get("level") in {"critical", "high"}:
            lines.append("SAFETY_PRIORITY: respond with immediate stabilization, urge real-time human/emergency support for imminent danger, and avoid long analysis before safety steps.")
        recall = self.recall(query, mode=active_mode)
        if recall:
            lines.append("Relevant advanced memories:")
            total = 0
            for idx, item in enumerate(recall, 1):
                kind = sanitize_text(item.get("kind") or "memory", max_chars=40)
                tags = ",".join([sanitize_text(tag, max_chars=24) for tag in list(item.get("tags") or [])[:6]])
                when = format_timestamp(safe_float(item.get("updated_ts") or item.get("timestamp")))
                if kind == "advanced_episode":
                    body = f"User: {sanitize_text(item.get('user') or '', max_chars=420)} | Assistant memory: {sanitize_text(item.get('assistant_summary') or '', max_chars=520)}"
                else:
                    body = sanitize_text(item.get("text") or "", max_chars=520)
                line = f"{idx}. kind={kind} score={item.get('_score')} mmr={item.get('_mmr')} tags={tags} when={when}\n{body}"
                total += len(line)
                if total > ADVANCED_MEMORY_CONTEXT_CHARS:
                    lines.append("[advanced memory trimmed]")
                    break
                lines.append(line)
        else:
            lines.append("Relevant advanced memories: none yet.")
        lines.append("[/advanced_therapeutic_memory_graph]")
        return "\n".join(lines)

    def dashboard_text(self, query: str = "") -> str:
        profile = self.memory.get("profile") if isinstance(self.memory.get("profile"), dict) else {}
        top_tags = profile.get("tag_counts") if isinstance(profile.get("tag_counts"), dict) else {}
        return "\n".join(
            [
                f"Advanced memory: {len(self.facts)} facts, {len(self.events)} episodes",
                f"Mode profile: {profile.get('last_mode', 'unknown')} | turns {int(profile.get('turn_count') or 0)} | last risk {profile.get('last_risk_level', 'unknown')}",
                f"Top tags: {', '.join(list(top_tags.keys())[:8]) or 'none yet'}",
                f"Recall preview: {len(self.recall(query or 'current support context', mode=profile.get('last_mode', 'General'), top_k=5))} likely relevant memories",
            ]
        )


ADVANCED_ORIGINAL_BUILD_MEMORY_CONTEXT = build_in_house_vector_memory_context


def build_in_house_vector_memory_context(data: Dict[str, Any], prompt: str, mode: str) -> str:
    base = ADVANCED_ORIGINAL_BUILD_MEMORY_CONTEXT(data, prompt, mode)
    try:
        graph = AdvancedTherapeuticMemoryGraph(data)
        advanced = graph.context_block(prompt, mode=mode)
    except Exception as exc:
        advanced = f"[advanced_therapeutic_memory_graph]\nAdvanced memory unavailable: {sanitize_text(exc, max_chars=220)}\n[/advanced_therapeutic_memory_graph]"
    return sanitize_text(base + "\n\n" + advanced, max_chars=11000)


ADVANCED_ORIGINAL_VOICE_STORE_LATEST_TURN = DesktopMedSafeApp._voice_memory_store_latest_turn


def _advanced_voice_memory_store_latest_turn(self: "DesktopMedSafeApp") -> None:
    ADVANCED_ORIGINAL_VOICE_STORE_LATEST_TURN(self)
    try:
        history = list(self.data_cache.get("assistant_history") or [])
        last_user = ""
        last_assistant = ""
        ...
        if not last_user or not last_assistant:
            return
        sig = _voice_patch_sha(f"advanced\n{last_mode}\n{last_user}\n{last_assistant}")
        if sig == getattr(self, "advanced_voice_last_stored_signature", ""):
            return
        graph = AdvancedTherapeuticMemoryGraph(self.data_cache)
        if graph.store_turn(last_user, last_assistant, mode=last_mode):
            self.advanced_voice_last_stored_signature = sig
            self.persist_data_async()
    except Exception as exc:
        self._voice_set_status(f"Advanced memory store skipped: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp._voice_memory_store_latest_turn = _advanced_voice_memory_store_latest_turn


ADVANCED_ORIGINAL_DESKTOP_INIT = DesktopMedSafeApp.__init__


def _advanced_voice_desktop_init(self: "DesktopMedSafeApp") -> None:
    ADVANCED_ORIGINAL_DESKTOP_INIT(self)
    self.advanced_voice_last_stored_signature = ""
    self.advanced_voice_interrupt_count = 0
    self.advanced_voice_listener_awake = True
    self.advanced_voice_wake_word_enabled = True
    self.advanced_voice_auto_capture_after_interrupt = True
    self.advanced_voice_last_command_ts = 0.0
    self.advanced_voice_session_notes: List[str] = []
    try:
        AdvancedTherapeuticMemoryGraph(self.data_cache)
    except Exception:
        pass


DesktopMedSafeApp.__init__ = _advanced_voice_desktop_init


def _advanced_voice_command_result(self: "DesktopMedSafeApp", text: str) -> bool:
    clean = _advanced_normalize_for_match(text)
    now = time.time()
    if not clean:
        return True
    if now - safe_float(getattr(self, "advanced_voice_last_command_ts", 0.0)) < 0.45:
        return True
    if clean in ADVANCED_STOP_COMMANDS or any(clean.startswith(cmd + " ") for cmd in ADVANCED_STOP_COMMANDS):
        self.advanced_voice_last_command_ts = now
        self.stop_bark_voice()
        self._voice_set_status("Voice command handled: stopped Bark output.")
        return True
    if "listener sleep" in clean or clean in {"go to sleep", "sleep", "stop listening"}:
        self.advanced_voice_listener_awake = False
        self._voice_set_status("Listener sleeping. Say 'MedSafe' or 'hey MedSafe' to wake it.")
        return True
    if any(wake in clean for wake in ADVANCED_WAKE_WORDS) and ("wake" in clean or not bool(getattr(self, "advanced_voice_listener_awake", True))):
        self.advanced_voice_listener_awake = True
        self._voice_set_status("Listener awake.")
        remainder = clean
        for wake in ADVANCED_WAKE_WORDS:
            remainder = remainder.replace(wake, " ")
        remainder = remainder.replace("wake up", " ").strip()
        if remainder and len(remainder.split()) >= 3:
            self.run_on_ui_thread(self._voice_submit_text, remainder)
        return True
    if "therapy mode" in clean or clean.startswith("therapy "):
        self.on_assistant_mode_change("Therapy")
        remainder = re.sub(r"\b(?:switch to )?therapy mode\b", "", clean).strip()
        if remainder and len(remainder.split()) >= 2:
            self.run_on_ui_thread(self._voice_submit_text, remainder)
        else:
            self._voice_set_status("Switched to Therapy mode.")
        return True
    if "recovery mode" in clean or clean.startswith("recovery "):
        self.on_assistant_mode_change("Recovery Coach")
        remainder = re.sub(r"\b(?:switch to )?recovery mode\b", "", clean).strip()
        if remainder and len(remainder.split()) >= 2:
            self.run_on_ui_thread(self._voice_submit_text, remainder)
        else:
            self._voice_set_status("Switched to Recovery Coach mode.")
        return True
    if "general mode" in clean:
        self.on_assistant_mode_change("General")
        self._voice_set_status("Switched to General mode.")
        return True
    if "memory report" in clean or "what do you remember" in clean:
        try:
            graph = AdvancedTherapeuticMemoryGraph(self.data_cache)
            self._voice_set_status(graph.dashboard_text(clean))
        except Exception as exc:
            self._voice_set_status(f"Memory report unavailable: {sanitize_text(exc, max_chars=180)}")
        return True
    return False


DesktopMedSafeApp._advanced_voice_command_result = _advanced_voice_command_result


ADVANCED_ORIGINAL_VOICE_SUBMIT_TEXT = DesktopMedSafeApp._voice_submit_text


def _advanced_voice_submit_text(self: "DesktopMedSafeApp", text: str) -> None:
    clean = sanitize_text(text, max_chars=1600)
    if not clean:
        return
    if self._advanced_voice_command_result(clean):
        return
    risk = advanced_therapist_risk_route(clean, getattr(self, "assistant_mode", "General"))
    if risk.get("level") in {"critical", "high"}:
        prefix = "Safety priority detected. "
        self._voice_set_status(prefix + "; ".join(risk.get("reasons") or []))
    ADVANCED_ORIGINAL_VOICE_SUBMIT_TEXT(self, clean)


DesktopMedSafeApp._voice_submit_text = _advanced_voice_submit_text


ADVANCED_ORIGINAL_VOICE_LISTENER_LOOP = DesktopMedSafeApp._voice_listener_loop


def _advanced_voice_listener_loop(self: "DesktopMedSafeApp") -> None:
    misses = 0
    while not self.voice_listener_stop.is_set():
        try:
            text = self.local_stt.recognize_once(phrase_time_limit=10.0)
            clean = sanitize_text(text, max_chars=1600)
            if not clean:
                misses += 1
                continue
            lowered = _advanced_normalize_for_match(clean)
            if bool(getattr(self, "advanced_voice_wake_word_enabled", True)) and not bool(getattr(self, "advanced_voice_listener_awake", True)):
                if not any(wake in lowered for wake in ADVANCED_WAKE_WORDS):
                    self.run_on_ui_thread(self._voice_set_status, "Listener asleep. Wake word not heard.")
                    continue
            # In always-listening mode, wake words are stripped when present, but not required while awake.
            for wake in ADVANCED_WAKE_WORDS:
                if lowered.startswith(wake):
                    clean = sanitize_text(clean[len(wake):], max_chars=1600).strip() or clean
                    break
            self.run_on_ui_thread(self._voice_submit_text, clean)
            misses = 0
        except Exception as exc:
            misses += 1
            delay = min(4.0, 0.65 + (misses * 0.25))
            self.run_on_ui_thread(self._voice_set_status, f"Listener waiting: {sanitize_text(exc, max_chars=180)}")
            time.sleep(delay)


DesktopMedSafeApp._voice_listener_loop = _advanced_voice_listener_loop


ADVANCED_ORIGINAL_BARK_STOP = BarkCppVoiceEngine.stop
ADVANCED_ORIGINAL_BARK_SPEAK_WORKER = BarkCppVoiceEngine._speak_worker


def _advanced_bark_stop(self: "BarkCppVoiceEngine") -> None:
    self.generation_id = int(getattr(self, "generation_id", 0)) + 1
    ADVANCED_ORIGINAL_BARK_STOP(self)


def _advanced_bark_speak_async(self: "BarkCppVoiceEngine", text: str) -> None:
    chunks = _advanced_sentence_chunks(text, max_chars=int(os.environ.get("BARK_CHUNK_CHARS", "360") or "360"))
    if not chunks:
        return
    with self.lock:
        self.generation_id = int(getattr(self, "generation_id", 0)) + 1
    ...
    def worker() -> None:
        self.app.run_on_ui_thread(self.app._voice_set_status, f"Bark queued {len(chunks)} spoken chunk(s).")
        for index, chunk in enumerate(chunks, 1):
            if self.stop_event.is_set() or int(getattr(self, "generation_id", 0)) != generation_id:
                break
            self.app.run_on_ui_thread(self.app._voice_set_status, f"Bark chunk {index}/{len(chunks)}...")
            ADVANCED_ORIGINAL_BARK_SPEAK_WORKER(self, chunk)
            if self.stop_event.is_set() or int(getattr(self, "generation_id", 0)) != generation_id:
                break
        self.app.run_on_ui_thread(self.app._voice_set_status, "Bark queue finished." if int(getattr(self, "generation_id", 0)) == generation_id else "Bark queue cancelled.")
    threading.Thread(target=worker, daemon=True).start()


BarkCppVoiceEngine.stop = _advanced_bark_stop
BarkCppVoiceEngine.speak_async = _advanced_bark_speak_async


ADVANCED_ORIGINAL_VOICE_INTERRUPT_DETECTED = DesktopMedSafeApp.on_voice_interrupt_detected


def _advanced_voice_interrupt_detected(self: "DesktopMedSafeApp", rms: float = 0.0) -> None:
    self.advanced_voice_interrupt_count = int(getattr(self, "advanced_voice_interrupt_count", 0)) + 1
    self.advanced_voice_session_notes.append(f"barge_in:{int(rms)}@{int(time.time())}")
    self.advanced_voice_session_notes = self.advanced_voice_session_notes[-30:]
    ADVANCED_ORIGINAL_VOICE_INTERRUPT_DETECTED(self, rms)
    if not bool(getattr(self, "advanced_voice_auto_capture_after_interrupt", True)):
        return
    if bool(getattr(self, "assistant_request_pending", False)) or bool(getattr(self, "assistant_reply_streaming", False)):
        return
    def capture() -> None:
        try:
            time.sleep(0.35)
            text = self.local_stt.recognize_once(phrase_time_limit=7.0)
            if text:
                self.run_on_ui_thread(self._voice_submit_text, text)
        except Exception as exc:
            self.run_on_ui_thread(self._voice_set_status, f"Interrupt auto-capture skipped: {sanitize_text(exc, max_chars=160)}")
    threading.Thread(target=capture, daemon=True).start()


DesktopMedSafeApp.on_voice_interrupt_detected = _advanced_voice_interrupt_detected


ADVANCED_ORIGINAL_REFRESH_CONTEXT_PANEL = DesktopMedSafeApp.refresh_assistant_context_panel


def _advanced_refresh_assistant_context_panel(self: "DesktopMedSafeApp") -> None:
    ADVANCED_ORIGINAL_REFRESH_CONTEXT_PANEL(self)
    try:
        box = self.ids.get("assistant_context_window_report")
        if box is None:
            return
        current = sanitize_text(getattr(box, "text", ""), max_chars=5200)
        prompt = sanitize_text(self.ids.assistant_input.text if "assistant_input" in self.ids else "", max_chars=1200)
        graph = AdvancedTherapeuticMemoryGraph(self.data_cache)
        voice_line = (
            f"\n\nAdvanced voice session: awake={bool(getattr(self, 'advanced_voice_listener_awake', True))} "
            f"wake_word={bool(getattr(self, 'advanced_voice_wake_word_enabled', True))} "
            f"interruptions={int(getattr(self, 'advanced_voice_interrupt_count', 0))} "
            f"auto_capture={bool(getattr(self, 'advanced_voice_auto_capture_after_interrupt', True))}"
        )
        box.text = sanitize_text(current + "\n\n" + graph.dashboard_text(prompt) + voice_line, max_chars=6200)
    except Exception:
        pass


DesktopMedSafeApp.refresh_assistant_context_panel = _advanced_refresh_assistant_context_panel


ADVANCED_ORIGINAL_BUILD_ASSISTANT_TAB = DesktopMedSafeApp._build_assistant_tab


def _advanced_build_assistant_tab(self: "DesktopMedSafeApp", tab: Any) -> None:
    ADVANCED_ORIGINAL_BUILD_ASSISTANT_TAB(self, tab)
    if ctk is None:
        return
    voice_frame = self.ids.get("voice_frame")
    if voice_frame is None:
        return
    try:
        voice_frame.grid_columnconfigure((0, 1, 2, 3), weight=1)
        wake = self._button(voice_frame, text="Wake Word On", command=self.on_voice_toggle_wake_word, tone="neutral")
        wake.grid(row=3, column=0, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_wake_button"] = wake
        auto = self._button(voice_frame, text="Auto Capture On", command=self.on_voice_toggle_auto_capture, tone="neutral")
        auto.grid(row=3, column=1, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_auto_capture_button"] = auto
        report = self._button(voice_frame, text="Memory Report", command=self.on_advanced_memory_report, tone="neutral")
        report.grid(row=3, column=2, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_memory_report_button"] = report
        clear = self._button(voice_frame, text="Trim Memory", command=self.on_advanced_memory_trim, tone="neutral")
        clear.grid(row=3, column=3, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_memory_trim_button"] = clear
    except Exception:
        pass


DesktopMedSafeApp._build_assistant_tab = _advanced_build_assistant_tab


def on_voice_toggle_wake_word(self: "DesktopMedSafeApp") -> None:
    self.advanced_voice_wake_word_enabled = not bool(getattr(self, "advanced_voice_wake_word_enabled", True))
    button = self.ids.get("voice_wake_button")
    try:
        if button is not None:
            button.configure(text="Wake Word On" if self.advanced_voice_wake_word_enabled else "Wake Word Off")
    except Exception:
        pass
    self._voice_set_status("Wake-word gating enabled." if self.advanced_voice_wake_word_enabled else "Wake-word gating disabled; listener accepts any captured phrase while on.")


DesktopMedSafeApp.on_voice_toggle_wake_word = on_voice_toggle_wake_word


def on_voice_toggle_auto_capture(self: "DesktopMedSafeApp") -> None:
    self.advanced_voice_auto_capture_after_interrupt = not bool(getattr(self, "advanced_voice_auto_capture_after_interrupt", True))
    button = self.ids.get("voice_auto_capture_button")
    try:
        if button is not None:
            button.configure(text="Auto Capture On" if self.advanced_voice_auto_capture_after_interrupt else "Auto Capture Off")
    except Exception:
        pass
    self._voice_set_status("Interrupt auto-capture enabled." if self.advanced_voice_auto_capture_after_interrupt else "Interrupt auto-capture disabled.")


DesktopMedSafeApp.on_voice_toggle_auto_capture = on_voice_toggle_auto_capture


def on_advanced_memory_report(self: "DesktopMedSafeApp") -> None:
    try:
        prompt = sanitize_text(self.ids.assistant_input.text if "assistant_input" in self.ids else "current context", max_chars=1200)
        graph = AdvancedTherapeuticMemoryGraph(self.data_cache)
        self._voice_set_status(graph.dashboard_text(prompt))
        self.refresh_assistant_context_panel()
    except Exception as exc:
        self._voice_set_status(f"Advanced memory unavailable: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_advanced_memory_report = on_advanced_memory_report


def on_advanced_memory_trim(self: "DesktopMedSafeApp") -> None:
    try:
        graph = AdvancedTherapeuticMemoryGraph(self.data_cache)
        graph.memory["facts"] = graph.facts[:ADVANCED_MEMORY_MAX_FACTS // 2]
        graph.memory["events"] = graph.events[-ADVANCED_MEMORY_MAX_EVENTS // 2 :]
        graph.memory["updated_ts"] = time.time()
        self.persist_data_async()
        self.refresh_assistant_context_panel()
        self._voice_set_status("Advanced memory trimmed to the most useful recent half.")
    except Exception as exc:
        self._voice_set_status(f"Could not trim memory: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_advanced_memory_trim = on_advanced_memory_trim
# --- end advanced voice/memory/therapy patch layer -----------------------------

# --- ultra voice/memory/therapy orchestration layer ----------------------------
# v4 layer: adds therapist-grade orchestration packets, richer local memory
# consolidation, adaptive barge-in controls, voice diagnostics, and session state.
# It remains single-file and stores all durable data in the existing encrypted vault.

ULTRA_PATCH_VERSION = "2026-05-08.ultra.voice-memory-therapist-orchestrator.v4"
ULTRA_SESSION_MAX_NOTES = 80
ULTRA_PLAN_MAX_CHARS = 9000
ULTRA_MEMORY_MAINTENANCE_EVERY_TURNS = 6


def _ultra_norm(value: Any, *, max_chars: int = 6000) -> str:
    return sanitize_text(value, max_chars=max_chars).lower()


def _ultra_contains_any(text: str, needles: Tuple[str, ...]) -> bool:
    lowered = _ultra_norm(text, max_chars=8000)
    return any(needle in lowered for needle in needles)


def _ultra_count_matches(text: str, needles: Tuple[str, ...]) -> int:
    lowered = _ultra_norm(text, max_chars=8000)
    return sum(1 for needle in needles if needle in lowered)


def _ultra_safe_list(value: Any, *, limit: int = 20) -> List[str]:
...
def _ultra_clause_split(text: str) -> List[str]:
    clean = sanitize_text(text, max_chars=4000)
    chunks = re.split(r"(?<=[.!?])\s+|\n+|;\s+", clean)
    return [sanitize_text(chunk, max_chars=320) for chunk in chunks if len(sanitize_text(chunk, max_chars=320)) >= 8]


ULTRA_RISK_RULES: Tuple[Tuple[str, str, Tuple[str, ...], Tuple[str, ...]], ...] = (
    (
        "critical",
        "imminent_self_harm_or_suicide",
        ("kill myself", "end my life", "suicide", "suicidal", "i want to die", "i'm going to die", "hurt myself", "self harm", "self-harm"),
        ("tonight", "right now", "now", "plan", "means", "pills", "gun", "knife", "bridge", "goodbye"),
    ),
    (
        "critical",
        "overdose_or_poisoning",
        ("overdose", "took too much", "too many pills", "can't breathe", "blue lips", "passed out", "unresponsive", "poison", "fentanyl", "narcan"),
        ("now", "right now", "sleepy", "vomit", "chest", "breathing", "emergency"),
    ),
    (
        "critical",
        "violence_immediate_danger",
        ("they're going to hurt me", "someone is hurting me", "domestic violence", "attack me", "threatened me", "weapon"),
        ("now", "right now", "nearby", "in the house", "outside", "emergency"),
    ),
    (
        "high",
        "dangerous_withdrawal_or_detox",
        ("withdrawal", "detox", "seizure", "delirium", "shaking", "hallucinating", "benzo", "alcohol withdrawal", "can't stop drinking"),
        ("severe", "confused", "heart", "blood pressure", "alone", "emergency"),
    ),
    (
        "high",
        "relapse_or_craving_spike",
        ("craving", "urge", "about to use", "going to use", "relapse", "slip", "triggered", "dealer", "plug"),
        ("right now", "tonight", "can't stop", "alone", "cash", "picked up", "using"),
    ),
    (
        "moderate",
        "panic_or_overwhelm",
        ("panic", "panic attack", "overwhelmed", "spiral", "can't calm", "racing thoughts", "dissociate", "dissociating"),
        ("can't breathe", "chest", "numb", "terrified", "right now"),
    ),
    (
        "moderate",
        "medication_uncertainty",
        ("dose", "missed", "double dose", "extra dose", "side effect", "interaction", "mg", "pills"),
        ("unsafe", "too much", "mixed", "alcohol", "opioid", "benzo", "dizzy", "faint"),
    ),
)


ULTRA_INTENT_RULES: Dict[str, Tuple[str, ...]] = {
    "voice_command": ("stop talking", "stop listening", "listener sleep", "wake up", "voice diagnostics", "memory report", "conversation map"),
    "therapy_support": ("feel", "anxious", "depressed", "sad", "ashamed", "trauma", "panic", "cope", "ground", "relationship"),
    "recovery_support": ("craving", "recovery", "relapse", "sober", "clean", "urge", "trigger", "streak", "use again"),
    "medication_safety": ("dose", "mg", "pill", "medication", "interaction", "missed", "side effect", "pharmacy"),
    "dental_recovery": ("tooth", "teeth", "dental", "gum", "brush", "floss", "rinse", "extraction", "crown"),
    "planning": ("plan", "schedule", "routine", "today", "tomorrow", "next", "remind", "checklist"),
    "memory_management": ("remember", "forget", "memory", "you know", "what do you remember", "save this"),
}


class UltraTherapistOrchestrator:
    """Builds a compact control packet before Gemma sees the user prompt."""

    def __init__(self, data: Dict[str, Any], mode: str) -> None:
        self.data = data
        self.mode = normalize_assistant_mode(mode)
        self.state = self._ensure_state()

    def _ensure_state(self) -> Dict[str, Any]:
        raw = self.data.get("assistant_ultra_state")
        if not isinstance(raw, dict):
            raw = {}
        raw.setdefault("schema", "UltraVoiceTherapyState/v4")
        raw.setdefault("turn_count", 0)
        raw.setdefault("session_notes", [])
        raw.setdefault("last_risk_level", "low")
        raw.setdefault("last_intents", [])
        raw.setdefault("speaking_style", "adaptive")
        raw.setdefault("voice_depth", "balanced")
        raw.setdefault("last_updated_ts", time.time())
        if not isinstance(raw.get("session_notes"), list):
            raw["session_notes"] = []
        self.data["assistant_ultra_state"] = raw
        return raw

    def classify_intents(self, text: str) -> List[str]:
        scores: List[Tuple[int, str]] = []
        for intent, needles in ULTRA_INTENT_RULES.items():
            score = _ultra_count_matches(text, needles)
            if score:
                scores.append((score, intent))
        scores.sort(key=lambda item: (item[0], item[1]), reverse=True)
        return [intent for _score, intent in scores[:4]] or ["general_support"]

    def assess_risk(self, text: str) -> Dict[str, Any]:
        clean = _ultra_norm(text, max_chars=8000)
        levels = {"low": 0, "moderate": 1, "high": 2, "critical": 3}
        best = "low"
        reasons: List[str] = []
        matched: List[str] = []
        for level, name, primary, amplifiers in ULTRA_RISK_RULES:
            primary_hits = [phrase for phrase in primary if phrase in clean]
            amp_hits = [phrase for phrase in amplifiers if phrase in clean]
            if not primary_hits:
                continue
            candidate = level
            if level == "high" and len(amp_hits) >= 2:
                candidate = "critical"
            if levels[candidate] > levels[best]:
                best = candidate
            reasons.append(name)
            matched.extend(primary_hits[:4] + amp_hits[:4])
        if best == "low" and _ultra_contains_any(clean, ("help", "urgent", "emergency", "unsafe", "scared")):
            best = "moderate"
            reasons.append("distress_or_urgency_language")
        return {
            "level": best,
            "reasons": sorted(set(reasons)) or ["none"],
            "matched_terms": sorted(set(matched))[:12],
            "requires_human_help_first": best in {"critical", "high"},
        }

    def voice_policy(self, prompt: str, intents: List[str], risk: Dict[str, Any]) -> Dict[str, Any]:
        depth = sanitize_text(self.state.get("voice_depth") or "balanced", max_chars=24).lower()
        if risk.get("level") in {"critical", "high"}:
            return {"style": "urgent-brief", "max_sentences": 6, "chunk_chars": 220, "pace": "short calm commands"}
        if "therapy_support" in intents or self.mode == "Therapy":
            return {"style": "warm-reflective", "max_sentences": 9 if depth != "concise" else 5, "chunk_chars": 300, "pace": "slow supportive"}
        if "recovery_support" in intents or self.mode == "Recovery Coach":
            return {"style": "direct-momentum", "max_sentences": 8 if depth != "concise" else 5, "chunk_chars": 280, "pace": "clear action ladder"}
        if depth == "deep":
            return {"style": "detailed-but-structured", "max_sentences": 14, "chunk_chars": 420, "pace": "structured"}
        return {"style": "practical", "max_sentences": 8, "chunk_chars": 340, "pace": "conversational"}

    def response_protocol(self, intents: List[str], risk: Dict[str, Any]) -> List[str]:
        level = sanitize_text(risk.get("level") or "low", max_chars=24)
        if level == "critical":
            return [
                "Start with immediate safety: encourage calling emergency services/local crisis line or a trusted nearby person now.",
                "Ask the user to move away from means/substances/unsafe locations when relevant.",
                "Use short supportive sentences; do not debate, diagnose, or give complex analysis.",
                "After safety, offer one grounding action and one next contact step.",
            ]
        if level == "high":
            return [
                "Prioritize real-time human support and medical/professional help when overdose, withdrawal, or immediate relapse risk appears.",
                "Make a 10-minute safety plan before longer reflection.",
                "Use direct but non-shaming language.",
            ]
        if "therapy_support" in intents or self.mode == "Therapy":
            return [
                "Validate first, then reflect the likely emotion/need without claiming certainty.",
                "Offer one grounding exercise, one reframe, and one next step.",
                "Use gentle questions; avoid sounding like a scripted intake form.",
            ]
        if "recovery_support" in intents or self.mode == "Recovery Coach":
            return [
                "Treat cravings/slips as solvable moments, not moral failures.",
                "Structure around next 10 minutes, next hour, and tonight.",
                "Suggest environment changes, outreach, urge surfing, and friction against access.",
            ]
        if "medication_safety" in intents:
            return [
                "Be conservative; do not invent dosing advice.",
                "Use stored vault data when available, and recommend pharmacist/clinician confirmation for uncertain dosing or interactions.",
                "Flag urgent symptoms clearly.",
            ]
        return ["Answer directly, use vault context carefully, and keep next steps concrete."]

    def session_memory_notes(self, prompt: str, intents: List[str], risk: Dict[str, Any]) -> List[str]:
        notes = _ultra_safe_list(self.state.get("session_notes"), limit=ULTRA_SESSION_MAX_NOTES)
        recent = notes[-8:]
        if risk.get("level") in {"critical", "high"}:
            recent.append(f"risk:{risk.get('level')}:{','.join(risk.get('reasons') or [])}")
        if intents:
            recent.append("intent:" + ",".join(intents[:3]))
        return recent[-10:]

    def build_packet(self, prompt: str) -> str:
        clean_prompt = sanitize_text(prompt, max_chars=2600)
        intents = self.classify_intents(clean_prompt)
        risk = self.assess_risk(clean_prompt)
        voice = self.voice_policy(clean_prompt, intents, risk)
        protocol = self.response_protocol(intents, risk)
        session_notes = self.session_memory_notes(clean_prompt, intents, risk)
        profile = self._profile_summary()
        packet = [
            "[ultra_therapist_orchestrator]",
            f"version={ULTRA_PATCH_VERSION} mode={self.mode}",
            f"intent={', '.join(intents)} risk_level={risk.get('level')} risk_reasons={'; '.join(risk.get('reasons') or [])}",
            f"voice_style={voice.get('style')} max_spoken_sentences={voice.get('max_sentences')} bark_chunk_chars={voice.get('chunk_chars')} pace={voice.get('pace')}",
            "safety_boundary=This is supportive coaching, not diagnosis, emergency services, detox, or a replacement for clinician/pharmacist care.",
            "response_protocol:",
        ]
        packet.extend([f"- {item}" for item in protocol])
        packet.append("memory_hygiene:")
        packet.extend(
            [
                "- Treat memory as fallible context; current message and explicit vault facts override older memories.",
                "- Do not mention internal tags/scores unless the user asks for diagnostics.",
                "- Prefer stable facts/preferences and recent high-importance episodes; ignore stale contradictions unless relevant.",
            ]
        )
        if session_notes:
            packet.append("recent_voice_session_notes:")
            packet.extend([f"- {note}" for note in session_notes[-8:]])
        if profile:
            packet.append("ultra_profile_summary:")
            packet.append(profile)
        packet.append("[/ultra_therapist_orchestrator]")
        return sanitize_text("\n".join(packet), max_chars=ULTRA_PLAN_MAX_CHARS)

    def _profile_summary(self) -> str:
        try:
            graph = AdvancedTherapeuticMemoryGraph(self.data)
            profile = graph.memory.get("profile") if isinstance(graph.memory.get("profile"), dict) else {}
            tags = profile.get("tag_counts") if isinstance(profile.get("tag_counts"), dict) else {}
            top_tags = ", ".join(list(tags.keys())[:8])
            return sanitize_text(
                f"advanced_turns={int(profile.get('turn_count') or 0)} last_risk={profile.get('last_risk_level', 'unknown')} "
                f"affective_valence={safe_float(profile.get('affective_valence')):.2f} top_tags={top_tags or 'none'}",
                max_chars=600,
            )
        except Exception:
            return ""

    def record_turn_metadata(self, prompt: str, reply: str) -> None:
        intents = self.classify_intents(prompt)
        risk = self.assess_risk(prompt)
        self.state["turn_count"] = int(self.state.get("turn_count") or 0) + 1
        self.state["last_intents"] = intents
        self.state["last_risk_level"] = risk.get("level", "low")
        self.state["last_updated_ts"] = time.time()
        notes = _ultra_safe_list(self.state.get("session_notes"), limit=ULTRA_SESSION_MAX_NOTES)
        note = f"turn:{int(self.state['turn_count'])}:mode={self.mode}:intent={','.join(intents[:3])}:risk={risk.get('level')}:reply_chars={len(str(reply or ''))}"
        notes.append(note)
        self.state["session_notes"] = notes[-ULTRA_SESSION_MAX_NOTES:]


class UltraMemoryConsolidator:
    """Adds higher-level facts and contradiction hygiene over AdvancedTherapeuticMemoryGraph."""

    def __init__(self, data: Dict[str, Any]) -> None:
        self.data = data
        raw = self.data.get("assistant_ultra_memory")
        if not isinstance(raw, dict):
            raw = {}
        raw.setdefault("schema", "UltraMemoryConsolidator/v4")
        raw.setdefault("beliefs", [])
        raw.setdefault("contradictions", [])
        raw.setdefault("maintenance_turns", 0)
        raw.setdefault("updated_ts", time.time())
        if not isinstance(raw.get("beliefs"), list):
            raw["beliefs"] = []
        if not isinstance(raw.get("contradictions"), list):
            raw["contradictions"] = []
        self.data["assistant_ultra_memory"] = raw
        self.memory = raw

    @property
    def beliefs(self) -> List[Dict[str, Any]]:
        return self.memory.get("beliefs") if isinstance(self.memory.get("beliefs"), list) else []

    def observe_turn(self, user_text: str, assistant_text: str, *, mode: str = "General") -> None:
        facts = self._extract_beliefs(user_text, mode=mode)
        merged = {str(item.get("id")): dict(item) for item in self.beliefs if item.get("id")}
        for fact in facts:
            fid = str(fact.get("id") or "")
            if not fid:
                continue
            prior = merged.get(fid, {})
            fact["created_ts"] = safe_float(prior.get("created_ts")) or time.time()
            fact["updated_ts"] = time.time()
            fact["hits"] = int(prior.get("hits") or 0) + 1
            fact["confidence"] = min(1.0, max(safe_float(prior.get("confidence")), safe_float(fact.get("confidence"))) + 0.04)
            merged[fid] = fact
        values = list(merged.values())
        values.sort(key=lambda item: (safe_float(item.get("confidence")), int(item.get("hits") or 0), safe_float(item.get("updated_ts"))), reverse=True)
        self.memory["beliefs"] = values[:160]
        self.memory["maintenance_turns"] = int(self.memory.get("maintenance_turns") or 0) + 1
        self.memory["updated_ts"] = time.time()
        if int(self.memory.get("maintenance_turns") or 0) % ULTRA_MEMORY_MAINTENANCE_EVERY_TURNS == 0:
            self._detect_contradictions()

    def _extract_beliefs(self, user_text: str, *, mode: str) -> List[Dict[str, Any]]:
        clean = sanitize_text(user_text, max_chars=2800)
        lowered = clean.lower()
        candidates: List[Tuple[str, str, float]] = []
        preference_patterns = (
            (r"\bi prefer\s+([^.!?]{3,160})", "preference"),
            (r"\bi like\s+([^.!?]{3,160})", "preference"),
            (r"\bi don't like\s+([^.!?]{3,160})", "preference_negative"),
            (r"\bremember that\s+([^.!?]{3,180})", "explicit_memory"),
            (r"\bmy\s+([a-z][a-z\s]{2,40})\s+is\s+([^.!?]{2,160})", "self_fact"),
            (r"\bi am\s+([^.!?]{3,130})", "self_state"),
            (r"\bi'm\s+([^.!?]{3,130})", "self_state"),
        )
        for pattern, kind in preference_patterns:
            for match in re.finditer(pattern, lowered, re.I):
                text = sanitize_text(match.group(0), max_chars=220)
                if text:
                    confidence = 0.82 if kind == "explicit_memory" else 0.58
                    candidates.append((kind, text, confidence))
        if any(token in lowered for token in ("craving", "trigger", "relapse", "sober", "clean")):
            for clause in _ultra_clause_split(clean)[:5]:
                if any(token in clause.lower() for token in ("craving", "trigger", "relapse", "sober", "clean")):
                    candidates.append(("recovery_pattern", clause, 0.62))
        if any(token in lowered for token in ("panic", "anxious", "overwhelmed", "grounding", "cope")):
            for clause in _ultra_clause_split(clean)[:5]:
                if any(token in clause.lower() for token in ("panic", "anxious", "overwhelmed", "grounding", "cope")):
                    candidates.append(("therapy_pattern", clause, 0.56))
        out: List[Dict[str, Any]] = []
        for kind, text, confidence in candidates[:12]:
            out.append(
                {
                    "id": _voice_patch_sha(f"ultra-belief:{kind}:{text}"),
                    "kind": kind,
                    "mode": normalize_assistant_mode(mode),
                    "text": sanitize_text(text, max_chars=260),
                    "confidence": confidence,
                    "tags": _voice_patch_tags(text),
                    "vector": _voice_patch_vector(text),
                }
            )
        return out
    ...
    def _detect_contradictions(self) -> None:
        ...
        recent = self.beliefs[:120]
        for i, left in enumerate(recent):
            ltext = _ultra_norm(left.get("text"), max_chars=400)
            if not ltext:
                continue
            for right in recent[i + 1 : i + 18]:
                rtext = _ultra_norm(right.get("text"), max_chars=400)
                if not rtext:
                    continue
                neg_flip = ((" don't " in ltext or " do not " in ltext or " hate " in ltext) and (" like " in rtext or " prefer " in rtext)) or ((" don't " in rtext or " do not " in rtext or " hate " in rtext) and (" like " in ltext or " prefer " in ltext))
                semantic_close = _voice_patch_cosine(left.get("vector") or [], right.get("vector") or []) > 0.42
                if neg_flip and semantic_close:
                    contradictions.append(
                        {
                            "id": _voice_patch_sha(f"contra:{left.get('id')}:{right.get('id')}"),
                            "left": left.get("text"),
                            "right": right.get("text"),
                            "detected_ts": time.time(),
                        }
                    )
        self.memory["contradictions"] = contradictions[:40]

    def context_block(self, query: str) -> str:
        clean = sanitize_text(query, max_chars=2200)
        q_vec = _voice_patch_vector(clean)
        scored: List[Tuple[float, Dict[str, Any]]] = []
        for belief in self.beliefs:
            score = _voice_patch_cosine(q_vec, belief.get("vector") or [])
            score += 0.08 * safe_float(belief.get("confidence"))
            if score > 0.10:
                scored.append((score, belief))
        scored.sort(key=lambda item: item[0], reverse=True)
        lines = ["[ultra_consolidated_memory]", f"beliefs={len(self.beliefs)} contradictions={len(self.memory.get('contradictions') or [])}"]
        for idx, (score, belief) in enumerate(scored[:7], 1):
            lines.append(f"{idx}. score={score:.3f} kind={belief.get('kind')} confidence={safe_float(belief.get('confidence')):.2f}: {sanitize_text(belief.get('text'), max_chars=260)}")
        contradictions = self.memory.get("contradictions") if isinstance(self.memory.get("contradictions"), list) else []
        if contradictions:
            lines.append("memory_conflict_note=Some older beliefs conflict; ask or defer to the user's latest correction when relevant.")
        lines.append("[/ultra_consolidated_memory]")
        return sanitize_text("\n".join(lines), max_chars=2600)


ULTRA_ORIGINAL_BUILD_MEMORY_CONTEXT = build_in_house_vector_memory_context


def build_in_house_vector_memory_context(data: Dict[str, Any], prompt: str, mode: str) -> str:
    base = ULTRA_ORIGINAL_BUILD_MEMORY_CONTEXT(data, prompt, mode)
    try:
        ultra = UltraMemoryConsolidator(data).context_block(prompt)
    except Exception as exc:
        ultra = f"[ultra_consolidated_memory]\nUnavailable: {sanitize_text(exc, max_chars=220)}\n[/ultra_consolidated_memory]"
    return sanitize_text(base + "\n\n" + ultra, max_chars=13500)


ULTRA_ORIGINAL_RUN_ASSISTANT_REQUEST = run_assistant_request
...
def run_assistant_request(
    ...
    active_mode = normalize_assistant_mode(mode)
    orchestrator = UltraTherapistOrchestrator(data, active_mode)
    packet = orchestrator.build_packet(prompt)
    enhanced_prompt = sanitize_text(prompt, max_chars=2200) + "\n\n" + packet
    result = ULTRA_ORIGINAL_RUN_ASSISTANT_REQUEST(key, data, enhanced_prompt, selected_med_id, settings, mode=active_mode, on_delta=on_delta)
    try:
        orchestrator.record_turn_metadata(prompt, result)
    except Exception:
        pass
    return result


ULTRA_ORIGINAL_ADVANCED_STORE_LATEST_TURN = DesktopMedSafeApp._voice_memory_store_latest_turn


def _ultra_voice_memory_store_latest_turn(self: "DesktopMedSafeApp") -> None:
    ULTRA_ORIGINAL_ADVANCED_STORE_LATEST_TURN(self)
    try:
        history = list(self.data_cache.get("assistant_history") or [])
        last_user = ""
        last_assistant = ""
        mode = normalize_assistant_mode(getattr(self, "assistant_mode", "General"))
        for item in reversed(history):
            role = sanitize_text(item.get("role") or "", max_chars=24).lower()
            content = sanitize_text(item.get("content") or "", max_chars=12000)
            if role == "assistant" and not last_assistant:
                last_assistant = content
                mode = normalize_assistant_mode(item.get("mode") or mode)
            elif role == "user" and last_assistant:
                last_user = content
                break
        if not last_user or not last_assistant:
            return
        sig = _voice_patch_sha(f"ultra-store:{mode}:{last_user}:{last_assistant}")
        if sig == getattr(self, "ultra_voice_last_stored_signature", ""):
            return
        UltraMemoryConsolidator(self.data_cache).observe_turn(last_user, last_assistant, mode=mode)
        UltraTherapistOrchestrator(self.data_cache, mode).record_turn_metadata(last_user, last_assistant)
        self.ultra_voice_last_stored_signature = sig
        self.persist_data_async()
    except Exception as exc:
        self._voice_set_status(f"Ultra memory store skipped: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp._voice_memory_store_latest_turn = _ultra_voice_memory_store_latest_turn


ULTRA_ORIGINAL_DESKTOP_INIT = DesktopMedSafeApp.__init__


def _ultra_desktop_init(self: "DesktopMedSafeApp") -> None:
    ULTRA_ORIGINAL_DESKTOP_INIT(self)
    self.ultra_voice_last_stored_signature = ""
    self.ultra_last_interrupt_ts = 0.0
    self.ultra_interrupt_rms_history: List[float] = []
    self.ultra_voice_turns_since_calibration = 0
    self.ultra_barge_in_sensitivity = float(os.environ.get("VOICE_BARGE_IN_SENSITIVITY", "1.0") or "1.0")
    self.ultra_voice_diagnostics_enabled = False
    try:
        UltraTherapistOrchestrator(self.data_cache, getattr(self, "assistant_mode", "General"))
        UltraMemoryConsolidator(self.data_cache)
    except Exception:
        pass


DesktopMedSafeApp.__init__ = _ultra_desktop_init


ULTRA_ORIGINAL_ADVANCED_COMMAND_RESULT = DesktopMedSafeApp._advanced_voice_command_result


def _ultra_voice_command_result(self: "DesktopMedSafeApp", text: str) -> bool:
    clean = _advanced_normalize_for_match(text)
    if not clean:
        return True
    if clean.startswith("remember this") or clean.startswith("remember that"):
        memo = re.sub(r"^remember\s+(this|that)\s*", "", clean, flags=re.I).strip()
        if memo:
            try:
                UltraMemoryConsolidator(self.data_cache).observe_turn(f"remember that {memo}", "Stored as an explicit memory note.", mode=getattr(self, "assistant_mode", "General"))
                self.persist_data_async()
                self._voice_set_status("Stored explicit ultra memory note.")
            except Exception as exc:
                self._voice_set_status(f"Could not store explicit memory: {sanitize_text(exc, max_chars=180)}")
            return True
    if "voice diagnostics" in clean or clean == "diagnostics":
        self.on_ultra_voice_diagnostics()
        return True
    if "conversation map" in clean or "session map" in clean:
        self.on_ultra_conversation_map()
        return True
    if "set voice concise" in clean or "concise voice" in clean:
        self._ultra_set_voice_depth("concise")
        return True
    if "set voice deep" in clean or "deep voice" in clean:
        self._ultra_set_voice_depth("deep")
        return True
    if "set voice balanced" in clean or "balanced voice" in clean:
        self._ultra_set_voice_depth("balanced")
        return True
    if "barge in more sensitive" in clean or "interrupt more sensitive" in clean:
        self.ultra_barge_in_sensitivity = max(0.55, float(getattr(self, "ultra_barge_in_sensitivity", 1.0)) * 0.82)
        self._voice_set_status(f"Barge-in sensitivity increased ({self.ultra_barge_in_sensitivity:.2f}).")
        return True
    if "barge in less sensitive" in clean or "interrupt less sensitive" in clean:
        self.ultra_barge_in_sensitivity = min(1.8, float(getattr(self, "ultra_barge_in_sensitivity", 1.0)) * 1.18)
        self._voice_set_status(f"Barge-in sensitivity reduced ({self.ultra_barge_in_sensitivity:.2f}).")
        return True
    return ULTRA_ORIGINAL_ADVANCED_COMMAND_RESULT(self, text)


DesktopMedSafeApp._advanced_voice_command_result = _ultra_voice_command_result


def _ultra_set_voice_depth(self: "DesktopMedSafeApp", depth: str) -> None:
    clean_depth = sanitize_text(depth, max_chars=24).lower()
    if clean_depth not in {"concise", "balanced", "deep"}:
        clean_depth = "balanced"
    try:
        orchestrator = UltraTherapistOrchestrator(self.data_cache, getattr(self, "assistant_mode", "General"))
        orchestrator.state["voice_depth"] = clean_depth
        orchestrator.state["last_updated_ts"] = time.time()
        self.persist_data_async()
    except Exception:
        pass
    self._voice_set_status(f"Voice depth set to {clean_depth}.")


DesktopMedSafeApp._ultra_set_voice_depth = _ultra_set_voice_depth


ULTRA_ORIGINAL_INTERRUPT_WORKER = VoiceActivityInterruptor._worker


def _ultra_interrupt_worker(self: "VoiceActivityInterruptor") -> None:
    try:
        import audioop
        import speech_recognition as sr  # type: ignore
    except Exception as exc:
        ...
        return
    recognizer = sr.Recognizer()
    floor = float(getattr(self, "energy_floor", 320.0) or 320.0)
    noise_ewma = floor
    recent_rms: List[float] = []
    try:
        with sr.Microphone() as source:
            recognizer.adjust_for_ambient_noise(source, duration=0.85)
            floor = max(180.0, float(getattr(recognizer, "energy_threshold", floor)))
            noise_ewma = floor
            self.energy_floor = floor
            while not self.stop_event.is_set():
                try:
                    audio = recognizer.listen(source, timeout=0.28, phrase_time_limit=0.42)
                except Exception:
                    continue
                raw = audio.get_raw_data(convert_rate=16000, convert_width=2)
                rms = float(audioop.rms(raw, 2)) if raw else 0.0
                recent_rms.append(rms)
                recent_rms = recent_rms[-24:]
                speaking = bool(getattr(self.app, "bark_voice", None) and self.app.bark_voice.playing)
                if not speaking:
                    noise_ewma = (noise_ewma * 0.92) + (rms * 0.08)
                    self.voice_frames = max(0, self.voice_frames - 1)
                    continue
                sensitivity = max(0.45, min(2.0, float(getattr(self.app, "ultra_barge_in_sensitivity", 1.0) or 1.0)))
                dynamic_floor = max(self.energy_floor, noise_ewma, 200.0)
                medianish = sorted(recent_rms)[len(recent_rms) // 2] if recent_rms else dynamic_floor
                threshold = max(dynamic_floor * (1.48 * sensitivity), medianish + (180.0 * sensitivity))
                if rms > threshold:
                    self.voice_frames += 1
                else:
                    self.voice_frames = max(0, self.voice_frames - 1)
                now = time.time()
                if self.voice_frames >= 2 and now >= self.cooldown_until:
                    self.cooldown_until = now + 0.85
                    self.voice_frames = 0
                    try:
                        self.app.ultra_interrupt_rms_history.append(float(rms))
                        self.app.ultra_interrupt_rms_history = self.app.ultra_interrupt_rms_history[-80:]
                    except Exception:
                        pass
                    self.app.run_on_ui_thread(self.app.on_voice_interrupt_detected, rms)
    except Exception as exc:
        self.app.run_on_ui_thread(self.app._voice_set_status, f"Ultra interrupt detector stopped: {sanitize_text(exc, max_chars=180)}")


VoiceActivityInterruptor._worker = _ultra_interrupt_worker


ULTRA_ORIGINAL_ADVANCED_BARK_SPEAK_ASYNC = BarkCppVoiceEngine.speak_async


def _ultra_bark_speak_async(self: "BarkCppVoiceEngine", text: str) -> None:
    # Runtime-chunk sizing adapts to ultra voice depth and risk state.
    try:
        state = self.app.data_cache.get("assistant_ultra_state") if isinstance(self.app.data_cache.get("assistant_ultra_state"), dict) else {}
        depth = sanitize_text(state.get("voice_depth") or "balanced", max_chars=24).lower()
        if depth == "concise":
            os.environ["BARK_CHUNK_CHARS"] = os.environ.get("BARK_CHUNK_CHARS", "260")
        elif depth == "deep":
            os.environ["BARK_CHUNK_CHARS"] = os.environ.get("BARK_CHUNK_CHARS", "460")
        else:
            os.environ["BARK_CHUNK_CHARS"] = os.environ.get("BARK_CHUNK_CHARS", "340")
    except Exception:
        pass
    return ULTRA_ORIGINAL_ADVANCED_BARK_SPEAK_ASYNC(self, text)


BarkCppVoiceEngine.speak_async = _ultra_bark_speak_async


def _ultra_voice_diagnostics_text(self: "DesktopMedSafeApp") -> str:
    try:
        state = self.data_cache.get("assistant_ultra_state") if isinstance(self.data_cache.get("assistant_ultra_state"), dict) else {}
        ultra_memory = self.data_cache.get("assistant_ultra_memory") if isinstance(self.data_cache.get("assistant_ultra_memory"), dict) else {}
        advanced = self.data_cache.get("assistant_advanced_memory") if isinstance(self.data_cache.get("assistant_advanced_memory"), dict) else {}
        bark = getattr(self, "bark_voice", None)
        rms_history = list(getattr(self, "ultra_interrupt_rms_history", []) or [])
        avg_rms = (sum(float(x) for x in rms_history) / len(rms_history)) if rms_history else 0.0
        return "\n".join(
            [
                f"Ultra patch: {ULTRA_PATCH_VERSION}",
                f"Mode: {normalize_assistant_mode(getattr(self, 'assistant_mode', 'General'))}",
                f"Voice depth: {state.get('voice_depth', 'balanced')} | last risk: {state.get('last_risk_level', 'low')} | turns: {int(state.get('turn_count') or 0)}",
                f"Listener: enabled={bool(getattr(self, 'voice_listener_enabled', False))} awake={bool(getattr(self, 'advanced_voice_listener_awake', True))} wake_word={bool(getattr(self, 'advanced_voice_wake_word_enabled', True))}",
                f"Interrupts: count={int(getattr(self, 'advanced_voice_interrupt_count', 0))} avg_rms={avg_rms:.1f} sensitivity={float(getattr(self, 'ultra_barge_in_sensitivity', 1.0)):.2f}",
                f"Bark: configured={bool(bark and bark.available())} speaking={bool(bark and bark.playing)} status={bark.status_text() if bark else 'missing'}",
                f"Memory: advanced_facts={len(advanced.get('facts') or [])} advanced_events={len(advanced.get('events') or [])} ultra_beliefs={len(ultra_memory.get('beliefs') or [])} contradictions={len(ultra_memory.get('contradictions') or [])}",
            ]
        )
    except Exception as exc:
        return f"Ultra diagnostics unavailable: {sanitize_text(exc, max_chars=220)}"


DesktopMedSafeApp._ultra_voice_diagnostics_text = _ultra_voice_diagnostics_text


def on_ultra_voice_diagnostics(self: "DesktopMedSafeApp") -> None:
    text = self._ultra_voice_diagnostics_text()
    self._voice_set_status(text)
    try:
        box = self.ids.get("assistant_context_window_report")
        if box is not None:
            current = sanitize_text(getattr(box, "text", ""), max_chars=5200)
...
DesktopMedSafeApp.on_ultra_voice_diagnostics = on_ultra_voice_diagnostics


def on_ultra_conversation_map(self: "DesktopMedSafeApp") -> None:
    try:
        prompt = sanitize_text(self.ids.assistant_input.text if "assistant_input" in self.ids else "current session", max_chars=1000)
        orchestrator = UltraTherapistOrchestrator(self.data_cache, getattr(self, "assistant_mode", "General"))
        risk = orchestrator.assess_risk(prompt)
        intents = orchestrator.classify_intents(prompt)
        graph = AdvancedTherapeuticMemoryGraph(self.data_cache)
        recall = graph.recall(prompt or "current session", mode=getattr(self, "assistant_mode", "General"), top_k=6)
        ultra = UltraMemoryConsolidator(self.data_cache)
        lines = [
            "Ultra Conversation Map",
            f"Intent: {', '.join(intents)}",
            f"Risk: {risk.get('level')} ({'; '.join(risk.get('reasons') or [])})",
            f"Recall hits: {len(recall)} | Ultra beliefs: {len(ultra.beliefs)}",
            "Top memory anchors:",
        ]
        for idx, item in enumerate(recall[:5], 1):
            body = sanitize_text(item.get("text") or item.get("user") or item.get("assistant_summary") or "", max_chars=240)
            lines.append(f"{idx}. {sanitize_text(item.get('kind') or 'memory', max_chars=32)}: {body}")
        text = "\n".join(lines)
        self._voice_set_status(text)
        self.refresh_assistant_context_panel()
    except Exception as exc:
        self._voice_set_status(f"Conversation map unavailable: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_ultra_conversation_map = on_ultra_conversation_map


ULTRA_ORIGINAL_REFRESH_CONTEXT_PANEL = DesktopMedSafeApp.refresh_assistant_context_panel


def _ultra_refresh_assistant_context_panel(self: "DesktopMedSafeApp") -> None:
    ULTRA_ORIGINAL_REFRESH_CONTEXT_PANEL(self)
    try:
        box = self.ids.get("assistant_context_window_report")
        if box is None:
            return
        current = sanitize_text(getattr(box, "text", ""), max_chars=7000)
        prompt = sanitize_text(self.ids.assistant_input.text if "assistant_input" in self.ids else "current context", max_chars=1000)
        orch = UltraTherapistOrchestrator(self.data_cache, getattr(self, "assistant_mode", "General"))
        compact = orch.build_packet(prompt)
        # Show a shortened packet in UI only; full packet goes into the model call.
        box.text = sanitize_text(current + "\n\n" + sanitize_text(compact, max_chars=1400), max_chars=9000)
    except Exception:
        pass


DesktopMedSafeApp.refresh_assistant_context_panel = _ultra_refresh_assistant_context_panel


ULTRA_ORIGINAL_BUILD_ASSISTANT_TAB = DesktopMedSafeApp._build_assistant_tab


def _ultra_build_assistant_tab(self: "DesktopMedSafeApp", tab: Any) -> None:
    ULTRA_ORIGINAL_BUILD_ASSISTANT_TAB(self, tab)
    if ctk is None:
        return
    voice_frame = self.ids.get("voice_frame")
    if voice_frame is None:
    ...
    try:
        voice_frame.grid_columnconfigure((0, 1, 2, 3), weight=1)
        diag = self._button(voice_frame, text="Ultra Diagnostics", command=self.on_ultra_voice_diagnostics, tone="neutral")
        diag.grid(row=4, column=0, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_ultra_diag_button"] = diag
        cmap = self._button(voice_frame, text="Conversation Map", command=self.on_ultra_conversation_map, tone="neutral")
        cmap.grid(row=4, column=1, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_ultra_map_button"] = cmap
        concise = self._button(voice_frame, text="Voice Concise", command=lambda: self._ultra_set_voice_depth("concise"), tone="neutral")
        concise.grid(row=4, column=2, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_ultra_concise_button"] = concise
        deep = self._button(voice_frame, text="Voice Deep", command=lambda: self._ultra_set_voice_depth("deep"), tone="neutral")
        deep.grid(row=4, column=3, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_ultra_deep_button"] = deep
    except Exception:
        pass


DesktopMedSafeApp._build_assistant_tab = _ultra_build_assistant_tab
# --- end ultra voice/memory/therapy orchestration layer ------------------------

# --- extreme voice/memory/therapy supervision layer ---------------------------
# This layer intentionally stays dependency-light and single-file. It adds a
# deterministic supervisory shell around the existing Gemma/Bark/vector-memory
# wiring rather than requiring another service.

EXTREME_PATCH_VERSION = "2026-05-08.extreme.supervised-voice-memory.v5"
EXTREME_MAX_LEDGER_ITEMS = 160
EXTREME_MAX_AUDIT_ITEMS = 220
EXTREME_MAX_PINNED_ITEMS = 48
EXTREME_CONTEXT_MAX_CHARS = 15500
EXTREME_STATE_IDLE = "idle"
EXTREME_STATE_LISTENING = "listening"
EXTREME_STATE_THINKING = "thinking"
EXTREME_STATE_SPEAKING = "speaking"
EXTREME_STATE_INTERRUPTED = "interrupted"
EXTREME_STATE_SAFETY = "safety"


def _extreme_hash(value: Any) -> str:
    try:
        return hashlib.sha256(sanitize_text(value, max_chars=12000).encode("utf-8", errors="ignore")).hexdigest()[:24]
    except Exception:
        return uuid.uuid4().hex[:24]


def _extreme_words(value: Any, *, limit: int = 220) -> List[str]:
    text = sanitize_text(value, max_chars=6000).lower()
    words = re.findall(r"[a-z0-9][a-z0-9'_-]{1,32}", text)
    stop = {
        "the", "and", "that", "this", "with", "for", "you", "your", "are", "was", "were", "have", "has",
        "had", "but", "not", "from", "into", "about", "what", "when", "where", "why", "how", "can", "could",
        "would", "should", "just", "like", "really", "very", "then", "than", "they", "them", "their", "there",
        "here", "been", "being", "will", "shall", "also", "because", "through", "while", "after", "before",
    }
    out: List[str] = []
    for word in words:
...
def _extreme_jaccard(a: Any, b: Any) -> float:
    aw = set(_extreme_words(a, limit=180))
    bw = set(_extreme_words(b, limit=180))
    if not aw or not bw:
        return 0.0
    return len(aw & bw) / max(1, len(aw | bw))


def _extreme_first_sentence(text: Any, *, max_chars: int = 220) -> str:
    clean = sanitize_text(text, max_chars=1200)
    match = re.search(r"(.+?[.!?])(?:\s|$)", clean)
    return sanitize_text((match.group(1) if match else clean), max_chars=max_chars)


class ExtremeSafetyShield:
    """Deterministic supervision for high-risk therapy/recovery/medical turns."""

    CRITICAL_TERMS = (
        "kill myself", "end my life", "suicide", "suicidal", "i want to die", "i'm going to die",
        "overdose", "took too much", "can't breathe", "chest pain", "unconscious", "seizure", "violent", "hurt someone",
        "dangerous withdrawal", "delirium", "blue lips", "passed out",
    )
    HIGH_TERMS = (
        "self harm", "cut myself", "harm myself", "relapse tonight", "use tonight", "panic attack", "craving is unbearable",
        "withdrawal", "detox", "i might do something", "not safe", "spiraling", "voices telling me", "paranoid",
    )
    MODERATE_TERMS = (
        "craving", "urge", "panic", "overwhelmed", "depressed", "hopeless", "ashamed", "triggered", "flashback",
        "missed dose", "early dose", "double dose", "side effects", "interaction", "withdrawals",
    )

    def __init__(self, data: Dict[str, Any], mode: str = "General") -> None:
        self.data = data
        self.mode = normalize_assistant_mode(mode)
        state = data.setdefault("assistant_extreme_state", {})
        if not isinstance(state, dict):
            state = {}
            data["assistant_extreme_state"] = state
        state.setdefault("schema", "ExtremeSafetyShield/v5")
        state.setdefault("last_state", EXTREME_STATE_IDLE)
        state.setdefault("last_risk", "low")
        state.setdefault("last_intent", "general")
        state.setdefault("voice_persona", "warm concise")
        self.state = state

    def classify(self, prompt: str) -> Dict[str, Any]:
        clean = sanitize_text(prompt, max_chars=3000)
        lowered = clean.lower()
        reasons: List[str] = []
        level = "low"
        ...
        intents = []
        try:
            intents = UltraTherapistOrchestrator(self.data, self.mode).classify_intents(clean)
        except Exception:
            pass
        if not intents:
            if any(token in lowered for token in ("dose", "med", "pill", "mg", "pharmacy")):
                intents.append("medication")
            if any(token in lowered for token in ("craving", "relapse", "sober", "clean", "recovery")):
                intents.append("recovery")
            if any(token in lowered for token in ("feel", "anxiety", "panic", "sad", "angry", "therapy")):
                intents.append("therapy")
            if not intents:
                intents.append("general")
        self.state["last_risk"] = level
        self.state["last_intent"] = ",".join(intents[:4])
        self.state["last_updated_ts"] = time.time()
        return {"level": level, "reasons": reasons[:8], "intents": intents[:6], "prompt_hash": _extreme_hash(clean)}

    def should_guardrail(self, prompt: str, risk: Optional[Dict[str, Any]] = None) -> bool:
        item = risk or self.classify(prompt)
        return item.get("level") == "critical"

    def deterministic_guardrail_reply(self, prompt: str) -> str:
        clean = sanitize_text(prompt, max_chars=1000)
        self.state["last_state"] = EXTREME_STATE_SAFETY
        return (
            "I’m really concerned about immediate safety from what you said. I can stay supportive, but this needs human help now.\n\n"
            "1. If you might hurt yourself, might hurt someone else, may have overdosed, cannot breathe, have chest pain, seizure symptoms, or feel medically unsafe, call your local emergency number now or go to the nearest emergency department.\n"
            "2. Move away from anything you could use to harm yourself or take more medication/substances. Put distance, a locked door, or another person between you and it.\n"
            "3. Contact a trusted person and use direct words: “I’m not safe alone right now. Please stay with me or call emergency help.”\n"
            "4. While help is coming: sit down, keep your feet on the floor, name five things you can see, and take slow breaths.\n\n"
            "I’m not a crisis line, clinician, or emergency service. The safest next step is immediate live support."
        )

    def build_packet(self, prompt: str, risk: Optional[Dict[str, Any]] = None) -> str:
        item = risk or self.classify(prompt)
        level = sanitize_text(item.get("level") or "low", max_chars=24)
        intents = ", ".join(sanitize_text(x, max_chars=40) for x in list(item.get("intents") or [])[:6])
        reasons = "; ".join(sanitize_text(x, max_chars=80) for x in list(item.get("reasons") or [])[:8]) or "none"
        style = sanitize_text(self.state.get("voice_persona") or "warm concise", max_chars=80)
        if level in {"high", "critical"}:
            response_contract = "lead with safety and live support; use short, direct steps; do not over-explain"
        elif "therapy" in intents or self.mode == "Therapy":
            response_contract = "validate, reflect, offer one grounding tool, then one doable next action"
        elif "recovery" in intents or self.mode == "Recovery Coach":
            response_contract = "protect next 10 minutes, next hour, and today; treat slips as data not identity"
        else:
            response_contract = "answer directly, then add practical next steps and uncertainty boundaries"
        return sanitize_text(
            "\n".join(
                [
                    "[extreme_supervisor_packet]",
                    f"version: {EXTREME_PATCH_VERSION}",
                    f"mode: {self.mode}",
                    f"detected_intents: {intents}",
                    f"risk_level: {level}",
                    f"risk_triggers: {reasons}",
                    f"voice_persona: {style}",
                    f"response_contract: {response_contract}",
                    "safety_policy: do not diagnose; do not claim to be a therapist/doctor/emergency service; for immediate danger, urge live emergency help.",
                    "memory_policy: use retrieved memory as user-provided context only; never treat memory as clinical proof; ignore stale or contradictory memory unless user confirms it.",
                    "speech_policy: if voice mode is active, prefer fewer sections, shorter sentences, and interrupt-friendly chunks.",
                    "[/extreme_supervisor_packet]",
                ]
            ),
            max_chars=4200,
...
class ExtremeMemoryLab:
    """Longer-horizon single-file memory ledger with pins, hypotheses, and audit trail."""

    def __init__(self, data: Dict[str, Any]) -> None:
        self.data = data
        raw = data.setdefault("assistant_extreme_memory", {})
        if not isinstance(raw, dict):
            raw = {}
            data["assistant_extreme_memory"] = raw
        raw.setdefault("schema", "ExtremeMemoryLab/v5")
        raw.setdefault("pins", [])
        raw.setdefault("ledger", [])
        raw.setdefault("audit", [])
        raw.setdefault("voice_profiles", {})
        raw.setdefault("last_maintenance_ts", 0.0)
        self.raw = raw

    def _append_unique(self, key: str, item: Dict[str, Any], limit: int) -> bool:
        rows = self.raw.get(key)
        if not isinstance(rows, list):
            rows = []
            self.raw[key] = rows
        sig = sanitize_text(item.get("signature") or _extreme_hash(json.dumps(item, sort_keys=True)), max_chars=80)
        item["signature"] = sig
        for existing in rows:
            if isinstance(existing, dict) and existing.get("signature") == sig:
                existing["last_seen_ts"] = time.time()
                existing["seen_count"] = int(existing.get("seen_count") or 1) + 1
                return False
        rows.append(item)
        self.raw[key] = rows[-limit:]
        return True

    def add_pin(self, text: str, *, source: str = "user", confidence: float = 0.76) -> bool:
        clean = sanitize_text(text, max_chars=700)
        if not clean:
            return False
        item = {
            "text": clean,
            "source": sanitize_text(source, max_chars=48),
            "confidence": max(0.0, min(1.0, float(confidence))),
            "created_ts": time.time(),
            "last_seen_ts": time.time(),
            "seen_count": 1,
            "signature": _extreme_hash("pin:" + clean.lower()),
        }
        return self._append_unique("pins", item, EXTREME_MAX_PINNED_ITEMS)

    def forget_last(self) -> str:
        for key in ("pins", "ledger", "audit"):
            rows = self.raw.get(key)
            if isinstance(rows, list) and rows:
                removed = rows.pop()
                text = sanitize_text((removed.get("text") if isinstance(removed, dict) else str(removed)), max_chars=160)
                return f"Removed latest {key[:-1] if key.endswith('s') else key}: {text}"
        return "No extreme memory item was available to remove."

    def infer_user_facts(self, user_text: str, assistant_text: str, mode: str) -> List[Dict[str, Any]]:
        combined = sanitize_text(user_text, max_chars=1600)
        lowered = combined.lower()
        facts: List[Dict[str, Any]] = []
        patterns = [
            (r"\bremember(?: this| that)?[:\- ]+(.{4,260})", "explicit_remember", 0.92),
            (r"\bi prefer\s+(.{4,220})", "preference", 0.78),
            (r"\bplease call me\s+([A-Za-z0-9_ .'-]{2,80})", "name_preference", 0.86),
            (r"\bmy goal is\s+(.{4,220})", "goal", 0.78),
            (r"\bi am trying to\s+(.{4,220})", "goal", 0.72),
            (r"\bwhen i feel\s+(.{4,220})", "trigger_pattern", 0.68),
        ]
        for pattern, lane, confidence in patterns:
            for match in re.finditer(pattern, combined, flags=re.I | re.S):
                text = sanitize_text(match.group(1), max_chars=260)
                if text:
                    facts.append({"lane": lane, "text": text, "confidence": confidence})
        if any(x in lowered for x in ("craving", "relapse", "trigger", "sober", "clean")):
            facts.append({"lane": "recovery_signal", "text": _extreme_first_sentence(combined), "confidence": 0.62})
        if normalize_assistant_mode(mode) == "Therapy" and any(x in lowered for x in ("anxious", "panic", "sad", "angry", "overwhelmed", "ashamed")):
            facts.append({"lane": "therapy_signal", "text": _extreme_first_sentence(combined), "confidence": 0.58})
        return facts[:8]

    def add_turn(self, user_text: str, assistant_text: str, *, mode: str, risk: Optional[Dict[str, Any]] = None) -> None:
        clean_user = sanitize_text(user_text, max_chars=3000)
        clean_assistant = sanitize_text(assistant_text, max_chars=3000)
        if not clean_user or not clean_assistant:
            return
        now = time.time()
        risk_level = sanitize_text((risk or {}).get("level") or "unknown", max_chars=24)
        words = _extreme_words(clean_user + " " + clean_assistant, limit=40)
        item = {
            "text": _extreme_first_sentence(clean_user, max_chars=360),
            "assistant_hint": _extreme_first_sentence(clean_assistant, max_chars=360),
            "mode": normalize_assistant_mode(mode),
            "risk": risk_level,
            "keywords": words[:28],
            "created_ts": now,
            "last_seen_ts": now,
            "seen_count": 1,
            "importance": self._importance(clean_user, clean_assistant, risk_level),
            "signature": _extreme_hash("ledger:" + normalize_assistant_mode(mode) + clean_user + clean_assistant),
        }
        self._append_unique("ledger", item, EXTREME_MAX_LEDGER_ITEMS)
        for fact in self.infer_user_facts(clean_user, clean_assistant, mode):
            self.add_pin(f"{fact['lane']}: {fact['text']}", source="turn_extraction", confidence=float(fact.get("confidence") or 0.6))
        self._append_unique(
            "audit",
            {
                "text": f"turn stored mode={normalize_assistant_mode(mode)} risk={risk_level} words={len(words)}",
                "created_ts": now,
                "signature": _extreme_hash("audit:" + str(now) + clean_user[:200]),
            },
            EXTREME_MAX_AUDIT_ITEMS,
        )
        self.maintenance()

    def _importance(self, user_text: str, assistant_text: str, risk_level: str) -> float:
        score = 0.34
        lowered = (user_text + " " + assistant_text).lower()
        if "remember" in lowered or "prefer" in lowered or "goal" in lowered:
            score += 0.22
        if risk_level in {"high", "critical"}:
            score += 0.30
        elif risk_level == "moderate":
            score += 0.16
        if any(x in lowered for x in ("medication", "dose", "overdose", "allergy", "relapse", "craving", "panic")):
            score += 0.15
        return max(0.0, min(1.0, score))

    def recall(self, prompt: str, *, mode: str = "General", top_k: int = 8) -> List[Dict[str, Any]]:
        clean_prompt = sanitize_text(prompt, max_chars=2200)
        now = time.time()
        rows = []
        for key, base_boost in (("pins", 0.20), ("ledger", 0.0)):
            source = self.raw.get(key)
            if not isinstance(source, list):
                continue
            for item in source:
                if not isinstance(item, dict):
                    continue
                text = sanitize_text(item.get("text") or item.get("assistant_hint") or "", max_chars=900)
                if not text:
                    continue
                recency_days = max(0.0, (now - float(item.get("last_seen_ts") or item.get("created_ts") or now)) / 86400.0)
                recency = 1.0 / (1.0 + recency_days / 21.0)
                sim = _extreme_jaccard(clean_prompt, text + " " + " ".join(item.get("keywords") or []))
                importance = float(item.get("importance") or item.get("confidence") or 0.48)
                mode_bonus = 0.05 if normalize_assistant_mode(item.get("mode") or mode) == normalize_assistant_mode(mode) else 0.0
                score = (sim * 0.48) + (importance * 0.30) + (recency * 0.16) + mode_bonus + base_boost
                rows.append(dict(item, _score=score, _source=key))
        rows.sort(key=lambda x: float(x.get("_score") or 0.0), reverse=True)
        selected: List[Dict[str, Any]] = []
        for row in rows:
            text = sanitize_text(row.get("text") or "", max_chars=700)
            if not text:
                continue
            if any(_extreme_jaccard(text, existing.get("text") or "") > 0.74 for existing in selected):
                continue
            selected.append(row)
    ...
    def context_block(self, prompt: str, *, mode: str = "General") -> str:
        recall = self.recall(prompt, mode=mode, top_k=9)
        pins = [x for x in recall if x.get("_source") == "pins"][:5]
        ledger = [x for x in recall if x.get("_source") == "ledger"][:6]
        lines = ["[extreme_memory_lab]", f"schema: {self.raw.get('schema')}" ]
        if pins:
            lines.append("pinned_or_stable_context:")
            for idx, item in enumerate(pins, 1):
                lines.append(f"{idx}. {sanitize_text(item.get('text'), max_chars=260)}")
        if ledger:
            lines.append("episodic_context:")
            for idx, item in enumerate(ledger, 1):
                lines.append(
                    f"{idx}. risk={sanitize_text(item.get('risk'), max_chars=24)} importance={float(item.get('importance') or 0.0):.2f}: "
                    f"{sanitize_text(item.get('text'), max_chars=240)}"
                )
        if not pins and not ledger:
            lines.append("No high-confidence extreme memory recall for this prompt.")
        lines.append("[/extreme_memory_lab]")
        return sanitize_text("\n".join(lines), max_chars=3800)

    def report(self) -> str:
        pins = [x for x in self.raw.get("pins", []) if isinstance(x, dict)]
        ledger = [x for x in self.raw.get("ledger", []) if isinstance(x, dict)]
        audit = [x for x in self.raw.get("audit", []) if isinstance(x, dict)]
        top = sorted(pins, key=lambda x: float(x.get("confidence") or 0.0), reverse=True)[:8]
        lines = [
            "Extreme Memory Lab",
            f"Pins: {len(pins)} | Episodes: {len(ledger)} | Audit events: {len(audit)}",
            "Top stable pins:",
        ]
        for idx, item in enumerate(top, 1):
            lines.append(f"{idx}. {sanitize_text(item.get('text'), max_chars=220)}")
        if not top:
            lines.append("No pinned user facts yet. Say: remember this: ...")
        return sanitize_text("\n".join(lines), max_chars=2400)

    def maintenance(self) -> None:
        now = time.time()
        if now - float(self.raw.get("last_maintenance_ts") or 0.0) < 60.0:
            return
        self.raw["last_maintenance_ts"] = now
        for key, limit in (("pins", EXTREME_MAX_PINNED_ITEMS), ("ledger", EXTREME_MAX_LEDGER_ITEMS), ("audit", EXTREME_MAX_AUDIT_ITEMS)):
            rows = [x for x in (self.raw.get(key) or []) if isinstance(x, dict)]
            if key == "ledger":
                rows.sort(key=lambda x: (float(x.get("importance") or 0.0), float(x.get("last_seen_ts") or x.get("created_ts") or 0.0)), reverse=True)
            elif key == "pins":
                rows.sort(key=lambda x: (float(x.get("confidence") or 0.0), float(x.get("seen_count") or 0), float(x.get("last_seen_ts") or 0.0)), reverse=True)
            else:
                rows = rows[-limit:]
            self.raw[key] = rows[:limit] if key != "audit" else rows[-limit:]


def _extreme_set_app_state(self: "DesktopMedSafeApp", state: str, detail: str = "") -> None:
    clean_state = sanitize_text(state, max_chars=32) or EXTREME_STATE_IDLE
    self.extreme_voice_state = clean_state
    try:
        if isinstance(getattr(self, "data_cache", None), dict):
            ext = self.data_cache.setdefault("assistant_extreme_state", {})
            if isinstance(ext, dict):
                ext["last_state"] = clean_state
                ext["last_state_detail"] = sanitize_text(detail, max_chars=200)
                ext["last_updated_ts"] = time.time()
    except Exception:
        pass
    if detail:
        try:
            self._voice_set_status(f"{clean_state}: {detail}")
        except Exception:
            pass


DesktopMedSafeApp._extreme_set_app_state = _extreme_set_app_state


EXTREME_ORIGINAL_BUILD_MEMORY_CONTEXT = build_in_house_vector_memory_context


def build_in_house_vector_memory_context(data: Dict[str, Any], prompt: str, mode: str) -> str:
    base = EXTREME_ORIGINAL_BUILD_MEMORY_CONTEXT(data, prompt, mode)
    try:
        extreme = ExtremeMemoryLab(data).context_block(prompt, mode=mode)
    except Exception as exc:
        extreme = f"[extreme_memory_lab]\nUnavailable: {sanitize_text(exc, max_chars=220)}\n[/extreme_memory_lab]"
    return sanitize_text(base + "\n\n" + extreme, max_chars=EXTREME_CONTEXT_MAX_CHARS)


EXTREME_ORIGINAL_RUN_ASSISTANT_REQUEST = run_assistant_request
...
def run_assistant_request(
    ...
    active_mode = normalize_assistant_mode(mode)
    shield = ExtremeSafetyShield(data, active_mode)
    risk = shield.classify(prompt)
    if shield.should_guardrail(prompt, risk):
        reply = shield.deterministic_guardrail_reply(prompt)
        if on_delta is not None:
            try:
                on_delta(reply)
            except Exception:
                pass
        return reply
    packet = shield.build_packet(prompt, risk)
    memory = ExtremeMemoryLab(data).context_block(prompt, mode=active_mode)
    enhanced_prompt = sanitize_text(prompt, max_chars=2600) + "\n\n" + packet + "\n\n" + memory
    result = EXTREME_ORIGINAL_RUN_ASSISTANT_REQUEST(key, data, enhanced_prompt, selected_med_id, settings, mode=active_mode, on_delta=on_delta)
    try:
        ExtremeMemoryLab(data).add_turn(prompt, result, mode=active_mode, risk=risk)
    except Exception:
        pass
    return result


EXTREME_ORIGINAL_VOICE_STORE_LATEST_TURN = DesktopMedSafeApp._voice_memory_store_latest_turn


def _extreme_voice_memory_store_latest_turn(self: "DesktopMedSafeApp") -> None:
    EXTREME_ORIGINAL_VOICE_STORE_LATEST_TURN(self)
    try:
        history = list(self.data_cache.get("assistant_history") or [])
        last_user = ""
        last_assistant = ""
        ...
        if not last_user or not last_assistant:
            return
        sig = _extreme_hash("extreme-store:" + last_mode + last_user + last_assistant)
        if sig == getattr(self, "extreme_last_stored_signature", ""):
            return
        risk = ExtremeSafetyShield(self.data_cache, last_mode).classify(last_user)
        ExtremeMemoryLab(self.data_cache).add_turn(last_user, last_assistant, mode=last_mode, risk=risk)
        self.extreme_last_stored_signature = sig
        self.persist_data_async()
    except Exception as exc:
        self._voice_set_status(f"Extreme memory store skipped: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp._voice_memory_store_latest_turn = _extreme_voice_memory_store_latest_turn


EXTREME_ORIGINAL_VOICE_SUBMIT_TEXT = DesktopMedSafeApp._voice_submit_text


def _extreme_voice_submit_text(self: "DesktopMedSafeApp", text: str) -> None:
    clean = sanitize_text(text, max_chars=1800)
    lowered = clean.lower().strip()
    if not clean:
        return EXTREME_ORIGINAL_VOICE_SUBMIT_TEXT(self, text)
    if lowered.startswith("remember this") or lowered.startswith("pin this memory"):
        memory_text = re.sub(r"^(remember this|pin this memory)[:\-\s]*", "", clean, flags=re.I).strip()
        if memory_text:
            ExtremeMemoryLab(self.data_cache).add_pin(memory_text, source="voice_command", confidence=0.94)
            self.persist_data_async()
            self._voice_set_status(f"Pinned memory: {sanitize_text(memory_text, max_chars=140)}")
            return
    if lowered in {"memory lab", "show memory lab", "open memory lab"}:
        self.on_extreme_memory_lab()
        return
    if lowered in {"forget last memory", "delete last memory", "remove last memory"}:
        msg = ExtremeMemoryLab(self.data_cache).forget_last()
        self.persist_data_async()
        self._voice_set_status(msg)
        return
    if lowered in {"ground me", "grounding protocol", "start grounding", "calm me down"}:
        self.on_extreme_grounding_protocol()
        return
    if lowered in {"safety plan", "make safety plan", "start safety plan"}:
        self.on_extreme_safety_plan()
        return
    if lowered in {"voice coach", "voice coach mode", "optimize voice"}:
        self.on_extreme_voice_coach()
        return
    self._extreme_set_app_state(EXTREME_STATE_LISTENING, "voice text captured")
    return EXTREME_ORIGINAL_VOICE_SUBMIT_TEXT(self, clean)


DesktopMedSafeApp._voice_submit_text = _extreme_voice_submit_text


EXTREME_ORIGINAL_ON_ASSISTANT_SEND = DesktopMedSafeApp.on_assistant_send


def _extreme_on_assistant_send(self: "DesktopMedSafeApp") -> None:
    self._extreme_set_app_state(EXTREME_STATE_THINKING, "Gemma request started")
    return EXTREME_ORIGINAL_ON_ASSISTANT_SEND(self)


DesktopMedSafeApp.on_assistant_send = _extreme_on_assistant_send


EXTREME_ORIGINAL_FINISH_STREAM = DesktopMedSafeApp._finish_assistant_live_stream


def _extreme_finish_assistant_live_stream(self: "DesktopMedSafeApp", request_id: int, final_text: str, mode: str) -> None:
    EXTREME_ORIGINAL_FINISH_STREAM(self, request_id, final_text, mode)
    if request_id == getattr(self, "assistant_request_id", 0):
        self._extreme_set_app_state(EXTREME_STATE_SPEAKING if bool(getattr(self, "voice_tts_enabled", True)) else EXTREME_STATE_IDLE, "assistant reply complete")


DesktopMedSafeApp._finish_assistant_live_stream = _extreme_finish_assistant_live_stream


EXTREME_ORIGINAL_BARK_SPEAK_ASYNC = BarkCppVoiceEngine.speak_async


def _extreme_bark_speak_async(self: "BarkCppVoiceEngine", text: str) -> None:
    try:
        self.app._extreme_set_app_state(EXTREME_STATE_SPEAKING, "Bark output queued")
    except Exception:
        pass
    return EXTREME_ORIGINAL_BARK_SPEAK_ASYNC(self, text)


BarkCppVoiceEngine.speak_async = _extreme_bark_speak_async


EXTREME_ORIGINAL_BARK_STOP = BarkCppVoiceEngine.stop


def _extreme_bark_stop(self: "BarkCppVoiceEngine") -> None:
    try:
        self.app._extreme_set_app_state(EXTREME_STATE_INTERRUPTED, "Bark stopped")
    except Exception:
        pass
    return EXTREME_ORIGINAL_BARK_STOP(self)


BarkCppVoiceEngine.stop = _extreme_bark_stop


def on_extreme_safety_plan(self: "DesktopMedSafeApp") -> None:
    prompt = (
        "Help me create a simple support/safety plan. Keep it practical: warning signs, grounding steps, who to contact, "
        "how to make the environment safer, and what to do if risk becomes immediate."
    )
    try:
        self.ids.assistant_input.text = prompt
        self.on_assistant_send()
    except Exception as exc:
        self._voice_set_status(f"Could not start safety plan: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_extreme_safety_plan = on_extreme_safety_plan


def on_extreme_grounding_protocol(self: "DesktopMedSafeApp") -> None:
    text = (
        "Grounding protocol: put both feet on the floor. Name five things you can see, four things you can feel, "
        "three things you can hear, two things you can smell, and one thing you can taste. Now unclench your jaw, "
        "lower your shoulders, and take three slow breaths. The next task is only to get through the next two minutes."
    )
    try:
        self.append_assistant_message("assistant", text, mode=getattr(self, "assistant_mode", "General"), persist=True)
    except TypeError:
        try:
            self.append_assistant_message("assistant", text)
        except Exception:
            pass
    self._voice_set_status("Grounding protocol ready.")
    try:
        self._voice_speak_reply_if_needed(text)
    except Exception:
        pass


DesktopMedSafeApp.on_extreme_grounding_protocol = on_extreme_grounding_protocol


def on_extreme_memory_lab(self: "DesktopMedSafeApp") -> None:
    try:
        text = ExtremeMemoryLab(self.data_cache).report()
        self._voice_set_status(text)
        box = self.ids.get("assistant_context_window_report")
        if box is not None:
            current = sanitize_text(getattr(box, "text", ""), max_chars=6200)
            box.text = sanitize_text(current + "\n\n" + text, max_chars=9000)
    except Exception as exc:
        self._voice_set_status(f"Extreme memory lab unavailable: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_extreme_memory_lab = on_extreme_memory_lab


def on_extreme_voice_coach(self: "DesktopMedSafeApp") -> None:
    try:
        state = self.data_cache.setdefault("assistant_extreme_state", {})
        if not isinstance(state, dict):
            state = {}
            self.data_cache["assistant_extreme_state"] = state
        current = sanitize_text(state.get("voice_persona") or "warm concise", max_chars=80)
        next_value = "warm concise" if current != "warm concise" else "calm reflective"
        state["voice_persona"] = next_value
        self.persist_data_async()
        self._voice_set_status(f"Voice coach persona set to {next_value}.")
    except Exception as exc:
        self._voice_set_status(f"Voice coach unavailable: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_extreme_voice_coach = on_extreme_voice_coach


def on_extreme_trim_memory(self: "DesktopMedSafeApp") -> None:
    try:
        lab = ExtremeMemoryLab(self.data_cache)
        lab.raw["last_maintenance_ts"] = 0.0
        lab.maintenance()
        self.persist_data_async()
        self._voice_set_status("Extreme memory maintenance complete.")
    except Exception as exc:
        self._voice_set_status(f"Memory maintenance failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_extreme_trim_memory = on_extreme_trim_memory


EXTREME_ORIGINAL_VOICE_DIAGNOSTICS = DesktopMedSafeApp._ultra_voice_diagnostics_text


def _extreme_voice_diagnostics_text(self: "DesktopMedSafeApp") -> str:
    base = EXTREME_ORIGINAL_VOICE_DIAGNOSTICS(self)
    try:
        shield_state = self.data_cache.get("assistant_extreme_state") if isinstance(self.data_cache.get("assistant_extreme_state"), dict) else {}
        mem = self.data_cache.get("assistant_extreme_memory") if isinstance(self.data_cache.get("assistant_extreme_memory"), dict) else {}
        extra = "\n".join(
            [
                f"Extreme patch: {EXTREME_PATCH_VERSION}",
                f"State machine: {getattr(self, 'extreme_voice_state', EXTREME_STATE_IDLE)} | stored={shield_state.get('last_state', EXTREME_STATE_IDLE)} | risk={shield_state.get('last_risk', 'low')}",
                f"Extreme memory: pins={len(mem.get('pins') or [])} ledger={len(mem.get('ledger') or [])} audit={len(mem.get('audit') or [])}",
                f"Voice persona: {shield_state.get('voice_persona', 'warm concise')}",
            ]
        )
        return sanitize_text(base + "\n" + extra, max_chars=4200)
    except Exception as exc:
        return base + f"\nExtreme diagnostics unavailable: {sanitize_text(exc, max_chars=180)}"


DesktopMedSafeApp._ultra_voice_diagnostics_text = _extreme_voice_diagnostics_text


EXTREME_ORIGINAL_DESKTOP_INIT = DesktopMedSafeApp.__init__


def _extreme_desktop_init(self: "DesktopMedSafeApp", *args: Any, **kwargs: Any) -> None:
    EXTREME_ORIGINAL_DESKTOP_INIT(self, *args, **kwargs)
    self.extreme_voice_state = EXTREME_STATE_IDLE
    self.extreme_last_stored_signature = ""


DesktopMedSafeApp.__init__ = _extreme_desktop_init


EXTREME_ORIGINAL_BUILD_ASSISTANT_TAB = DesktopMedSafeApp._build_assistant_tab


def _extreme_build_assistant_tab(self: "DesktopMedSafeApp", tab: Any) -> None:
    EXTREME_ORIGINAL_BUILD_ASSISTANT_TAB(self, tab)
    if ctk is None:
        return
    voice_frame = self.ids.get("voice_frame")
    if voice_frame is None:
    ...
    try:
        voice_frame.grid_columnconfigure((0, 1, 2, 3), weight=1)
        safety = self._button(voice_frame, text="Safety Plan", command=self.on_extreme_safety_plan, tone="neutral")
        safety.grid(row=5, column=0, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_extreme_safety_button"] = safety
        memory = self._button(voice_frame, text="Memory Lab", command=self.on_extreme_memory_lab, tone="neutral")
        memory.grid(row=5, column=1, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_extreme_memory_button"] = memory
        ground = self._button(voice_frame, text="Ground Me", command=self.on_extreme_grounding_protocol, tone="neutral")
        ground.grid(row=5, column=2, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_extreme_ground_button"] = ground
        coach = self._button(voice_frame, text="Voice Coach", command=self.on_extreme_voice_coach, tone="neutral")
        coach.grid(row=5, column=3, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_extreme_coach_button"] = coach
    except Exception:
        pass


DesktopMedSafeApp._build_assistant_tab = _extreme_build_assistant_tab
# --- end extreme voice/memory/therapy supervision layer -----------------------

# --- apex production hardening voice/memory/therapist layer -------------------
# This v6 layer remains single-file and dependency-light. It adds a supervisory
# command bus, local session ledger, crash-safe memory snapshots, Bark/Gemma
# health checks, and stricter therapist-support boundaries.

APEX_PATCH_VERSION = "2026-05-08.apex.production-voice-memory.v6"
APEX_MAX_EVENTS = 260
APEX_MAX_SNAPSHOTS = 18
APEX_MAX_SESSION_NOTES = 180
APEX_MAX_COMMAND_HISTORY = 120
APEX_VOICE_STATE_COMMAND = "command"
APEX_VOICE_STATE_HEALTH = "health"
APEX_VOICE_STATE_PRIVACY = "privacy"


def _apex_now_iso() -> str:
    try:
...
def _apex_hash(value: Any, *, size: int = 20) -> str:
    try:
        clean = sanitize_text(value, max_chars=50000)
        return hashlib.sha256(clean.encode("utf-8", errors="ignore")).hexdigest()[:size]
    except Exception:
        return uuid.uuid4().hex[:size]


def _apex_norm(value: Any, *, max_chars: int = 3000) -> str:
    return re.sub(r"\s+", " ", sanitize_text(value, max_chars=max_chars)).strip()


def _apex_lower(value: Any, *, max_chars: int = 3000) -> str:
    return _apex_norm(value, max_chars=max_chars).lower()


def _apex_take(rows: Any, limit: int) -> List[Any]:
    if not isinstance(rows, list):
        return []
    return rows[-max(0, int(limit)):]


def _apex_append_limited(container: Dict[str, Any], key: str, item: Dict[str, Any], limit: int) -> None:
    rows = container.setdefault(key, [])
    if not isinstance(rows, list):
        rows = []
        container[key] = rows
    rows.append(item)
    container[key] = rows[-max(1, int(limit)):]


def _apex_emergency_reply(reason: str = "possible immediate risk") -> str:
    return (
        "I’m going to treat this as urgent because it may involve immediate safety risk. "
        "I’m not an emergency service or clinician. Please contact local emergency services, a crisis line, "
        "or a trusted person who can be physically with you now.\n\n"
        "Right now: move away from anything you could use to harm yourself or take more of; sit somewhere visible or public if you can; "
        "send one direct message to someone safe: “I’m not safe alone right now. Please call me or come over.”\n\n"
        f"Detected concern: {sanitize_text(reason, max_chars=180)}."
    )


class ApexSafetyRouter:
    """A deterministic gate before the LLM. This is not a diagnosis engine."""

    CRITICAL_PATTERNS = (
        ("self-harm or suicide intent", r"\b(kill myself|end my life|suicide|suicidal|i want to die|can't stay safe|cannot stay safe)\b"),
        ("possible overdose or poisoning", r"\b(overdose|od\b|took too much|poison|can't breathe|cannot breathe|blue lips|passed out|unconscious|seizure|chest pain)\b"),
        ("violence or immediate danger", r"\b(hurt someone|kill someone|weapon|gun|knife|violent|not safe at home|being attacked)\b"),
        ("dangerous withdrawal or acute medical risk", r"\b(delirium tremens|withdrawal seizure|severe withdrawal|hallucinating from withdrawal|medical emergency)\b"),
    )
    HIGH_PATTERNS = (
        ("acute self-harm signal", r"\b(self harm|cut myself|harm myself|might do something|unsafe alone|not safe alone)\b"),
        ("relapse/craving escalation", r"\b(about to relapse|going to use|dealer|craving is unbearable|can't stop myself|cannot stop myself)\b"),
        ("severe panic or dissociation", r"\b(panic attack|dissociating|flashback|spiraling|can't cope|cannot cope)\b"),
    )
    MODERATE_PATTERNS = (
        ("distress", r"\b(anxious|depressed|hopeless|overwhelmed|triggered|ashamed|lonely|panic|craving)\b"),
        ("medication uncertainty", r"\b(missed dose|double dose|side effect|interaction|early dose|withdrawal|withdrawals)\b"),
    )

    def __init__(self, data: Dict[str, Any], mode: str = "General") -> None:
        self.data = data
        self.mode = normalize_assistant_mode(mode)
        state = data.setdefault("assistant_apex_state", {})
        if not isinstance(state, dict):
            state = {}
            data["assistant_apex_state"] = state
        state.setdefault("schema", "ApexSafetyRouter/v6")
        state.setdefault("last_level", "low")
        state.setdefault("last_reason", "none")
        state.setdefault("turns_seen", 0)
        self.state = state

    def classify(self, text: Any) -> Dict[str, Any]:
        clean = sanitize_text(text, max_chars=5000)
        lowered = clean.lower()
        level = "low"
        reason = "general support"
        matched: List[str] = []
        for label, pattern in self.CRITICAL_PATTERNS:
            if re.search(pattern, lowered):
                level = "critical"
                reason = label
                matched.append(label)
        if level != "critical":
            for label, pattern in self.HIGH_PATTERNS:
                if re.search(pattern, lowered):
                    level = "high"
                    reason = label
                    matched.append(label)
        if level == "low":
            for label, pattern in self.MODERATE_PATTERNS:
                if re.search(pattern, lowered):
                    level = "moderate"
                    reason = label
                    matched.append(label)
        ...
        self.state["turns_seen"] = int(self.state.get("turns_seen") or 0) + 1
        return {"level": level, "reason": reason, "matched": matched[:8], "intents": intents, "hash": _apex_hash(clean)}

    def _intents(self, lowered: str) -> List[str]:
        intents: List[str] = []
        tests = (
            ("therapy", ("feel", "anxiety", "panic", "sad", "angry", "therapy", "trauma", "triggered", "overwhelmed")),
            ("recovery", ("craving", "relapse", "sober", "sobriety", "clean", "use again", "recovery")),
            ("medication", ("dose", "pill", "med", "mg", "pharmacy", "side effect", "interaction")),
            ("memory", ("remember", "memory", "forget", "pin this", "recall")),
            ("voice", ("voice", "listen", "talk", "bark", "microphone", "interrupt")),
            ("planning", ("plan", "schedule", "routine", "next step", "checklist")),
        )
    ...
    def model_packet(self, prompt: Any, classification: Optional[Dict[str, Any]] = None) -> str:
        item = classification or self.classify(prompt)
        level = sanitize_text(item.get("level") or "low", max_chars=24)
        reason = sanitize_text(item.get("reason") or "none", max_chars=120)
        intents = ", ".join(sanitize_text(x, max_chars=40) for x in list(item.get("intents") or [])[:8])
        if level == "critical":
            priority = "do not roleplay; direct user to immediate live help; keep answer short"
        elif level == "high":
            priority = "lead with safety, reduce time horizon to minutes, encourage live support"
        elif level == "moderate":
            priority = "validate, stabilize, ask one clarifying question only if essential, give one concrete next action"
        else:
            priority = "answer directly; keep voice output usable; distinguish memory from verified fact"
        return sanitize_text(
            "\n".join(
                [
                    "[apex_runtime_packet]",
                    f"version: {APEX_PATCH_VERSION}",
                    f"mode: {self.mode}",
                    f"risk_level: {level}",
                    f"risk_reason: {reason}",
                    f"intents: {intents}",
                    f"response_priority: {priority}",
                    "boundaries: not a licensed therapist, prescriber, pharmacist, crisis line, or emergency service; do not diagnose; do not change medication instructions; escalate emergencies to live help",
                    "memory_policy: use memories as uncertain context only; if memory conflicts with current user message, trust current user message and say so",
                    "voice_policy: if likely spoken aloud, use short paragraphs, no giant lists unless asked, no markdown tables",
                    "[/apex_runtime_packet]",
                ]
            ),
            max_chars=2600,
...
class ApexSessionLedger:
    """Transcript-level session memory stored inside the existing encrypted app data."""

    def __init__(self, data: Dict[str, Any]) -> None:
        raw = data.setdefault("assistant_apex_session", {})
        if not isinstance(raw, dict):
            raw = {}
            data["assistant_apex_session"] = raw
        raw.setdefault("schema", "ApexSessionLedger/v6")
        raw.setdefault("events", [])
        raw.setdefault("notes", [])
        raw.setdefault("command_history", [])
        raw.setdefault("session_id", uuid.uuid4().hex[:12])
        raw.setdefault("started_ts", time.time())
        self.raw = raw

    def event(self, kind: str, detail: Any, *, meta: Optional[Dict[str, Any]] = None) -> None:
        item = {
            "id": uuid.uuid4().hex[:12],
            "ts": time.time(),
            "at": _apex_now_iso(),
            "kind": sanitize_text(kind, max_chars=48),
            "detail": sanitize_text(detail, max_chars=1200),
            "meta": meta or {},
        }
        _apex_append_limited(self.raw, "events", item, APEX_MAX_EVENTS)

    def command(self, text: Any, action: str) -> None:
        item = {"ts": time.time(), "at": _apex_now_iso(), "text": sanitize_text(text, max_chars=260), "action": sanitize_text(action, max_chars=80)}
        _apex_append_limited(self.raw, "command_history", item, APEX_MAX_COMMAND_HISTORY)
        self.event("voice_command", action, meta={"text_hash": _apex_hash(text, size=12)})

    def observe_turn(self, user_text: Any, assistant_text: Any, *, mode: str = "General", risk: Optional[Dict[str, Any]] = None) -> None:
        user = sanitize_text(user_text, max_chars=2400)
        assistant = sanitize_text(assistant_text, max_chars=3200)
        if not user or not assistant:
            return
        signature = _apex_hash(f"{mode}\n{user}\n{assistant}")
        if signature == self.raw.get("last_turn_signature"):
            return
        self.raw["last_turn_signature"] = signature
        self.event(
            "assistant_turn",
            _apex_norm(user, max_chars=520),
            meta={
                "mode": normalize_assistant_mode(mode),
                "assistant_preview": _apex_norm(assistant, max_chars=520),
                "risk": risk or {},
                "signature": signature,
            },
        )
        note = self._note_from_turn(user, assistant, mode, risk or {})
        if note:
            _apex_append_limited(self.raw, "notes", note, APEX_MAX_SESSION_NOTES)

    def _note_from_turn(self, user: str, assistant: str, mode: str, risk: Dict[str, Any]) -> Dict[str, Any]:
        lowered = user.lower()
        tags: List[str] = []
        for tag, tokens in {
            "preference": ("i prefer", "don't", "do not", "please", "i like"),
            "recovery": ("craving", "relapse", "sober", "recovery", "clean"),
            "therapy": ("anxious", "panic", "sad", "angry", "triggered", "overwhelmed"),
            "medication": ("dose", "mg", "pill", "med", "side effect", "interaction"),
            "voice": ("voice", "listen", "interrupt", "bark", "talk"),
        }.items():
            if any(token in lowered for token in tokens):
                tags.append(tag)
        importance = 0.25 + (0.3 if tags else 0.0)
        if (risk.get("level") or "low") in {"high", "critical"}:
            importance += 0.35
        ...
        return {
            ...
            "summary": _apex_norm(user, max_chars=360),
            "assistant_summary": _apex_norm(assistant, max_chars=360),
            "risk_level": sanitize_text(risk.get("level") or "low", max_chars=24),
        }

    def capsule(self, *, max_chars: int = 2400) -> str:
        events = [x for x in self.raw.get("events", []) if isinstance(x, dict)]
        notes = [x for x in self.raw.get("notes", []) if isinstance(x, dict)]
        commands = [x for x in self.raw.get("command_history", []) if isinstance(x, dict)]
        top_notes = sorted(notes, key=lambda x: (float(x.get("importance") or 0.0), float(x.get("ts") or 0.0)), reverse=True)[:10]
        lines = [
            "Apex Session Capsule",
            f"session_id: {self.raw.get('session_id', 'unknown')}",
            f"started: {_apex_now_iso() if not self.raw.get('started_ts') else time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(float(self.raw.get('started_ts') or time.time())))}",
            f"events: {len(events)} | notes: {len(notes)} | commands: {len(commands)}",
            "",
            "Top notes:",
        ]
        if not top_notes:
            lines.append("- No high-value session notes yet.")
        for note in top_notes:
            tag_text = ",".join(note.get("tags") or []) or "general"
            lines.append(f"- [{tag_text}; risk={note.get('risk_level', 'low')}] {sanitize_text(note.get('summary'), max_chars=220)}")
        if commands:
            lines.append("")
            lines.append("Recent voice commands:")
            for cmd in commands[-8:]:
                lines.append(f"- {sanitize_text(cmd.get('action'), max_chars=80)}")
        return sanitize_text("\n".join(lines), max_chars=max_chars)


class ApexSnapshotManager:
    """Crash-safe snapshots of the memory/control state inside the encrypted vault."""

    SNAPSHOT_KEYS = (
        "assistant_context_memory",
        "assistant_vector_memory",
        "assistant_advanced_memory",
        "assistant_ultra_memory",
        "assistant_extreme_memory",
        "assistant_apex_session",
        "assistant_apex_state",
    )

    def __init__(self, data: Dict[str, Any]) -> None:
        raw = data.setdefault("assistant_apex_snapshots", {})
        if not isinstance(raw, dict):
            raw = {}
            data["assistant_apex_snapshots"] = raw
        raw.setdefault("schema", "ApexSnapshotManager/v6")
    ...
    def create(self, reason: str = "manual") -> Dict[str, Any]:
        ...
        for key in self.SNAPSHOT_KEYS:
            ...
            if isinstance(value, (dict, list, str, int, float, bool)) or value is None:
                try:
                    # json round-trip keeps the snapshot value independent and serializable.
                    payload[key] = json.loads(json.dumps(value, default=str))
                except Exception:
                    payload[key] = sanitize_text(value, max_chars=1000)
        recent_history = []
        try:
            recent_history = [x for x in list(self.data.get("assistant_history") or [])[-12:] if isinstance(x, dict)]
        except Exception:
            recent_history = []
        payload["assistant_history_tail"] = recent_history
        encoded = json.dumps(payload, sort_keys=True, default=str)
        snap = {
            "id": uuid.uuid4().hex[:14],
            "ts": time.time(),
            "at": _apex_now_iso(),
            "reason": sanitize_text(reason, max_chars=120),
            "hash": _apex_hash(encoded, size=24),
            "bytes": len(encoded.encode("utf-8", errors="ignore")),
            "payload": payload,
        }
        rows = self.raw.setdefault("snapshots", [])
        if not isinstance(rows, list):
            rows = []
        rows.append(snap)
        self.raw["snapshots"] = rows[-APEX_MAX_SNAPSHOTS:]
        return snap

    def report(self) -> str:
        rows = [x for x in self.raw.get("snapshots", []) if isinstance(x, dict)]
        lines = ["Apex Memory Snapshots", f"count: {len(rows)}/{APEX_MAX_SNAPSHOTS}"]
        if not rows:
            lines.append("No snapshots yet.")
        for snap in rows[-8:]:
            lines.append(f"- {snap.get('at', '?')} id={snap.get('id', '?')} reason={snap.get('reason', '?')} hash={snap.get('hash', '?')} size={human_size(int(snap.get('bytes') or 0))}")
        return sanitize_text("\n".join(lines), max_chars=2200)


class ApexHealthMonitor:
    """Local readiness checks without requiring network access."""

    OPTIONAL_MODULES = ("customtkinter", "sounddevice", "soundfile", "speech_recognition", "cryptography")

    def __init__(self, app: "DesktopMedSafeApp") -> None:
        self.app = app

    def module_status(self) -> List[str]:
        rows: List[str] = []
        for name in self.OPTIONAL_MODULES:
            try:
                ...
                rows.append(f"{name}: missing ({sanitize_text(exc, max_chars=80)})")
        return rows

    def bark_status(self) -> List[str]:
        rows: List[str] = []
        bark = getattr(self.app, "bark_voice", None)
        if bark is None:
            return ["Bark engine: not initialized"]
        try:
            binary = bark.default_binary_path()
            model = bark.default_model_path()
            rows.append(f"Bark binary: {'ok' if binary.exists() else 'missing'} -> {binary}")
            rows.append(f"Bark q8_0 model: {'ok' if model.exists() else 'missing'} -> {model}")
            rows.append(f"Bark playing: {bool(getattr(bark, 'playing', False))}")
        except Exception as exc:
            rows.append(f"Bark status failed: {sanitize_text(exc, max_chars=160)}")
        return rows

    def gemma_status(self) -> List[str]:
        rows: List[str] = []
        try:
            paths = require_paths()
            rows.append(f"Gemma model file: {'ok' if paths.plain_model_path.exists() else 'missing'} -> {paths.plain_model_path}")
            rows.append(f"Gemma encrypted file: {'ok' if paths.encrypted_model_path.exists() else 'missing'} -> {paths.encrypted_model_path}")
            settings = getattr(self.app, "settings", {}) if isinstance(getattr(self.app, "settings", {}), dict) else {}
            rows.append(f"Backend setting: {settings.get('inference_backend', 'Auto')} | auto={settings.get('auto_selected_inference_backend', '')}")
        except Exception as exc:
            rows.append(f"Gemma status failed: {sanitize_text(exc, max_chars=160)}")
        return rows

    def storage_status(self) -> List[str]:
        rows: List[str] = []
        try:
            paths = require_paths()
            usage = shutil.disk_usage(paths.root)
            rows.append(f"Storage root: {paths.root}")
            rows.append(f"Free space: {human_size(int(usage.free))} / {human_size(int(usage.total))}")
            rows.append(f"Vault: {'ok' if paths.vault_path.exists() else 'not created yet'}")
        except Exception as exc:
            rows.append(f"Storage status failed: {sanitize_text(exc, max_chars=160)}")
        return rows

    def report(self) -> str:
        lines = [
            f"Apex Health Check ({APEX_PATCH_VERSION})",
            "",
            "Modules:",
            *[f"- {row}" for row in self.module_status()],
            "",
            "Gemma:",
            *[f"- {row}" for row in self.gemma_status()],
            "",
            "Bark.cpp:",
            *[f"- {row}" for row in self.bark_status()],
            "",
            "Storage:",
            *[f"- {row}" for row in self.storage_status()],
        ]
        return sanitize_text("\n".join(lines), max_chars=5000)


class ApexVoiceCommandBus:
    """Single entry point for local voice commands before text goes to Gemma."""

    def __init__(self, app: "DesktopMedSafeApp") -> None:
        self.app = app

    def handle(self, text: Any) -> bool:
        clean = _apex_norm(text, max_chars=1200)
        lowered = clean.lower()
        if not lowered:
            return True
        # Let ordinary user requests through unless they look like local commands.
        command_like = lowered.startswith(("apex ", "system ", "remember this", "pin this memory", "forget last", "health check", "voice ", "privacy ", "snapshot", "session capsule")) or lowered in {
            "health check", "run health check", "bark health", "gemma health", "system report", "snapshot memory", "memory snapshot",
            "session capsule", "show session capsule", "privacy lockdown", "privacy mode", "mute voice", "unmute voice",
            "stop listener", "start listener", "listener sleep", "listener wake", "crisis drill", "reset voice state",
        }
        if not command_like:
            return False
        action = "unhandled"
        try:
            if lowered.startswith("apex "):
                ...
                clean = clean[5:].strip()
            if lowered in {"health check", "run health check", "system report", "bark health", "gemma health"}:
                action = "health_check"
                self.app.on_apex_health_check()
            elif lowered in {"snapshot memory", "memory snapshot", "snapshot"}:
                action = "snapshot_memory"
                self.app.on_apex_memory_snapshot()
            elif lowered in {"session capsule", "show session capsule"}:
                action = "session_capsule"
                self.app.on_apex_session_capsule()
            elif lowered in {"privacy lockdown", "privacy mode"}:
                action = "privacy_lockdown"
                self.app.on_apex_privacy_lockdown()
            elif lowered in {"mute voice", "voice mute"}:
                action = "voice_muted"
                setattr(self.app, "voice_tts_enabled", False)
                self.app._voice_set_status("Voice output muted.")
            elif lowered in {"unmute voice", "voice unmute"}:
                action = "voice_unmuted"
                setattr(self.app, "voice_tts_enabled", True)
                self.app._voice_set_status("Voice output unmuted.")
            elif lowered in {"listener sleep", "stop listener"}:
                action = "listener_sleep"
                setattr(self.app, "advanced_voice_listener_awake", False)
                self.app._voice_set_status("Listener asleep. Say or type listener wake to resume.")
            elif lowered in {"listener wake", "start listener"}:
                action = "listener_wake"
                setattr(self.app, "advanced_voice_listener_awake", True)
                self.app._voice_set_status("Listener awake.")
            elif lowered in {"crisis drill", "safety drill"}:
                action = "crisis_drill"
                self.app.on_apex_crisis_drill()
            elif lowered in {"reset voice state", "reset state"}:
                action = "reset_voice_state"
                try:
                    self.app._extreme_set_app_state(EXTREME_STATE_IDLE, "reset by command")
                except Exception:
                    pass
                self.app._voice_set_status("Voice state reset.")
            elif lowered.startswith("voice persona ") or lowered.startswith("set voice persona "):
                action = "voice_persona"
                persona = re.sub(r"^(set\s+)?voice\s+persona\s*", "", clean, flags=re.I).strip()
                self.app.on_apex_set_voice_persona(persona)
            elif lowered.startswith("remember this") or lowered.startswith("pin this memory"):
                action = "pin_memory"
                memory_text = re.sub(r"^(remember this|pin this memory)[:\-\s]*", "", clean, flags=re.I).strip()
                if memory_text:
                    self.app.on_apex_pin_memory(memory_text)
                else:
                    self.app._voice_set_status("Say: remember this, followed by the memory.")
            else:
                return False
            ApexSessionLedger(self.app.data_cache).command(clean, action)
            try:
                self.app.persist_data_async()
            except Exception:
                pass
            return True
        except Exception as exc:
            try:
                self.app._voice_set_status(f"Apex command failed: {sanitize_text(exc, max_chars=180)}")
            except Exception:
                pass
            return True


APEX_ORIGINAL_RUN_ASSISTANT_REQUEST = run_assistant_request
...
def run_assistant_request(
    ...
    active_mode = normalize_assistant_mode(mode)
    router = ApexSafetyRouter(data, active_mode)
    classification = router.classify(prompt)
    if classification.get("level") == "critical":
        reply = _apex_emergency_reply(str(classification.get("reason") or "critical risk"))
        if on_delta is not None:
            try:
                on_delta(reply)
            except Exception:
                pass
        try:
            ApexSessionLedger(data).event("critical_guardrail", reply, meta=classification)
        except Exception:
            pass
        return reply
    packet = router.model_packet(prompt, classification)
    try:
        snapshot = ApexSnapshotManager(data).create(reason=f"pre_turn:{classification.get('level', 'low')}")
        snapshot_note = f"[apex_snapshot]\nid: {snapshot.get('id')} hash: {snapshot.get('hash')} reason: {snapshot.get('reason')}\n[/apex_snapshot]"
    except Exception as exc:
        snapshot_note = f"[apex_snapshot]\nunavailable: {sanitize_text(exc, max_chars=180)}\n[/apex_snapshot]"
    try:
        capsule = ApexSessionLedger(data).capsule(max_chars=1600)
    except Exception:
        capsule = ""
    enhanced_prompt = "\n\n".join(
        x
        for x in (
            sanitize_text(prompt, max_chars=3000),
            packet,
            snapshot_note,
            "[apex_session_capsule]\n" + sanitize_text(capsule, max_chars=1600) + "\n[/apex_session_capsule]" if capsule else "",
        )
        if x
    )
    result = APEX_ORIGINAL_RUN_ASSISTANT_REQUEST(key, data, enhanced_prompt, selected_med_id, settings, mode=active_mode, on_delta=on_delta)
    try:
        ApexSessionLedger(data).observe_turn(prompt, result, mode=active_mode, risk=classification)
    except Exception:
        pass
    return result


APEX_ORIGINAL_VOICE_SUBMIT_TEXT = DesktopMedSafeApp._voice_submit_text


def _apex_voice_submit_text(self: "DesktopMedSafeApp", text: str) -> None:
    clean = _apex_norm(text, max_chars=1600)
    if not clean:
        return
    try:
        if ApexVoiceCommandBus(self).handle(clean):
            return
    except Exception as exc:
        try:
            self._voice_set_status(f"Apex command router skipped: {sanitize_text(exc, max_chars=180)}")
        except Exception:
            pass
    return APEX_ORIGINAL_VOICE_SUBMIT_TEXT(self, clean)


DesktopMedSafeApp._voice_submit_text = _apex_voice_submit_text


APEX_ORIGINAL_FINISH_STREAM = DesktopMedSafeApp._finish_assistant_live_stream


def _apex_finish_assistant_live_stream(self: "DesktopMedSafeApp", request_id: int, final_text: str, mode: str) -> None:
    APEX_ORIGINAL_FINISH_STREAM(self, request_id, final_text, mode)
    if request_id != getattr(self, "assistant_request_id", 0):
        return
    try:
        history = list(self.data_cache.get("assistant_history") or [])
        ...
        for item in reversed(history):
            if not isinstance(item, dict):
                continue
            if sanitize_text(item.get("role") or "", max_chars=24).lower() == "user":
                last_user = sanitize_text(item.get("content") or "", max_chars=3000)
                break
        risk = ApexSafetyRouter(self.data_cache, mode).classify(last_user) if last_user else {"level": "low", "reason": "unknown"}
        ApexSessionLedger(self.data_cache).observe_turn(last_user, final_text, mode=mode, risk=risk)
        if risk.get("level") in {"high", "critical"}:
            ApexSnapshotManager(self.data_cache).create(reason=f"post_turn:{risk.get('level')}")
        self.persist_data_async()
    except Exception as exc:
        try:
            self._voice_set_status(f"Apex ledger skipped: {sanitize_text(exc, max_chars=180)}")
        except Exception:
            pass


DesktopMedSafeApp._finish_assistant_live_stream = _apex_finish_assistant_live_stream


APEX_ORIGINAL_VOICE_DIAGNOSTICS = DesktopMedSafeApp._ultra_voice_diagnostics_text


def _apex_voice_diagnostics_text(self: "DesktopMedSafeApp") -> str:
    base = APEX_ORIGINAL_VOICE_DIAGNOSTICS(self)
    try:
        state = self.data_cache.get("assistant_apex_state") if isinstance(self.data_cache.get("assistant_apex_state"), dict) else {}
        session = self.data_cache.get("assistant_apex_session") if isinstance(self.data_cache.get("assistant_apex_session"), dict) else {}
        snapshots = self.data_cache.get("assistant_apex_snapshots") if isinstance(self.data_cache.get("assistant_apex_snapshots"), dict) else {}
        extra = "\n".join(
            [
                f"Apex patch: {APEX_PATCH_VERSION}",
                f"Apex risk: {state.get('last_level', 'low')} | reason={state.get('last_reason', 'none')} | turns={state.get('turns_seen', 0)}",
                f"Apex session: events={len(session.get('events') or [])} notes={len(session.get('notes') or [])} commands={len(session.get('command_history') or [])}",
                f"Apex snapshots: {len(snapshots.get('snapshots') or [])}/{APEX_MAX_SNAPSHOTS}",
            ]
        )
        return sanitize_text(base + "\n" + extra, max_chars=6200)
    except Exception as exc:
        return sanitize_text(base + f"\nApex diagnostics unavailable: {sanitize_text(exc, max_chars=180)}", max_chars=6200)


DesktopMedSafeApp._ultra_voice_diagnostics_text = _apex_voice_diagnostics_text


def on_apex_health_check(self: "DesktopMedSafeApp") -> None:
    try:
        text = ApexHealthMonitor(self).report()
        ApexSessionLedger(self.data_cache).event("health_check", text)
        self._voice_set_status(text)
        box = self.ids.get("assistant_context_window_report")
        if box is not None:
            current = sanitize_text(getattr(box, "text", ""), max_chars=6000)
            box.text = sanitize_text(current + "\n\n" + text, max_chars=10000)
        self.persist_data_async()
    except Exception as exc:
        self._voice_set_status(f"Health check failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_apex_health_check = on_apex_health_check


def on_apex_memory_snapshot(self: "DesktopMedSafeApp") -> None:
    try:
        snap = ApexSnapshotManager(self.data_cache).create(reason="manual_ui_or_voice")
        text = f"Apex memory snapshot saved: id={snap.get('id')} hash={snap.get('hash')} size={human_size(int(snap.get('bytes') or 0))}"
        ApexSessionLedger(self.data_cache).event("memory_snapshot", text, meta={"snapshot_id": snap.get("id")})
        self.persist_data_async()
        self._voice_set_status(text)
    except Exception as exc:
        self._voice_set_status(f"Snapshot failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_apex_memory_snapshot = on_apex_memory_snapshot


def on_apex_session_capsule(self: "DesktopMedSafeApp") -> None:
    try:
        text = ApexSessionLedger(self.data_cache).capsule(max_chars=3200)
        self._voice_set_status(text)
        box = self.ids.get("assistant_context_window_report")
        if box is not None:
            current = sanitize_text(getattr(box, "text", ""), max_chars=6500)
            box.text = sanitize_text(current + "\n\n" + text, max_chars=11000)
    except Exception as exc:
        self._voice_set_status(f"Session capsule failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_apex_session_capsule = on_apex_session_capsule


def on_apex_privacy_lockdown(self: "DesktopMedSafeApp") -> None:
    try:
        setattr(self, "voice_tts_enabled", False)
        setattr(self, "voice_listener_enabled", False)
        setattr(self, "advanced_voice_listener_awake", False)
        bark = getattr(self, "bark_voice", None)
        if bark is not None:
            try:
                bark.stop()
        ...
        try:
            ...
            pass
        ApexSessionLedger(self.data_cache).event("privacy_lockdown", "Muted voice output, disabled listener, stopped Bark/interruptor.")
        ApexSnapshotManager(self.data_cache).create(reason="privacy_lockdown")
        self.persist_data_async()
        self._voice_set_status("Privacy lockdown: listener off, voice output muted, Bark stopped, memory snapshot saved.")
    except Exception as exc:
        self._voice_set_status(f"Privacy lockdown failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_apex_privacy_lockdown = on_apex_privacy_lockdown


def on_apex_crisis_drill(self: "DesktopMedSafeApp") -> None:
    text = (
        "Crisis drill: choose one safe place, one safe person, and one friction step. "
        "Safe place: move there now or name it. Safe person: prepare a direct message. "
        "Friction step: put distance between you and anything risky. If danger is immediate, use live emergency support."
    )
    try:
        self.append_assistant_message("assistant", text, mode=getattr(self, "assistant_mode", "General"), persist=True)
    except TypeError:
        try:
            ...
            pass
    ApexSessionLedger(self.data_cache).event("crisis_drill", text)
    self.persist_data_async()
    self._voice_set_status("Crisis drill ready.")
    try:
        self._voice_speak_reply_if_needed(text)
    except Exception:
        pass
...
DesktopMedSafeApp.on_apex_crisis_drill = on_apex_crisis_drill


def on_apex_set_voice_persona(self: "DesktopMedSafeApp", persona: Any) -> None:
    clean = sanitize_text(persona, max_chars=80) or "warm concise"
    try:
        ext = self.data_cache.setdefault("assistant_extreme_state", {})
        if not isinstance(ext, dict):
            ext = {}
            self.data_cache["assistant_extreme_state"] = ext
        ext["voice_persona"] = clean
        ApexSessionLedger(self.data_cache).event("voice_persona", clean)
        self.persist_data_async()
        self._voice_set_status(f"Voice persona set to: {clean}")
    except Exception as exc:
        self._voice_set_status(f"Could not set voice persona: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_apex_set_voice_persona = on_apex_set_voice_persona


def on_apex_pin_memory(self: "DesktopMedSafeApp", memory_text: Any) -> None:
    clean = sanitize_text(memory_text, max_chars=900)
    if not clean:
        self._voice_set_status("No memory text to pin.")
        return
    try:
        if "ExtremeMemoryLab" in globals():
            ExtremeMemoryLab(self.data_cache).add_pin(clean, source="apex_command", confidence=0.97)
        ApexSessionLedger(self.data_cache).event("pinned_memory", clean)
        ApexSnapshotManager(self.data_cache).create(reason="pinned_memory")
        self.persist_data_async()
        self._voice_set_status(f"Pinned memory: {sanitize_text(clean, max_chars=180)}")
    except Exception as exc:
        self._voice_set_status(f"Pin memory failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_apex_pin_memory = on_apex_pin_memory


APEX_ORIGINAL_REFRESH_CONTEXT_PANEL = DesktopMedSafeApp.refresh_assistant_context_panel


def _apex_refresh_assistant_context_panel(self: "DesktopMedSafeApp") -> None:
    APEX_ORIGINAL_REFRESH_CONTEXT_PANEL(self)
    try:
        box = self.ids.get("assistant_context_window_report")
        if box is None:
            return
        current = sanitize_text(getattr(box, "text", ""), max_chars=7000)
        session = ApexSessionLedger(self.data_cache).capsule(max_chars=1400)
        snaps = ApexSnapshotManager(self.data_cache).report()
        state = self.data_cache.get("assistant_apex_state") if isinstance(self.data_cache.get("assistant_apex_state"), dict) else {}
        extra = "\n\n".join(
            [
                f"Apex runtime: version={APEX_PATCH_VERSION}; last risk={state.get('last_level', 'low')}; reason={state.get('last_reason', 'none')}",
                session,
                snaps,
            ]
        )
        box.text = sanitize_text(current + "\n\n" + extra, max_chars=12000)
    except Exception:
        pass


DesktopMedSafeApp.refresh_assistant_context_panel = _apex_refresh_assistant_context_panel


APEX_ORIGINAL_DESKTOP_INIT = DesktopMedSafeApp.__init__


def _apex_desktop_init(self: "DesktopMedSafeApp", *args: Any, **kwargs: Any) -> None:
    APEX_ORIGINAL_DESKTOP_INIT(self, *args, **kwargs)
    try:
        ApexSessionLedger(self.data_cache).event("app_init", APEX_PATCH_VERSION)
        ApexSnapshotManager(self.data_cache).create(reason="app_init")
    except Exception:
        pass


DesktopMedSafeApp.__init__ = _apex_desktop_init


APEX_ORIGINAL_BUILD_ASSISTANT_TAB = DesktopMedSafeApp._build_assistant_tab


def _apex_build_assistant_tab(self: "DesktopMedSafeApp", tab: Any) -> None:
    APEX_ORIGINAL_BUILD_ASSISTANT_TAB(self, tab)
    if ctk is None:
        return
    voice_frame = self.ids.get("voice_frame")
    if voice_frame is None:
    ...
    try:
        voice_frame.grid_columnconfigure((0, 1, 2, 3), weight=1)
        health = self._button(voice_frame, text="Apex Health", command=self.on_apex_health_check, tone="neutral")
        health.grid(row=6, column=0, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_apex_health_button"] = health
        snap = self._button(voice_frame, text="Snapshot", command=self.on_apex_memory_snapshot, tone="neutral")
        snap.grid(row=6, column=1, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_apex_snapshot_button"] = snap
        capsule = self._button(voice_frame, text="Session Capsule", command=self.on_apex_session_capsule, tone="neutral")
        capsule.grid(row=6, column=2, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_apex_capsule_button"] = capsule
        privacy = self._button(voice_frame, text="Privacy Lock", command=self.on_apex_privacy_lockdown, tone="neutral")
        privacy.grid(row=6, column=3, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_apex_privacy_button"] = privacy
    except Exception:
        pass


DesktopMedSafeApp._build_assistant_tab = _apex_build_assistant_tab
# --- end apex production hardening voice/memory/therapist layer ---------------

# --- sovereign v7 autonomy/privacy/recovery voice supervisor -----------------
# This layer keeps the app single-file. It hardens consent/privacy controls,
# adds a local voice timeline, improves fallback behavior when model/audio tools
# are missing, and adds recovery-focused urge planning without adding services.

SOVEREIGN_PATCH_VERSION = "2026-05-08.sovereign.local-therapist-voice-memory.v7"
SOVEREIGN_MAX_TIMELINE = 360
SOVEREIGN_MAX_RECOVERY_PLANS = 80
SOVEREIGN_MAX_POLICY_EVENTS = 180
SOVEREIGN_DEFAULT_RETENTION_DAYS = 45
SOVEREIGN_CRISIS_BYPASS = "sovereign_critical_bypass"


def _sovereign_now_iso() -> str:
    try:
        return datetime.now().isoformat(timespec="seconds")
    except Exception:
        return str(int(time.time()))
...
def _sovereign_norm(value: Any, *, max_chars: int = 4000) -> str:
    return re.sub(r"\s+", " ", sanitize_text(value, max_chars=max_chars)).strip()


def _sovereign_hash(value: Any, *, size: int = 16) -> str:
    try:
        return hashlib.sha256(_sovereign_norm(value, max_chars=50000).encode("utf-8", errors="ignore")).hexdigest()[:size]
    except Exception:
        return uuid.uuid4().hex[:size]


def _sovereign_append(container: Dict[str, Any], key: str, item: Dict[str, Any], limit: int) -> None:
    rows = container.setdefault(key, [])
    if not isinstance(rows, list):
        rows = []
        container[key] = rows
...
def _sovereign_redact(value: Any, *, max_chars: int = 4000) -> str:
    text = sanitize_text(value, max_chars=max_chars)
    if not text:
        return ""
    patterns = (
        ...
        (r"\b(?:api[_-]?key|token|secret|password|passphrase)\s*[:=]\s*[^\s,;]+", "[secret]"),
...
class SovereignConsentLedger:
    def __init__(self, data: Dict[str, Any]) -> None:
        self.data = data
        state = data.setdefault("assistant_sovereign_state", {})
        if not isinstance(state, dict):
            state = {}
            data["assistant_sovereign_state"] = state
        state.setdefault("schema", "SovereignConsentLedger/v7")
        state.setdefault("version", SOVEREIGN_PATCH_VERSION)
        state.setdefault("local_only", True)
        state.setdefault("memory_capture_enabled", True)
        state.setdefault("voice_output_enabled", True)
        state.setdefault("listener_enabled_by_policy", True)
        state.setdefault("redaction_enabled", True)
        state.setdefault("privacy_lockdown", False)
        state.setdefault("autonomous_actions", False)
        state.setdefault("retention_days", SOVEREIGN_DEFAULT_RETENTION_DAYS)
        state.setdefault("last_policy_decision", "allow")
        state.setdefault("last_policy_reason", "default local-only policy")
        state.setdefault("timeline", [])
        state.setdefault("policy_events", [])
        self.state = state

    def redact(self, text: Any, *, max_chars: int = 4000) -> str:
        if bool(self.state.get("redaction_enabled", True)):
            return _sovereign_redact(text, max_chars=max_chars)
        return sanitize_text(text, max_chars=max_chars)

    def set_flag(self, name: str, value: bool, reason: str = "manual") -> None:
        if name not in {
            "memory_capture_enabled",
            "voice_output_enabled",
            "listener_enabled_by_policy",
            "redaction_enabled",
            "privacy_lockdown",
            "autonomous_actions",
            "local_only",
        ...
        self.policy_event("policy_change", f"{name}={bool(value)}", {"reason": sanitize_text(reason, max_chars=120)})

    def set_retention_days(self, days: int) -> None:
        clean = max(1, min(3650, int(days)))
        self.state["retention_days"] = clean
        self.policy_event("retention_change", f"retention_days={clean}")

    def allow(self, action: str, text: Any = "") -> Tuple[bool, str]:
        privacy = bool(self.state.get("privacy_lockdown", False))
        action_key = sanitize_text(action, max_chars=60).lower()
        reason = "allowed by local-only policy"
        allowed = True
        if privacy and action_key in {"memory", "voice", "listener", "timeline", "tts"}:
            allowed = False
            reason = "privacy lockdown blocks memory/listener/voice timeline actions"
        elif action_key in {"memory", "timeline"} and not bool(self.state.get("memory_capture_enabled", True)):
            allowed = False
            reason = "memory capture is disabled"
        elif action_key in {"voice", "tts"} and not bool(self.state.get("voice_output_enabled", True)):
            allowed = False
            reason = "voice output is disabled by policy"
        elif action_key == "listener" and not bool(self.state.get("listener_enabled_by_policy", True)):
            allowed = False
            reason = "listener is disabled by policy"
        self.state["last_policy_decision"] = "allow" if allowed else "deny"
        self.state["last_policy_reason"] = reason
        if not allowed:
            self.policy_event("policy_deny", f"{action_key}: {reason}", {"hash": _sovereign_hash(text)})
        return allowed, reason

    def policy_event(self, kind: str, text: Any, meta: Optional[Dict[str, Any]] = None) -> None:
        item = {
            "id": uuid.uuid4().hex[:12],
            "ts": time.time(),
            "iso": _sovereign_now_iso(),
            "kind": sanitize_text(kind, max_chars=60),
            "text": self.redact(text, max_chars=900),
            "meta": meta or {},
        }
        _sovereign_append(self.state, "policy_events", item, SOVEREIGN_MAX_POLICY_EVENTS)

    def timeline(self, kind: str, text: Any, meta: Optional[Dict[str, Any]] = None) -> None:
        allowed, _reason = self.allow("timeline", text)
        if not allowed:
            return
        item = {
            "id": uuid.uuid4().hex[:12],
            ...
            "iso": _sovereign_now_iso(),
            "kind": sanitize_text(kind, max_chars=60),
            "hash": _sovereign_hash(text),
            "text": self.redact(text, max_chars=1200),
            "meta": meta or {},
        }
        _sovereign_append(self.state, "timeline", item, SOVEREIGN_MAX_TIMELINE)

    def timeline_report(self, *, limit: int = 18, max_chars: int = 2600) -> str:
        rows = self.state.get("timeline") if isinstance(self.state.get("timeline"), list) else []
        if not rows:
            return "Sovereign Timeline: no captured local voice/session events yet."
        picked = rows[-max(1, min(60, int(limit))):]
        lines = [f"Sovereign Timeline Replay ({len(picked)}/{len(rows)} most recent)"]
        for item in picked:
            if not isinstance(item, dict):
                continue
            lines.append(
                f"- {sanitize_text(item.get('iso') or '', max_chars=32)} | {sanitize_text(item.get('kind') or '', max_chars=36)} | {sanitize_text(item.get('text') or '', max_chars=180)}"
            )
        return sanitize_text("\n".join(lines), max_chars=max_chars)

    def status(self) -> str:
        lines = [
            "Sovereign Consent/Privacy Status",
            f"version: {SOVEREIGN_PATCH_VERSION}",
            f"local_only: {bool(self.state.get('local_only', True))}",
            f"privacy_lockdown: {bool(self.state.get('privacy_lockdown', False))}",
            f"memory_capture: {bool(self.state.get('memory_capture_enabled', True))}",
            f"voice_output: {bool(self.state.get('voice_output_enabled', True))}",
            f"listener_allowed: {bool(self.state.get('listener_enabled_by_policy', True))}",
            f"redaction: {bool(self.state.get('redaction_enabled', True))}",
            f"autonomous_actions: {bool(self.state.get('autonomous_actions', False))}",
            f"retention_days: {int(self.state.get('retention_days') or SOVEREIGN_DEFAULT_RETENTION_DAYS)}",
            f"last_decision: {self.state.get('last_policy_decision', 'allow')} | {self.state.get('last_policy_reason', 'none')}",
            f"timeline_events: {len(self.state.get('timeline') or [])} | policy_events: {len(self.state.get('policy_events') or [])}",
        ]
        return "\n".join(lines)

    def purge_expired(self) -> Dict[str, Any]:
        days = max(1, int(self.state.get("retention_days") or SOVEREIGN_DEFAULT_RETENTION_DAYS))
        cutoff = time.time() - days * 86400.0
        removed: Dict[str, int] = {}
        for key in ("timeline", "policy_events"):
            rows = self.state.get(key)
            if not isinstance(rows, list):
                continue
            kept = [x for x in rows if not isinstance(x, dict) or safe_float(x.get("ts") or 0) >= cutoff]
            removed[key] = len(rows) - len(kept)
            self.state[key] = kept
        for key in ("assistant_voice_vector_memory", "assistant_advanced_memory_graph", "assistant_extreme_memory"):
            obj = self.data.get(key)
            if not isinstance(obj, dict):
                continue
            for list_key in ("docs", "episodic", "ledger", "audit"):
                rows = obj.get(list_key)
                if not isinstance(rows, list):
                    continue
                kept = [x for x in rows if not isinstance(x, dict) or safe_float(x.get("ts") or x.get("created_ts") or 0) >= cutoff]
                removed[f"{key}.{list_key}"] = len(rows) - len(kept)
                obj[list_key] = kept
        self.policy_event("retention_purge", json.dumps(removed, sort_keys=True))
        return removed


class SovereignSafetyKernel:
    CRITICAL = (
        ("explicit self-harm intent", r"\b(i am going to kill myself|i'm going to kill myself|kill myself tonight|end my life tonight|suicide plan|i have a plan to die|i can't stay safe|i cannot stay safe)\b"),
        ("possible overdose/poisoning emergency", r"\b(overdosed|overdose|took too much|took a whole bottle|poisoned myself|can't breathe|cannot breathe|unconscious|passed out|seizure|blue lips)\b"),
        ("violence or weapon danger", r"\b(i am going to kill someone|hurt someone right now|weapon in my hand|gun in my hand|knife in my hand)\b"),
    )
    HIGH = (
        ("self-harm escalation", r"\b(want to die|wish i was dead|hurt myself|self harm|cut myself|not safe alone|unsafe alone)\b"),
        ("acute relapse risk", r"\b(about to relapse|going to use|dealer is here|craving is unbearable|can't stop myself|cannot stop myself)\b"),
        ("medical uncertainty", r"\b(double dose|mixed pills|severe side effect|withdrawal seizure|chest pain|shortness of breath)\b"),
    )

    def __init__(self, data: Dict[str, Any], mode: str = "General") -> None:
        self.data = data
        self.mode = normalize_assistant_mode(mode)
        state = data.setdefault("assistant_sovereign_safety", {})
        if not isinstance(state, dict):
            state = {}
            data["assistant_sovereign_safety"] = state
        state.setdefault("schema", "SovereignSafetyKernel/v7")
        self.state = state

    def classify(self, text: Any) -> Dict[str, Any]:
        clean = sanitize_text(text, max_chars=6000)
        low = clean.lower()
        level = "low"
        reason = "general support"
        matched: List[str] = []
        for label, pattern in self.CRITICAL:
            if re.search(pattern, low):
                level = "critical"
                reason = label
                matched.append(label)
        if level != "critical":
            for label, pattern in self.HIGH:
                if re.search(pattern, low):
                    level = "high"
                    reason = label
                    matched.append(label)
        route = self.route(low, level)
        decision = {
            "level": level,
            "reason": reason,
            "route": route,
            "matched": matched[:6],
            "mode": self.mode,
            "hash": _sovereign_hash(clean),
        }
        self.state["last_decision"] = decision
        self.state["last_ts"] = time.time()
        return decision

    def route(self, lowered: str, level: str) -> str:
        if level == "critical":
            return "direct_live_help_bypass"
        if any(token in lowered for token in ("craving", "relapse", "sober", "sobriety", "use again", "recovery")):
            return "recovery_urge_plan"
        if any(token in lowered for token in ("panic", "flashback", "dissociat", "ground", "spiral")):
            return "grounding_protocol"
        if any(token in lowered for token in ("dose", "pill", "med", "mg", "interaction", "side effect")):
            return "med_safety_boundary"
        if any(token in lowered for token in ("remember", "forget", "memory", "privacy", "timeline")):
            return "memory_policy"
        return "supportive_chat"

    def direct_reply(self, decision: Dict[str, Any]) -> str:
        reason = sanitize_text(decision.get("reason") or "possible immediate danger", max_chars=180)
        return (
            "I’m going to treat this as urgent. I’m not an emergency service, clinician, or crisis line. "
            "Please contact local emergency services or a crisis hotline now, or ask a trusted person to stay with you in person.\n\n"
            "Do these first: move away from anything that could be used for harm or overdose; get to a visible/public place if possible; "
            "send one direct message: “I’m not safe alone right now. Please call me or come over.”\n\n"
            f"Reason I’m escalating: {reason}."
        )


class SovereignRecoveryPlanner:
    def __init__(self, data: Dict[str, Any]) -> None:
        self.data = data
        state = data.setdefault("assistant_sovereign_recovery", {})
        if not isinstance(state, dict):
            state = {}
            data["assistant_sovereign_recovery"] = state
        state.setdefault("schema", "SovereignRecoveryPlanner/v7")
        state.setdefault("plans", [])
        state.setdefault("urge_log", [])
        self.state = state

    def analyze(self, text: Any) -> Dict[str, Any]:
        clean = sanitize_text(text, max_chars=3000)
        low = clean.lower()
        halt = []
        if re.search(r"\bhungry|haven't eaten|havent eaten|low blood sugar\b", low):
            halt.append("hungry")
        if re.search(r"\bangry|rage|resent|furious\b", low):
            halt.append("angry")
        if re.search(r"\blonely|alone|isolated\b", low):
            halt.append("lonely")
        if re.search(r"\btired|exhausted|sleep deprived|can't sleep|cannot sleep\b", low):
            halt.append("tired")
        intensity = 0
        m = re.search(r"\b(\d{1,2})\s*/\s*10\b", low)
        if m:
            intensity = max(0, min(10, int(m.group(1))))
        elif any(w in low for w in ("unbearable", "about to", "can't stop", "cannot stop")):
            intensity = 9
        elif any(w in low for w in ("craving", "urge", "trigger")):
            intensity = 6
        return {"halt": halt, "intensity": intensity, "hash": _sovereign_hash(clean)}

    def plan(self, text: Any, *, source: str = "manual") -> str:
        info = self.analyze(text)
        halt = info.get("halt") or []
        intensity = int(info.get("intensity") or 0)
        if intensity >= 8:
            first = "For the next 10 minutes: do not negotiate with the urge; change rooms, put distance between you and access, and contact one safe person."
        elif intensity >= 5:
            first = "For the next 10 minutes: delay, hydrate, breathe slowly, and do one visible action that protects the streak."
        else:
            first = "For the next 10 minutes: keep it small and concrete; one supportive action is enough."
        halt_line = "HALT flags: " + (", ".join(halt) if halt else "none obvious from the text")
        steps = [
            "Sovereign Relapse/Urge Plan",
            first,
            halt_line,
            "1) Name the urge: “This is an urge, not a command.”",
            "2) Create friction: move away from triggers, money, contacts, substances, or tools.",
            "3) Regulate: 30 slow breaths or cold water on hands/face.",
            "4) Connect: message/call one safe person or support line.",
            "5) Replace: food, shower, walk, meeting, journal, or sleep reset depending on HALT.",
        ]
        plan_text = "\n".join(steps)
        item = {"id": uuid.uuid4().hex[:12], "ts": time.time(), "iso": _sovereign_now_iso(), "source": source, "intensity": intensity, "halt": halt, "text": plan_text}
        _sovereign_append(self.state, "plans", item, SOVEREIGN_MAX_RECOVERY_PLANS)
        _sovereign_append(self.state, "urge_log", {k: item[k] for k in ("id", "ts", "iso", "intensity", "halt", "source")}, SOVEREIGN_MAX_RECOVERY_PLANS)
        return plan_text

    def report(self) -> str:
        plans = self.state.get("plans") if isinstance(self.state.get("plans"), list) else []
        urges = self.state.get("urge_log") if isinstance(self.state.get("urge_log"), list) else []
        lines = [f"Sovereign Recovery Planner: plans={len(plans)} urge_logs={len(urges)}"]
        for item in plans[-5:]:
            if isinstance(item, dict):
                lines.append(f"- {item.get('iso')} intensity={item.get('intensity')} halt={','.join(item.get('halt') or []) or 'none'}")
        return "\n".join(lines)


class SovereignFallbackResponder:
    def __init__(self, data: Dict[str, Any], mode: str = "General") -> None:
        self.data = data
        self.mode = normalize_assistant_mode(mode)

    def reply(self, prompt: Any, *, reason: str = "model unavailable", decision: Optional[Dict[str, Any]] = None) -> str:
        clean = sanitize_text(prompt, max_chars=900)
        route = (decision or {}).get("route", "supportive_chat")
        if route == "recovery_urge_plan" or self.mode == "Recovery Coach":
            plan = SovereignRecoveryPlanner(self.data).plan(clean, source="fallback")
            return plan + f"\n\nFallback note: Gemma response path was unavailable ({sanitize_text(reason, max_chars=120)}), so I used the local deterministic recovery planner."
        if route == "grounding_protocol" or self.mode == "Therapy":
            return (
                "Sovereign Grounding Fallback\n"
                "1) Put both feet on the floor and name five things you see.\n"
                "2) Exhale longer than you inhale for ten breaths.\n"
                "3) Name the feeling in one sentence, without solving it yet.\n"
                "4) Choose one tiny next action: water, food, text someone, step outside, or rest.\n\n"
                f"Fallback note: {sanitize_text(reason, max_chars=160)}."
            )
        return (
            "The local model path was unavailable, so I’m using a deterministic fallback. "
            "Here is a safe next step: write the exact question in one sentence, check anything medication-related with a clinician/pharmacist, "
            "and use Therapy/Recovery mode for support planning rather than medical decisions.\n\n"
            f"Fallback reason: {sanitize_text(reason, max_chars=160)}."
        )


class SovereignPromptCompiler:
    def __init__(self, data: Dict[str, Any], mode: str = "General") -> None:
        self.data = data
        self.mode = normalize_assistant_mode(mode)

    def compile(self, prompt: Any, decision: Dict[str, Any]) -> str:
        consent = SovereignConsentLedger(self.data)
        recovery = SovereignRecoveryPlanner(self.data).report()
        privacy = consent.status()
        voice_profile = self.data.get("assistant_extreme_state") if isinstance(self.data.get("assistant_extreme_state"), dict) else {}
        lines = [
            "[sovereign_v7_control_packet]",
            f"version: {SOVEREIGN_PATCH_VERSION}",
            f"mode: {self.mode}",
            f"safety_level: {decision.get('level')} | route: {decision.get('route')} | reason: {decision.get('reason')}",
            "therapist_boundary: supportive coaching only; do not claim to be licensed therapy, emergency care, or medical prescribing.",
            "memory_policy: local encrypted vault only; treat memory as fallible user-provided context, not clinical evidence.",
            "response_policy: ask for consent before sensitive memory use when possible; prefer one grounded next step; voice answers should be concise unless user asks for depth.",
            "privacy_redaction: redact secrets, phone/email/card-like identifiers in logs and reports.",
            f"voice_persona: {sanitize_text(voice_profile.get('voice_persona') or 'warm concise', max_chars=80)}",
            sanitize_text(privacy, max_chars=1600),
            sanitize_text(recovery, max_chars=900),
            "[/sovereign_v7_control_packet]",
        ]
        return "\n".join(lines)


class SovereignVoiceCommandRouter:
    def __init__(self, app: "DesktopMedSafeApp") -> None:
        self.app = app

    def handle(self, text: Any) -> bool:
        clean = _sovereign_norm(text, max_chars=1800)
        low = clean.lower()
        if not clean:
            return True
        ledger = SovereignConsentLedger(self.app.data_cache)
        try:
            if low in {"consent status", "privacy status", "sovereign status"}:
                self.app.on_sovereign_consent_status()
                action = "consent_status"
            elif low in {"timeline replay", "replay timeline", "what did i say", "replay last"}:
                self.app.on_sovereign_timeline_replay()
                action = "timeline_replay"
            elif low in {"memory off", "disable memory", "stop remembering"}:
                ledger.set_flag("memory_capture_enabled", False, "voice_command")
                self.app._voice_set_status("Sovereign memory capture disabled.")
                action = "memory_off"
            elif low in {"memory on", "enable memory", "start remembering"}:
                ledger.set_flag("memory_capture_enabled", True, "voice_command")
                self.app._voice_set_status("Sovereign memory capture enabled.")
                action = "memory_on"
            elif low in {"voice off", "mute voice", "disable voice"}:
                ledger.set_flag("voice_output_enabled", False, "voice_command")
                setattr(self.app, "voice_tts_enabled", False)
                self.app.stop_bark_voice()
                self.app._voice_set_status("Sovereign voice output disabled.")
                action = "voice_off"
            elif low in {"voice on", "unmute voice", "enable voice"}:
                ledger.set_flag("voice_output_enabled", True, "voice_command")
                setattr(self.app, "voice_tts_enabled", True)
                self.app._voice_set_status("Sovereign voice output enabled.")
                action = "voice_on"
            elif low in {"privacy lockdown", "lock privacy", "privacy lock"}:
                self.app.on_sovereign_privacy_lockdown()
                action = "privacy_lockdown"
            elif low in {"privacy unlock", "unlock privacy"}:
                ledger.set_flag("privacy_lockdown", False, "voice_command")
                self.app._voice_set_status("Sovereign privacy lockdown cleared. Memory and voice still follow their individual toggles.")
                action = "privacy_unlock"
            elif low in {"purge old memory", "trim old memory", "apply retention"}:
                self.app.on_sovereign_purge_retention()
                action = "purge_retention"
            elif m := re.search(r"\bretention\s+(\d{1,4})\s+days?\b", low):
                ledger.set_retention_days(int(m.group(1)))
                self.app._voice_set_status(f"Sovereign retention set to {int(m.group(1))} days.")
                action = "retention_days"
            elif low in {"relapse plan", "urge plan", "craving plan", "sober plan"} or low.startswith("urge plan "):
                plan_prompt = re.sub(r"^(relapse plan|urge plan|craving plan|sober plan)\s*", "", clean, flags=re.I).strip() or clean
                self.app.on_sovereign_recovery_plan(plan_prompt)
                action = "recovery_plan"
            elif low in {"sovereign diagnostics", "sovereign diagnostic", "advanced diagnostics"}:
                self.app.on_sovereign_diagnostics()
                action = "diagnostics"
            elif low in {"fallback test", "test fallback"}:
                text = SovereignFallbackResponder(self.app.data_cache, getattr(self.app, "assistant_mode", "General")).reply("test fallback", reason="manual test")
                self.app._voice_set_status(text)
                action = "fallback_test"
            else:
                return False
            ledger.timeline("voice_command", clean, {"action": action})
            try:
                self.app.persist_data_async()
            except Exception:
                pass
            ...
            try:
                self.app._voice_set_status(f"Sovereign command failed: {sanitize_text(exc, max_chars=180)}")
            except Exception:
                pass
            return True


SOVEREIGN_ORIGINAL_RUN_ASSISTANT_REQUEST = run_assistant_request
...
def run_assistant_request(
    ...
    active_mode = normalize_assistant_mode(mode)
    ledger = SovereignConsentLedger(data)
    ledger.purge_expired()
    kernel = SovereignSafetyKernel(data, active_mode)
    decision = kernel.classify(prompt)
    ledger.timeline("user_prompt", prompt, {"mode": active_mode, "route": decision.get("route"), "risk": decision.get("level")})
    if decision.get("level") == "critical":
        reply = kernel.direct_reply(decision)
        ledger.timeline(SOVEREIGN_CRISIS_BYPASS, reply, decision)
        if on_delta is not None:
            try:
                on_delta(reply)
            except Exception:
                pass
        return reply
    packet = SovereignPromptCompiler(data, active_mode).compile(prompt, decision)
    enhanced_prompt = "\n\n".join([sanitize_text(prompt, max_chars=3200), packet])
    try:
        result = SOVEREIGN_ORIGINAL_RUN_ASSISTANT_REQUEST(key, data, enhanced_prompt, selected_med_id, settings, mode=active_mode, on_delta=on_delta)
    except Exception as exc:
        result = SovereignFallbackResponder(data, active_mode).reply(prompt, reason=str(exc), decision=decision)
        if on_delta is not None:
            try:
                on_delta(result)
            except Exception:
                pass
    ledger.timeline("assistant_reply", result, {"mode": active_mode, "route": decision.get("route"), "risk": decision.get("level")})
    try:
        if "ApexSessionLedger" in globals():
            ApexSessionLedger(data).event("sovereign_turn", f"risk={decision.get('level')} route={decision.get('route')}", meta=decision)
    except Exception:
        pass
    return result


SOVEREIGN_ORIGINAL_VOICE_SUBMIT_TEXT = DesktopMedSafeApp._voice_submit_text


def _sovereign_voice_submit_text(self: "DesktopMedSafeApp", text: str) -> None:
    clean = _sovereign_norm(text, max_chars=1800)
    if not clean:
        return
    ledger = SovereignConsentLedger(self.data_cache)
    ledger.timeline("voice_heard", clean, {"mode": getattr(self, "assistant_mode", "General")})
    try:
        if SovereignVoiceCommandRouter(self).handle(clean):
            return
    except Exception as exc:
        try:
            self._voice_set_status(f"Sovereign command router skipped: {sanitize_text(exc, max_chars=180)}")
        except Exception:
            pass
    allowed, reason = ledger.allow("listener", clean)
    if not allowed:
        self._voice_set_status(f"Voice text blocked by policy: {reason}")
        return
    return SOVEREIGN_ORIGINAL_VOICE_SUBMIT_TEXT(self, clean)


DesktopMedSafeApp._voice_submit_text = _sovereign_voice_submit_text


SOVEREIGN_ORIGINAL_SPEAK_REPLY = DesktopMedSafeApp._voice_speak_reply_if_needed


def _sovereign_speak_reply_if_needed(self: "DesktopMedSafeApp", reply: str) -> None:
    ledger = SovereignConsentLedger(self.data_cache)
    allowed, reason = ledger.allow("voice", reply)
    if not allowed:
        try:
            self._voice_set_status(f"Voice suppressed: {reason}")
        except Exception:
            pass
        return
    return SOVEREIGN_ORIGINAL_SPEAK_REPLY(self, reply)


DesktopMedSafeApp._voice_speak_reply_if_needed = _sovereign_speak_reply_if_needed


SOVEREIGN_ORIGINAL_FINISH_STREAM = DesktopMedSafeApp._finish_assistant_live_stream


def _sovereign_finish_assistant_live_stream(self: "DesktopMedSafeApp", request_id: int, final_text: str, mode: str) -> None:
    SOVEREIGN_ORIGINAL_FINISH_STREAM(self, request_id, final_text, mode)
    if request_id != getattr(self, "assistant_request_id", 0):
        return
    try:
        ledger = SovereignConsentLedger(self.data_cache)
        ledger.timeline("stream_finished", final_text, {"mode": normalize_assistant_mode(mode)})
        if "ApexSnapshotManager" in globals():
            risk = (self.data_cache.get("assistant_sovereign_safety") or {}).get("last_decision", {}) if isinstance(self.data_cache.get("assistant_sovereign_safety"), dict) else {}
            if risk.get("level") in {"high", "critical"}:
                ApexSnapshotManager(self.data_cache).create(reason=f"sovereign_post:{risk.get('level')}")
        self.persist_data_async()
    except Exception as exc:
        try:
            self._voice_set_status(f"Sovereign ledger skipped: {sanitize_text(exc, max_chars=180)}")
        except Exception:
            pass


DesktopMedSafeApp._finish_assistant_live_stream = _sovereign_finish_assistant_live_stream


SOVEREIGN_ORIGINAL_DIAGNOSTICS = DesktopMedSafeApp._ultra_voice_diagnostics_text


def _sovereign_voice_diagnostics_text(self: "DesktopMedSafeApp") -> str:
    base = SOVEREIGN_ORIGINAL_DIAGNOSTICS(self)
    try:
        ledger = SovereignConsentLedger(self.data_cache)
        safety = self.data_cache.get("assistant_sovereign_safety") if isinstance(self.data_cache.get("assistant_sovereign_safety"), dict) else {}
        recovery = SovereignRecoveryPlanner(self.data_cache).report()
        extra = "\n".join(
            [
                f"Sovereign patch: {SOVEREIGN_PATCH_VERSION}",
                f"Sovereign last safety: {(safety.get('last_decision') or {}).get('level', 'low')} | route={(safety.get('last_decision') or {}).get('route', 'unknown')}",
                ledger.status(),
                recovery,
            ]
        )
        return sanitize_text(base + "\n" + extra, max_chars=9000)
    except Exception as exc:
        return sanitize_text(base + f"\nSovereign diagnostics unavailable: {sanitize_text(exc, max_chars=180)}", max_chars=9000)


DesktopMedSafeApp._ultra_voice_diagnostics_text = _sovereign_voice_diagnostics_text


def on_sovereign_consent_status(self: "DesktopMedSafeApp") -> None:
    try:
        text = SovereignConsentLedger(self.data_cache).status()
        self._voice_set_status(text)
        box = self.ids.get("assistant_context_window_report")
        if box is not None:
            box.text = sanitize_text(sanitize_text(getattr(box, "text", ""), max_chars=7000) + "\n\n" + text, max_chars=12000)
    except Exception as exc:
        self._voice_set_status(f"Sovereign status failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_sovereign_consent_status = on_sovereign_consent_status


def on_sovereign_timeline_replay(self: "DesktopMedSafeApp") -> None:
    try:
        text = SovereignConsentLedger(self.data_cache).timeline_report(limit=24, max_chars=3600)
        self._voice_set_status(text)
        box = self.ids.get("assistant_context_window_report")
        if box is not None:
            box.text = sanitize_text(sanitize_text(getattr(box, "text", ""), max_chars=7000) + "\n\n" + text, max_chars=12000)
    except Exception as exc:
        self._voice_set_status(f"Timeline replay failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_sovereign_timeline_replay = on_sovereign_timeline_replay


def on_sovereign_recovery_plan(self: "DesktopMedSafeApp", seed_text: Any = "") -> None:
    try:
        text = SovereignRecoveryPlanner(self.data_cache).plan(seed_text or "current urge", source="ui_or_voice")
        SovereignConsentLedger(self.data_cache).timeline("recovery_plan", text)
        self.persist_data_async()
        self._voice_set_status(text)
        try:
            self.append_assistant_message("assistant", text, mode="Recovery Coach", persist=True)
        except TypeError:
            try:
                self.append_assistant_message("assistant", text)
            except Exception:
        ...
        self._voice_set_status(f"Recovery plan failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_sovereign_recovery_plan = on_sovereign_recovery_plan


def on_sovereign_privacy_lockdown(self: "DesktopMedSafeApp") -> None:
    try:
        ledger = SovereignConsentLedger(self.data_cache)
        ledger.set_flag("privacy_lockdown", True, "ui_or_voice")
        ledger.set_flag("voice_output_enabled", False, "privacy_lockdown")
        setattr(self, "voice_tts_enabled", False)
        setattr(self, "voice_listener_enabled", False)
        setattr(self, "advanced_voice_listener_awake", False)
        try:
        ...
        try:
            ...
            pass
        if "ApexSnapshotManager" in globals():
            ApexSnapshotManager(self.data_cache).create(reason="sovereign_privacy_lockdown")
        self.persist_data_async()
        self._voice_set_status("Sovereign privacy lockdown: listener off, Bark muted/stopped, local policy snapshot saved.")
    except Exception as exc:
        self._voice_set_status(f"Sovereign privacy lockdown failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_sovereign_privacy_lockdown = on_sovereign_privacy_lockdown


def on_sovereign_purge_retention(self: "DesktopMedSafeApp") -> None:
    try:
        removed = SovereignConsentLedger(self.data_cache).purge_expired()
        self.persist_data_async()
        self._voice_set_status("Sovereign retention purge complete: " + json.dumps(removed, sort_keys=True)[:700])
    except Exception as exc:
        self._voice_set_status(f"Retention purge failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_sovereign_purge_retention = on_sovereign_purge_retention


def on_sovereign_diagnostics(self: "DesktopMedSafeApp") -> None:
    try:
        text = self._ultra_voice_diagnostics_text()
        self._voice_set_status(text)
        box = self.ids.get("assistant_context_window_report")
        if box is not None:
            box.text = sanitize_text(sanitize_text(getattr(box, "text", ""), max_chars=6000) + "\n\n" + text, max_chars=14000)
    except Exception as exc:
        self._voice_set_status(f"Sovereign diagnostics failed: {sanitize_text(exc, max_chars=180)}")


DesktopMedSafeApp.on_sovereign_diagnostics = on_sovereign_diagnostics


SOVEREIGN_ORIGINAL_REFRESH_CONTEXT_PANEL = DesktopMedSafeApp.refresh_assistant_context_panel


def _sovereign_refresh_assistant_context_panel(self: "DesktopMedSafeApp") -> None:
    SOVEREIGN_ORIGINAL_REFRESH_CONTEXT_PANEL(self)
    try:
        box = self.ids.get("assistant_context_window_report")
        if box is None:
            return
        current = sanitize_text(getattr(box, "text", ""), max_chars=8500)
        ledger = SovereignConsentLedger(self.data_cache)
        safety = self.data_cache.get("assistant_sovereign_safety") if isinstance(self.data_cache.get("assistant_sovereign_safety"), dict) else {}
        last = safety.get("last_decision") if isinstance(safety.get("last_decision"), dict) else {}
        extra = "\n\n".join(
            [
                f"Sovereign runtime: {SOVEREIGN_PATCH_VERSION}; last_level={last.get('level', 'low')}; route={last.get('route', 'unknown')}",
                ledger.status(),
                SovereignRecoveryPlanner(self.data_cache).report(),
            ]
        )
        box.text = sanitize_text(current + "\n\n" + extra, max_chars=15000)
    except Exception:
        pass


DesktopMedSafeApp.refresh_assistant_context_panel = _sovereign_refresh_assistant_context_panel


SOVEREIGN_ORIGINAL_DESKTOP_INIT = DesktopMedSafeApp.__init__


def _sovereign_desktop_init(self: "DesktopMedSafeApp", *args: Any, **kwargs: Any) -> None:
    SOVEREIGN_ORIGINAL_DESKTOP_INIT(self, *args, **kwargs)
    try:
        ledger = SovereignConsentLedger(self.data_cache)
        ledger.policy_event("app_init", SOVEREIGN_PATCH_VERSION)
        ledger.purge_expired()
    except Exception:
        pass


DesktopMedSafeApp.__init__ = _sovereign_desktop_init


SOVEREIGN_ORIGINAL_BUILD_ASSISTANT_TAB = DesktopMedSafeApp._build_assistant_tab


def _sovereign_build_assistant_tab(self: "DesktopMedSafeApp", tab: Any) -> None:
    SOVEREIGN_ORIGINAL_BUILD_ASSISTANT_TAB(self, tab)
    if ctk is None:
        return
    voice_frame = self.ids.get("voice_frame")
    if voice_frame is None:
    ...
    try:
        voice_frame.grid_columnconfigure((0, 1, 2, 3), weight=1)
        status = self._button(voice_frame, text="Sovereign Status", command=self.on_sovereign_consent_status, tone="neutral")
        status.grid(row=7, column=0, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_sovereign_status_button"] = status
        timeline = self._button(voice_frame, text="Timeline Replay", command=self.on_sovereign_timeline_replay, tone="neutral")
        timeline.grid(row=7, column=1, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_sovereign_timeline_button"] = timeline
        recovery = self._button(voice_frame, text="Urge Plan", command=lambda: self.on_sovereign_recovery_plan("manual urge plan"), tone="neutral")
        recovery.grid(row=7, column=2, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_sovereign_recovery_button"] = recovery
        purge = self._button(voice_frame, text="Purge Old", command=self.on_sovereign_purge_retention, tone="neutral")
        purge.grid(row=7, column=3, sticky="ew", padx=6, pady=(0, 12))
        self.ids["voice_sovereign_purge_button"] = purge
    except Exception:
        pass


DesktopMedSafeApp._build_assistant_tab = _sovereign_build_assistant_tab
# --- end sovereign v7 autonomy/privacy/recovery voice supervisor -------------
